...
const httpRequest = require('../../../lib/action-library/dist/misc/http-request').default
...
const urlTest = async (urls) => {
	return Bluebird.map(urls, (url) => {
		...
		return httpRequest(null, null, null, request)
			.then(_.constant(isResolved))
			.catch(_.constant(isRejected))
	})
}

ava.test('should retrieve valid urls', async (test) => {
	const urls = [
...
ava.test('should reject invalid urls', async (test) => {
	const urls = [
		'duff',
		'http://www.error.com/',
