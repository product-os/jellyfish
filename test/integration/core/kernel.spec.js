...
ava('.insertCard() should not materialize two equal links between the same cards', async (test) => {
	...
	const linkCard1 = await test.context.kernel.insertCard(
		test.context.context, test.context.kernel.sessions.admin, {
			slug: `link-${card1.slug}-is-attached-to-${card2.slug}-1`,
			type: 'link',
			name: 'is attached to',
			data: {
	...
	await test.context.kernel.insertCard(
		test.context.context, test.context.kernel.sessions.admin, {
			slug: `link-${card1.slug}-is-attached-to-${card2.slug}-2`,
			type: 'link',
			name: 'is attached to',
			data: {
				inverseName: 'has attached element',
	...
	const element1 = await test.context.kernel.getCardById(
		test.context.context, test.context.kernel.sessions.admin, card1.id)
	const element2 = await test.context.kernel.getCardById(
		test.context.context, test.context.kernel.sessions.admin, card2.id)

	test.deepEqual(element1.links['is attached to'], [
		{
			id: card2.id,
			slug: card2.slug,
			type: card2.type,
	...
	test.deepEqual(element2.links['has attached element'], [
		{
			id: card1.id,
			slug: card1.slug,
			type: card1.type,
...
ava('.insertCard() should not materialize two equal links between the same cards given inverses', async (test) => {
	const card1 = await test.context.kernel.insertCard(
		test.context.context, test.context.kernel.sessions.admin, {
			slug: 'foo-bar',
			type: 'card'
	...
	await test.context.kernel.insertCard(
		test.context.context, test.context.kernel.sessions.admin, {
			slug: `link-${card2.slug}-has-attached-element-${card1.slug}`,
			type: 'link',
