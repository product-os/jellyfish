...
exports.goto = async (page, path, { forceReload } = {}) => {
...
exports.createChatMessage = async (page, scopeSelector, messageText) => {
	...
	const sendCommand = await page.evaluate(async () => {
		return document
			.querySelector('[data-test-send-command]')
			.getAttribute('data-test-send-command');
	});
	...
	await page.waitForSelector(
		`${scopeSelector} [data-test="event-card__message"]`,
		exports.WAIT_OPTS,
	);
};
...
exports.waitForSelectorToDisappear = async (
	page,
	selector,
	retryCount = 30,
) => {
...
exports.waitForThenDismissAlert = async (page, alertPrefix) => {
	const notificationButtonSelector = `//*[contains(@class, "notification-item")][//*[contains(text(), "${alertPrefix}")]]//button`;
...
exports.navigateToHomeChannelItem = async (page, menuStack) => {
	while (menuStack.length) {
		...
		const isExpanded = await page.evaluate((item) => {
			return item.getAttribute('data-expanded');
		}, menuItem);
		if (isExpanded === 'false') {
...
exports.waitForSelectorInsideScrollable = async (
	page,
	scrollableSelector,
	selector,
) => {
	...
	while (true) {
		...
		const result = await page.evaluateHandle(lookForElementInsideScrollable, {
			elemSelector: scrollableSelector,
			selector,
		});
...
exports.waitForSelectorInsideScrollableToDisappear = async (
	page,
	scrollableSelector,
	selector,
) => {
	...
	await page.route('**/*', async (route) => {
		...
		if (url.href.startsWith(oauthUrl)) {
			...
			await route.fulfill({
				...
				headers: {
					Location: `${
						environment.livechat.host
					}/oauth/callback?state=${encodeURIComponent(state)}&code=${code}`,
				},
			});
		} else if (
			url.pathname === '/api/v2/oauth/oauth-provider-balena-api@1.0.0'
		) {
			const body = route.request().postDataJSON();

			if (
				body.slug !== `${user.card.slug}@${user.card.version}` ||
				body.code !== code
			) {
