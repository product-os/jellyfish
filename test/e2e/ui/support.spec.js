...
test('Updates to support threads should be reflected in the support thread list', async ({
	...
	await page
		.locator('[data-test="lens-selector--lens-support-threads"]')
		.click();
...
test('Updates to messages should be reflected in the threads timeline', async ({
	...
	await page.evaluate(
		...
		},
		{
			...
			newMessage: messageTextAfter,
		},
	);

	// Wait for the updated message text to appear
	await page.waitForSelector(
		`//*[@data-test="event-card__message"]//p[text()="${messageTextAfter}"]`,
	);
});
...
test('Support thread timeline should default to sending whispers', async ({
	...
	const messageText = await macros.getElementText(
		page,
		'.event-card--whisper [data-test="event-card__message"]',
	);
	expect(messageText.trim()).toEqual(rand);
});
...
test('Support thread timeline should send a message if the input is prefixed with a "%" character', async ({
	...
	const messageText = await macros.getElementText(
		page,
		'.event-card--message [data-test="event-card__message"]',
	);
	expect(messageText.trim()).toEqual(rand);
});
...
test.skip('Users should be able to audit a support thread', async ({
	...
	const threadWithIssue = await page.evaluate((id) => {
		return window.sdk.card.getWithLinks(
			id,
			'support thread is attached to improvement',
		);
	}, supportThread.id);
	const issueFromDB =
		threadWithIssue.links['support thread is attached to improvement'][0];
	expect(issueFromDB.name).toEqual(name);
	expect(issueFromDB.type).toEqual(
		'improvement',
		'Should be able to create a new improvement',
	);
	expect(issueFromDB.data.repository).toEqual(
		'balena-io/balena',
		'The issue should be created on the balena product repo',
	);
	...
	for (const score of scores) {
		...
		await page
			.locator(`[data-test="feedback-form__${score.slug}--${identifier}"]`)
			.click();
	}
...
test('A user can edit their own message', async ({ page }) => {
	const supportThread = await page.evaluate(() => {
		return window.sdk.card.create({
			type: 'support-thread@1.0.0',
			data: {
	...
	await page
		.locator(`${eventSelector} [data-test="event-header__link--edit-message"]`)
		.click();
	...
	const newMessageText = await macros.getElementText(
		page,
		`${eventSelector} [data-test="event-card__message"]`,
	);
	...
	await page.waitForSelector(
		`${eventSelector} [data-test="event-card--edited-at"]`,
	);
});
...
test('You can select a user and a group from the auto-complete options', async ({
	...
	const userMatch = await macros.getElementText(
		page,
		'.rta__autocomplete .rta__item:first-child',
	);
	...
	const groupMatch = await macros.getElementText(
		page,
		'.rta__autocomplete .rta__item:first-child',
	);
...
test('Should be able to filter support threads by timeline message', async ({
	...
	await macros.setInputValue(
		page,
		selectors.filterModalValueInput,
		thread1.messagePayload,
	);
