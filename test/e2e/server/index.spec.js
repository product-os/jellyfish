...
ava.serial(
	...
	async (test) => {
		...
		const result1 = await test.context.http('POST', '/api/v2/action', {
			card: `${user.slug}@${user.version}`,
			type: 'user',
			action: 'action-create-session@1.0.0',
			arguments: {
		...
		const result3 = await test.context.http('POST', '/api/v2/action', {
			card: `${user.slug}@${user.version}`,
			type: 'user',
			action: 'action-create-session@1.0.0',
			arguments: {
...
ava.serial(
	'should fail with a user error given no input card',
	async (test) => {
		const result = await test.context.http('POST', '/api/v2/action', {
			type: 'user',
			action: 'action-create-session@1.0.0',
...
ava.serial('should fail to query with a non JSON string', async (test) => {
	const result = await test.context.http('POST', '/api/v2/query', 'foo:bar', {
		Authorization: `Bearer ${test.context.token}`
	})
...
ava.serial('should get all elements by type', async (test) => {
	...
	const result = await test.context.http('GET', '/api/v2/type/user', null, {
