...
ava.serial.only(
	'/api/v2/registry should return 400 if the token isn\'t present',
	async (test) => {
		const actor = await sdk.auth.whoami()
		const b64Auth = b64encode(`${actor.slug}`)

		const result = await helpers.http('GET', '/api/v2/registry', null, {
...
ava.serial(
	'/api/v2/registry should return 401 if the user isn\'t valid',
	async (test) => {
...
ava.serial(
	'/api/v2/registry should return 401 if the token isn\'t valid',
	async (test) => {
		const actor = await sdk.auth.whoami()
...
ava.serial(
	'/api/v2/registry should return a JWT with no scope (this happens on "docker login")',
	async (test) => {
		const actor = await sdk.auth.whoami()
		const token = sdk.getAuthToken()
		...
		const decoded = jsonwebtoken.verify(
			...
			b64decode(REGISTRY_TOKEN_AUTH_CERT_PUB),
			{
				algorithms: [ 'ES256' ]
			}
		)
...
ava.serial(
	'/api/v2/registry should return a JWT with empty access permissions if the scope is not accessible',
	async (test) => {
		const actor = await sdk.auth.whoami()
		const token = sdk.getAuthToken()
		const b64Auth = b64encode(`${actor.slug}:${token}`)
...
ava.serial(
	'/api/v2/registry should return a JWT with correct access permissions when a scope is set',
	async (test) => {
		const actor = await sdk.auth.whoami()
		const token = sdk.getAuthToken()
		const b64Auth = b64encode(`${actor.slug}:${token}`)
		...
		test.is(
			decoded.access.length,
			1,
			'The access array should contain 1 entry'
		)
		test.is(
			decoded.access[0].name,
			thread.slug,
			'Access should be provided to the card slug'
		)
		test.deepEqual(
			decoded.access[0].actions,
			[ 'push', 'pull' ],
			'Pull and push actions should be authorized'
		)
	}
)
