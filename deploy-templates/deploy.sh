...
kubectl_release=v1.18.2 \
  ...
  && [[ "6ea8261b503c6c63d616878837dc70b758d4a3aeb9996ade8e83b51aedac9698" == "$(openssl sha1 -sha256 kubectl | awk '{print $2}')" ]] \
  && chmod +x ./kubectl \
...
command -v aws || (wget -O awscliv2.zip https://awscli.amazonaws.com/awscli-exe-linux-x86_64-2.0.9.zip \
  && [[ "90b250a0995ec177f63b76a4808f908c9b80e8584e3f625ef468b8e52621b23f" == "$(openssl sha1 -sha256 awscliv2.zip | awk '{print $2}')" ]] \
  && unzip awscliv2.zip \
  && ./aws/install \
  && aws --version)
...
command -v aws-iam-authenticator || (wget -O aws-iam-authenticator https://amazon-eks.s3.us-west-2.amazonaws.com/1.15.10/2020-02-22/bin/linux/amd64/aws-iam-authenticator \
  && [[ "fe958eff955bea1499015b45dc53392a33f737630efd841cd574559cc0f41800" == "$(openssl sha1 -sha256 aws-iam-authenticator | awk '{print $2}')" ]] \
