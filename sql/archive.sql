...
BEGIN
    LOOP
        ...
        SELECT array_agg(id) INTO contract_ids FROM cards WHERE (type='execute@1.0.0' OR type='session@1.0.0' OR type='external-event@1.0.0' OR active=false)
        ...
        SELECT array_agg(id) INTO link_ids FROM links2 WHERE fromid = ANY(contract_ids) OR toid = ANY(contract_ids);
        RAISE INFO 'Archiving % links...', array_length(link_ids, 1);

        -- Archive link records from links2 table
        WITH moved_rows AS (
            DELETE FROM links2 WHERE id = ANY(link_ids) RETURNING *
        ) INSERT INTO archive_links2 SELECT * FROM moved_rows;

        -- Archive link records from cards table
        WITH moved_rows AS (
            DELETE FROM cards WHERE id = ANY(link_ids) RETURNING *
