...
ava('getCard uses the API to fetch the card if not already cached', async (test) => {
	...
	test.is(ctx.sdk.query.callCount, 1)
	...
	test.is(ctx.store.dispatch.callCount, 1)
...
ava('getCard debounces calls to fetch the same card ID', async (test) => {
	...
	const cardPromise1 = getCardAction(cardId, 'user')
	const cardPromise2 = getCardAction(cardId, 'user')
