...
export const initiateThread = (ctx) => {
	return async ({
		...
		const mentions = helpers.getSlugsByPrefix('@', text, 'user-')
		const alerts = helpers.getSlugsByPrefix('!', text, 'user-')
		const groupMentions = helpers.getSlugsByPrefix('@@', text)
		const groupAlerts = helpers.getSlugsByPrefix('!!', text)
