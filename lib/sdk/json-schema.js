...
exports.merge = (schemas) => {
	try {
		// TODO: If an object matches both schema, then it should match the result
		// TODO: If an object matches only one schema, then it should not match the result
		return mergeSchema({
			type: 'object',
			allOf: schemas
		})
 ...
 * > false
 */
const disallowAdditionalProperties = (schema) => {
 ...
 * > }
 */
exports.filter = (schema, object) => {
	const ajv = new AJV({
		// https://github.com/epoberezkin/ajv#filtering-data
		removeAdditional: true
	})

	if (ajv.compile(disallowAdditionalProperties(schema))(object)) {
		return object
	}
