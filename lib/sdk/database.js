...
module.exports = class Database {
  ...
  // TODO: Make this function take action and target card IDs instead of
  // the actual cards, for security purposes
  async executeAction (action, targetCard, args) {
    const cardSchema = cardType.getSchema(CARDS.core.card)
    const actionCardSchema = cardType.getSchema(CARDS.core.action)

    jsonSchema.validate(cardSchema, action)
    jsonSchema.validate(cardSchema, targetCard)
    jsonSchema.validate(actionCardSchema, action)
    jsonSchema.validate(cardAction.getFilterSchema(action), targetCard)
    jsonSchema.validate(cardAction.getArgumentsSchema(action), args)

    const context = await this.getContext()
    const compiledArguments = cardAction.compileOptions(action, targetCard, context, args)

    // Support for actions that extend other actions
    const superActionSlug = cardAction.getSuperActionSlug(action)
    if (superActionSlug) {
      const superAction = await this.getCard(superActionSlug)
      if (!superAction) {
        throw new errors.JellyfishNoAction(`Unknown action: ${action.extends}`)
      }

      jsonSchema.validate(actionCardSchema, superAction)
      return this.executeAction(superAction, targetCard, compiledArguments)
    }

    const actionFunction = actions[action.slug]
    if (!actionFunction) {
      throw new errors.JellyfishNoAction(`Unknown action: ${action.slug}`)
    }
