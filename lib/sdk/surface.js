...
module.exports = class Surface {
   ...
   *
   * @param {String} id - card id
   * @returns {(Object|Null)} the card schema
   *
   * @example
   * const surface = new Surface(kernel)
   * await surface.initialize()
   ...
   */
	async getSchema (id) {
		...
		if (card.type === 'type') {
			return cardType.getSchema(card)
		}

		if (card.type === 'view') {
			return cardView.getSchema(card)
		}
   ...
   * await surface.initialize()
	...
	async signup (options) {
		return this.kernel.executeInternalAction('action-create-card', 'user', {
			properties: {
