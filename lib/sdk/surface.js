...
module.exports = class Surface {
  ...
  async getCard (id) {
    ...
    if (utils.isUUID(id)) {
      return _.first(await this.kernel.query({
        type: 'object',
        properties: {
          id: {
            type: 'string',
   ...
   * await surface.initialize()
   *
   * const context = await surface.getContext()
   */
  async getContext () {
  ...
  async executeAction (actionId, targetId, args) {
    ...
    return actionFunction(this, this.kernel, targetCard, context, compiledArguments)
  }
}
