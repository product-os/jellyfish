...
const actions = {
  ...
  'action-update-data-property': async (database, card, context, options) => {
    const property = _.concat([ 'data' ], _.split(options.property, '.'))

    if (_.isEqual(_.get(card, property), options.value)) {
      return card.id
    }

    _.set(card, property, options.value)
