/*
 * Copyright 2018 resin.io
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *    http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

const _ = require('lodash')
const time = require('./time')

const actions = {
  'action-create-event': async (database, card, context, options) => {
    return database.insertCard({
      type: options.type,
      links: [],
      tags: [],
      active: true,
      data: {
        timestamp: time.getCurrentTimestamp(),
        target: card.id,
        actor: context.actor.id,
        payload: options.payload
      }
    })
  },
  'action-create-card': async (database, card, context, options) => {
    const properties = {
      active: true,

      // TODO: Somehow prevent creating an event card, to force people to
      // use the create event action, in order to not attach a create event
      // on an event itself (at least for now)
      type: card.slug,

      // TODO: Check that links point to existing cards
      links: options.properties.links || [],

      tags: options.properties.tags || []
    }

    if (options.properties.slug) {
      properties.slug = options.properties.slug
    }

    if (options.properties.data) {
      properties.data = options.properties.data
    }

    const id = await database.insertCard(properties)

    await actions['action-create-event'](database, {
      id
    }, context, {
      type: 'create',
      payload: {}
    })

    return id
  },

  // TODO: Somehow prevent this action from running on an event
  'action-update-card': async (database, card, context, options) => {
    const id = await database.insertCard(_.omitBy({
      id: card.id,
      active: options.properties.active || card.active,
      slug: options.properties.slug || card.slug,
      type: card.type,
      links: options.properties.links || card.links,
      tags: options.properties.tags || card.tags,
      data: _.merge(card.data, options.properties.data)
    }, _.isNil), {
      override: true
    })

    await actions['action-create-event'](database, {
      id
    }, context, {
      type: 'update',
      payload: options.properties
    })

    return id
  },
  'action-delete-card': async (database, card, context, options) => {
    if (!card.active) {
      return card.id
    }

    const id = await database.insertCard(_.merge(card, {
      active: false
    }), {
      override: true
    })

    await actions['action-create-event'](database, {
      id
    }, context, {
      type: 'delete',
      payload: {}
    })

    return id
  },
  'action-restore-card': async (database, card, context, options) => {
    if (card.active) {
      return card.id
    }

    const id = await database.insertCard(_.merge(card, {
      active: true
    }), {
      override: true
    })

    await actions['action-create-event'](database, {
      id
    }, context, {
      type: 'restore',
      payload: {}
    })

    return id
  }
}

module.exports = actions
