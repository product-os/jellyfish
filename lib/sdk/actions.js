...
module.exports = {
  'action-create-event': async (surface, kernel, card, context, options) => {
  ...
  'action-insert-card': async (surface, kernel, card, context, options) => {
    const hasCard = options.properties.slug && Boolean(await surface.getCard(options.properties.slug))
    ...
    if (options.upsert && hasCard) {
      await surface.executeAction('action-create-event', id, {
        type: 'update',
  ...
  'action-update-property': async (surface, kernel, card, context, options) => {
    ...
    await surface.executeAction('action-create-event', card.id, {
