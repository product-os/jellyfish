...
module.exports = {
  ...
  'action-update-card': async (kernel, card, context, options) => {
    ...
    return insertCard(kernel, await kernel.getCard(updatedCard.type), context, {
      properties: _.omit(updatedCard, [ 'id', 'type' ]),
      upsert: true
    })
  },
  ...
  'action-update-property': async (kernel, card, context, options) => {
    const properties = {}
    _.set(properties, options.property, options.value)

    return kernel.executeInternalAction('action-update-card', card.id, {
      properties
    })
  }
}
