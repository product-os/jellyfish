...
exports.JellyfishStream = stream.JellyfishStream
...
class JellyfishSDK {
	constructor (API_URL, API_PREFIX, authToken) {
		...
		this.utils = utils
		this.cancelTokenSource = axios.default.CancelToken.source()
	...
	getById (type, id) {
		...
		return Bluebird.try(() => {
			return axios.default.get(`${this.API_BASE}id/${type}/${id}`, options)
		}).then((response) => {
	...
	getBySlug (type, slug) {
		...
		return Bluebird.try(() => {
			return axios.default.get(`${this.API_BASE}slug/${type}/${slug}`, options)
		}).then((response) => {
			return response.data
		}).catch((error) => {
			if (error.response && error.response.status === 404) {
