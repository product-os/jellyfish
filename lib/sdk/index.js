...
const getAxiosAdapter = require('./axios-adapter')
const {
	constraints
...
class JellyfishSDK {
	constructor (API_URL, API_PREFIX, authToken) {
		...
		this.axios = axios.create({
			adapter: getAxiosAdapter(API_URL)
		})
		...
		this.getConfig = () => {
			return Bluebird.try(() => {
				return this.axios.get(`${this.API_BASE}config`)
			})
		...
		this.getFile = (cardId, name) => {
			return Bluebird.try(() => {
				return this.axios.get(`${this.API_BASE}file/${cardId}/${name}`, {
					headers: {
	...
	get (endpoint, options) {
		...
		return Bluebird.try(() => {
			return this.axios.get(`${this.API_BASE}${trimSlash(endpoint)}`, requestOptions)
		})
			.tap((response) => {
				if (!response) {
	...
	post (endpoint, body, options) {
		...
		const requestOptions = this.authToken
			...
			: options
		return Bluebird.try(() => {
			return this.axios.post(`${this.API_BASE}${trimSlash(endpoint)}`, body, requestOptions)
		})
			.tap((response) => {
				if (!response) {
					throw new Error('Got empty response')
	...
	getByType (type) {
		...
		return Bluebird.try(() => {
			return this.axios.get(`${this.API_BASE}type/${type}`, options)
		}).then((response) => {
	...
	getById (id) {
		...
		return Bluebird.try(() => {
			return this.axios.get(`${this.API_BASE}id/${id}`, options)
		}).then((response) => {
			return response.data
		}).catch((error) => {
	...
	getBySlug (slug) {
		...
		return Bluebird.try(() => {
			return this.axios.get(`${this.API_BASE}slug/${slug}`, options)
		}).then((response) => {
			return response.data
		}).catch((error) => {
			if (error.response && error.response.status === 404) {
