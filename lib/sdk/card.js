...
class CardSdk {
	...
	async markAsRead (userSlug, card) {
		if (card.type !== 'message' && card.type !== 'whisper') {
			throw new Error(`Only cards of type "message" or "whisper" can be marked as read: received card of type ${card.type}`)
		}
	...
	async markAsUnread (userSlug, card) {
		if (card.type !== 'message' && card.type !== 'whisper') {
			throw new Error(
				`Only cards of type "message" or "whisper" can be marked as unread: received card of type ${card.type}`
			)
		}
