...
class CardSdk {
	...
	async update (id, card) {
		...
		/*
		 * A terrible and inefficient backwards compatibility
		 * layer on top of action-upsert-card.
		 * TODO: This bit of code should be refactored to use
		 * action-update-card along with JSON Patch.
		 */
		const fullCard = await this.get(id)
		card.slug = fullCard.slug
