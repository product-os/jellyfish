...
module.exports = class Kernel {
   ...
   * await kernel.initialize()
   *
   * const id = await kernel.executeInternalAction('action-create-card', 'user', {
   *   properties: {
   *     slug: 'johndoe',
   *     data: {
   ...
   */
	async executeInternalAction (actionId, targetId, args) {
		...
		const cardSchema = this.getSchema(CARDS.core.card)
		jsonSchema.validate(cardSchema, actionCard)
		jsonSchema.validate(cardSchema, targetCard)

		jsonSchema.validate(this.getSchema(CARDS.core.action), actionCard)
   ...
   */
	async executeAction (actionId, targetId, args) {
		...
		return this.executeInternalAction(actionId, targetId, compiledArguments)
	}
}
