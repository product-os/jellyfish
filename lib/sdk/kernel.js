   ...
   */
	async initialize () {
		...
		for (const card of [
			...
			CARDS['action-upsert-card'],
			CARDS['action-update-card'],
			CARDS['action-create-event'],
			CARDS['view-active'],
			CARDS['user-guest'],
			CARDS['view-read-user-admin'],
			CARDS['view-read-user-default'],
			CARDS['view-read-user-guest'],
			CARDS['view-write-user-guest']
		]) {
		...
		const guestUser = await this.getCard(this.sessions.admin, CARDS['user-guest'].slug)
