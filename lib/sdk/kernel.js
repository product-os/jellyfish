...
module.exports = class Kernel {
   ...
   * await kernel.initialize()
	 *
	 * const schema = kernel.getSchema(await kernel.getCard('type'))
	 * console.log(schema)
	 */
   ...
   */
	async insertCard (card, options = {}) {
		...
		const schema = this.getSchema(await this.getCard(card.type))
		if (!schema) {
   ...
   */
	...
	matchesSchema (schema, object) {
		return jsonSchema.match(schema, object).valid
	}
   ...
   * await kernel.initialize()
   *
   * const id = await kernel.executeAction('action-create-card', 'user', {
   *   properties: {
   *     slug: 'johndoe',
   *     data: {
   ...
   */
	async executeAction (actionId, targetId, args) {
		...
		if (!actionCard) {
			throw new this.errors.JellyfishNoAction(`Unknown action: ${actionId}`)
		}
		...
		if (!targetCard) {
			throw new this.errors.JellyfishNoElement(`Unknown target: ${targetId}`)
		}

		const cardSchema = this.getSchema(await this.getCard('card'))

		if (!this.matchesSchema(cardSchema, actionCard)) {
			throw new this.errors.JellyfishSchemaMismatch(`Invalid action: ${actionCard.id}`)
		}

		if (!this.matchesSchema(cardSchema, targetCard)) {
			throw new this.errors.JellyfishSchemaMismatch(`Invalid target: ${targetCard.id}`)
		}

		const actionSchema = this.getSchema(await this.getCard('action'))

		if (!this.matchesSchema(actionSchema, actionCard)) {
			throw new this.errors.JellyfishSchemaMismatch(`Invalid action: ${actionCard.id}`)
		}

		if (!this.matchesSchema(_.get(actionCard, [ 'data', 'filter' ], {
			type: 'object'
		}), targetCard)) {
			throw new this.errors.JellyfishSchemaMismatch('Target does not match filter')
		}

		if (!this.matchesSchema(cardAction.getArgumentsSchema(actionCard), args)) {
			throw new this.errors.JellyfishSchemaMismatch('Arguments do not match')
		}
		...
		if (!actionFunction) {
			throw new this.errors.JellyfishNoAction(`Unknown action function: ${actionCard.slug}`)
		}

		debug(`Executing internal action ${actionCard.slug}`)
		const currentDate = new Date()
		return actionFunction(this, targetCard, {
			// Always use the admin user for now
			actor: await this.getCard('user-admin'),
