...
const getFilteredQuery = async (kernel, session, schema) => {
	const roles = await kernel.getSessionRoles(session)
	const filters = await kernel.getQueryFilters(roles)
	filters.push(schema)
	return jsonSchema.merge(filters)
}

const getKeyCard = (slug, actor) => {
	return {
		slug: `session-${slug}`,
		type: 'session',
		links: [],
...
module.exports = class Kernel {
   ...
   */
	async initialize () {
		...
		await this.unsafeUpsertCard(CARDS.core.type)
		await this.unsafeUpsertCard(CARDS.core.session)
		await this.unsafeUpsertCard(CARDS.core.user)
		const adminUserId = await this.unsafeUpsertCard(CARDS.core['user-admin'])

		this.sessions = {
			admin: await this.unsafeUpsertCard(getKeyCard('admin-kernel', adminUserId))
		}
		...
		for (const card of [
			...
			CARDS.core['user-guest'],
			CARDS.core['view-read-user-admin'],
			CARDS.core['view-read-user-guest']
		]) {
		...
		for (const card of [
			...
			CARDS.essential['action-create-user'],
			CARDS.essential['action-create-session'],
		...
		const guestUser = await this.getCard(this.sessions.admin, CARDS.core['user-guest'].slug)
		this.sessions.guest = await this.insertCard(this.sessions.admin, getKeyCard('guest', guestUser.id), {
			override: true
		})
	}
   ...
   * await kernel.initialize()
   *
   * const card = await kernel.getCard('4a962ad9-20b5-4dd8-a707-bf819593cc84', 'foobar')
   *
   * if (card) {
   *   console.log(card)
   * }
   */
	async getCard (session, id) {
   ...
   * await kernel.initialize()
	 *
	 * const card = await kernel.getCard('4a962ad9-20b5-4dd8-a707-bf819593cc84', 'type')
	 * const schema = kernel.getSchema('4a962ad9-20b5-4dd8-a707-bf819593cc84', card)
	 * console.log(schema)
	 */
   ...
   */
	async unsafeUpsertCard (card) {
		jsonSchema.validate(CARDS.core.card.data.schema, card)
		jsonSchema.validate(CARDS.core[card.type].data.schema, card)
		return this.backend.upsertElement(this.getBucketForType(card.type), card)
	}

	/**
	 * @summary Get the list of roles that apply to a session
	 * @function
	 * @public
	 *
	 ...
	 * await kernel.initialize()
	 *
	 * const roles = await kernel.getSessionRoles('4a962ad9-20b5-4dd8-a707-bf819593cc84')
	 *
	 * for (const role of roles) {
	 *   console.log(role)
	 * }
	 */
	async getSessionRoles (session) {
		...
		return _.concat([ actor.slug ], actor.data.roles)
	}

	/**
	 * @summary Get the schema filters that apply to a set of roles
	 * @function
	 * @private
	 *
	 ...
	 * await kernel.initialize()
	 *
	 * const filters = await kernel.getQueryFilters([ 'resineer', 'user' ])
	 *
	 * for (const filter of filters) {
	 *   console.log(filter)
	 * }
	 */
	async getQueryFilters (roles) {
		return Bluebird.reduce(roles, (accumulator, role) => {
			return this.backend.getElementBySlug(this.getBucketForType('view'), `view-read-${role}`)
				.then(this.getSchema)
				.then((schema) => {
					if (schema) {
   ...
   */
	async insertCard (session, card, options = {}) {
		...
		jsonSchema.validate(this.getSchema(CARDS.core.card), card)

		const schema = this.getSchema(await this.getCard(session, card.type))
		if (!schema) {
   ...
   */
	getBucketForType (type) {
		...
		if (type === CARDS.core.session.slug) {
