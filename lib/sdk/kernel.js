...
module.exports = class Kernel {
   ...
   */
	async getCardById (session, id, options) {
		if (!this.backend.cache.hasKey(id)) {
			return null
		}
   ...
   */
	async getCardBySlug (session, slug, options) {
		if (!this.backend.cache.hasKey(slug)) {
			return null
		}
		...
		return Bluebird.reduce(roles, (accumulator, role) => {
			return this.backend.getElementBySlug(this.getBucketForType(CARD_TYPE), role)
				.then((card) => {
					...
					return this.getViewSchema(card)
				})
				.then((schema) => {
					if (schema) {
   ...
   */
	async insertCard (session, card, options = {}) {
		...
		jsonSchema.validate(this.getTypeSchema(CARDS.card), card)

		const schema = this.getTypeSchema(await this.getCardBySlug(session, card.type, {
			writeMode: true
		}))
   ...
   * await kernel.initialize()
	 *
	 * const card = await kernel.getCardBySlug('4a962ad9-20b5-4dd8-a707-bf819593cc84', 'user')
	 * const schema = kernel.getTypeSchema(card)
	 * console.log(schema)
	 */
   ...
   * await kernel.initialize()
	 ...
	 * const schema = kernel.getViewSchema(card)
	 * console.log(schema)
	 */
	// eslint-disable-next-line class-methods-use-this
	getViewSchema (card) {
