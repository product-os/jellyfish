...
module.exports = class Kernel {
   ...
   */
	 ...
	 *
	 * for (const role of roles.read) {
	 *   console.log(role)
	 * }
	 *
	 * for (const role of roles.write) {
	 *   console.log(role)
	 * }
	 */
	async getSessionRoles (session) {
		...
		const roles = _.concat([ actor.slug ], actor.data.roles)

		return {
			read: _.map(roles, (role) => {
				return `view-read-${role}`
			}),
			write: _.map(roles, (role) => {
				return `view-write-${role}`
			})
		}
	}
