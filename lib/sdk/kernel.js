...
module.exports = class Kernel {
   ...
   */
	async initialize () {
		...
		this.sessions = {
			admin: {
				uuid: await this.unsafeUpsertCard({
		...
		for (const card of [
			...
			await this.insertCard(this.sessions.admin.uuid, card, {
				override: true
			})
		}
		...
		const guestUserId = await this.insertCard(this.sessions.admin.uuid, CARDS.core['user-guest'], {
			override: true
		})

		this.sessions.guest = {
			uuid: await this.insertCard(this.sessions.admin.uuid, {
				slug: 'session-guest',
				type: 'session',
				links: [],
				tags: [],
