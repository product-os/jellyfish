...
module.exports = class Kernel {
  ...
  async insertCard (card, options = {}) {
    ...
    const bucket = card.type === ACTION_REQUEST_TYPE
  ...
  async query (schema, options = {}) {
    ...
    if (type) {
      if (type === ACTION_REQUEST_TYPE) {
        return this.backend.query(this.buckets.requests, mergedSchema)
      }

      return this.backend.query(this.buckets.cards, mergedSchema)
    }

    return _.concat(
      await this.backend.query(this.buckets.cards, mergedSchema),
      await this.backend.query(this.buckets.requests, mergedSchema)
    )
  }
