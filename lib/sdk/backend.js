...
module.exports = class Backend {
   ...
   * await backend.connect()
   *
   * const results = await backend.querySchema('foo', {
   *   type: 'object',
   *   properties: {
   *     type: {
   *       type: 'string',
   ...
   * }
   */
  async querySchema (table, schema) {
    const result = []
    ...
    if (_.isEqual(properties, [ 'id' ]) && schema.properties.id.const) {
      return [ await this.getElementById(table, schema.properties.id.const) ]
    }

    if (_.isEqual(properties, [ 'slug' ]) && schema.properties.slug.const) {
      return [ await this.getElementBySlug(table, schema.properties.slug.const) ]
    }
    ...
    return new Bluebird((resolve, reject) => {
      cursor.each((error, element) => {
        if (error) {
          reject(error)
        } else if (jsonSchema.isValid(schema, element)) {
          result.push(element)
        }
      }, () => {
