...
module.exports = class Backend {
   ...
   */
	async getElementById (table, id) {
		...
		const cachedElement = this.cache.get(id)
		if (cachedElement.hit && cachedElement.data.table === table) {
			return cachedElement.data.element
		}
   ...
   */
	async getElementBySlug (table, slug) {
		...
		const cachedElement = this.cache.get(slug)
		if (cachedElement.hit && cachedElement.data.table === table) {
			return cachedElement.data.element
		}
