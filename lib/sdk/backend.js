...
module.exports = class Backend {
   ...
   */
	async query (table, schema) {
		...
		await cursor.eachAsync(async (element) => {
			const expandedElement = await this.expandElement(table, schema, element)
			if (jsonSchema.filter(schema, expandedElement)) {
				results.push(expandedElement)
			}
		})
	...
	/**
	 * @summary Expand query results
	 * @description
	 * Detect expand requests by comparing element's `type` schema against the
	 * request schema.
	 * NOTE:
	 * - Only request schemas that include the cards type can be expanded
	 * - You cannot expand on expanded cards
	 *
	 * @function
	 * @public
	 *
	 ...
	 * await backend.connect()
	 *
	 * const element = await backend.expandElement('foo', {
	 *   type: 'object',
	 *   properties: {
	 *     data: {
	 *       type: 'object',
	 ...
	 *     slug: 'hello',
	 *     data: {
	 *       actor: 'b4d7c1d5-3694-479f-8bd0-85fcd77244d6'
	 *     }
	 *   })
	 */
	async expandElement (table, schema, element) {
		const result = _.cloneDeep(element)
		const type = await this.getElementBySlug(table, result.type)

		// This is a slightly brittle assumption that `anyOf` represents the user's
		// permissions
		const permissionsSchema = schema.anyOf ? { anyOf: schema.anyOf } : null

		if (!type) {
			return result
		}

		// If there are uuid fields in the type card schema, iterate over each path
		// to that uuid field. Compare this path against the query schema, and
		// if the query schema specifies that the field is an ojbect instead of
		// a uuid, expand the uuid, replacing the string value with the card itself
		const paths = utils.findUuidPaths(type.data.schema)

		if (paths.length) {
			paths.forEach(async (path) => {
				const field = _.get(schema.properties, path.join('.properties.'))
				if (_.get(field, [ 'type' ]) === 'object') {
					const uuid = _.get(result, path)
					const card = _.cloneDeep(await this.getElementById(table, uuid))

					// If there is no permissions schema or if the element passes
					// a filter, set it in place of the uuid
					if (!permissionsSchema || jsonSchema.filter(permissionsSchema, card)) {
						_.set(result, path, card)
					}
				}
			})
		}
