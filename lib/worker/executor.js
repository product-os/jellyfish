...
exports.insertCard = async (context, jellyfish, session, typeCard, options, object) => {
	...
	options.eventPayload = _.omit(evaluationResult, [ 'id' ])
	...
	return commit(context, jellyfish, session, typeCard, card, options, async () => {
		if (options.replace) {
			return jellyfish.replaceCard(context, session, evaluationResult)
		}

		if (options.override) {
			if (!card) {
				return jellyfish.insertCard(context, session, evaluationResult)
			}

			// TODO: Expose a JSON patch interface from the worker
			// so we don't have to do this diff comparison thing.
			const patch = jsonpatch.compare(
				jellyfish.defaults(card),
				jellyfish.defaults(Object.assign({}, evaluationResult, {
					name: typeof evaluationResult.name === 'string'
						? evaluationResult.name
						: null
				})))

			return jellyfish.patchCardBySlug(
				context, session, evaluationResult.slug, patch, {
