...
const getInputCard = async (jellyfish, session, identifier, options) => {
	...
	if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/.test(identifier)) {
		return jellyfish.getCardBySlug(options.ctx, session, identifier, {
			type: options.type
		})
	}

	const card = await jellyfish.getCardById(options.ctx, session, identifier, {
		type: options.type
	})
...
exports.insertCard = async (jellyfish, session, typeCard, options, object, executionContext) => {
	...
	logger.debug(executionContext, 'Inserting card', {
		slug: object.slug,
		type: typeCard.slug,
	...
	let card = null
	if (object.slug) {
		card = await jellyfish.getCardBySlug(executionContext, session, object.slug, getOptions)
	}
	if (!card && object.id) {
		card = await jellyfish.getCardById(executionContext, session, object.id, getOptions)
	}

	const hasCard = options.attachEvents && card
	const evaluationResult = jellyscript.evaluateObject(typeCard.data.schema, object)

	const insertedCard = await jellyfish.insertCard(executionContext, session, evaluationResult, {
		override: options.override,
		ctx: executionContext
	})

	if (_.isEqual(insertedCard, card)) {
		logger.debug(executionContext, 'Omitting pointless insertion', {
			slug: object.slug
		})
	...
	if (options.attachEvents) {
		...
		if (options.timestamp) {
			action.currentDate = new Date(options.timestamp)
		}

		await options.executeAction(session, action)
	}
	...
	if (
		...
		const attachedElements = insertedCard.links['has attached element'] || []
		const querySchema = {
			type: 'object',
			properties: {
				slug: {
					type: 'string',
					enum: _.map(attachedElements, 'slug')
				}
			},
			additionalProperties: true
		}

		const timelineTypes = _.uniq(
			_.map(attachedElements, 'type')
		)

		// If every materialized timeline card has a a type, add it to the query
		// (This is due to older link cards not materializing the type)
		if (timelineTypes.length === attachedElements.length) {
			querySchema.properties.type = {
				type: 'string',
				enum: timelineTypes
			}
		}
		const timeline = await jellyfish.query(executionContext, session, querySchema)

		for (const event of timeline) {
			if (!_.isEqual(event.markers, insertedCard.markers)) {
				event.markers = insertedCard.markers
				await jellyfish.insertCard(executionContext, session, event, {
					override: true,
					ctx: executionContext
				})
			}
		...
		await Bluebird.map(options.triggers, async (trigger) => {
			...
			if (insertedCard.data.target &&
				...
				matchCard.data.target = await jellyfish.getCardById(
					executionContext, session, insertedCard.data.target, {
						type: targetType
					})
			}
	...
	if (insertedCard.type === CARD_TYPE_TYPE) {
		...
		await Bluebird.map(typeTriggers, async (trigger) => {
			...
			await jellyfish.insertCard(executionContext, session, trigger, {
				override: true
			})
		...
		await Bluebird.map(jellyscript.getTypeTriggers(insertedCard), async (trigger) => {
			...
			const insertedTrigger = await jellyfish.insertCard(
				executionContext, session, trigger, {
					override: true
				})
...
exports.run = async (jellyfish, session, context, library, request) => {
	...
	const cards = await Bluebird.props({
		...
		actor: jellyfish.getCardById(execCtx, session, request.actor, {
			type: 'user'
		})
	})
