...
const insert = async (context, jellyfish, session, object, options) => {
	if (options.replace) {
		return jellyfish.replaceCard(context, session, object)
	}

	if (options.override) {
		const card = await jellyfish.getCardBySlug(context, session, object.slug, {
			type: object.type
		})
		...
		const patch = jsonpatch.compare(
			...
			jellyfish.defaults(Object.assign({}, object, {
...
exports.insertCard = async (context, jellyfish, session, typeCard, options, object) => {
	...
	const insertedCard = await insert(
		context, jellyfish, session, evaluationResult, {
			override: options.override,
			replace: options.replace
		})
