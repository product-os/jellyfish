...
const getInputCard = async (context, jellyfish, session, identifier) => {
	if (!uuid.isUUID(identifier)) {
		return jellyfish.getCardBySlug(context, session, `${identifier}@latest`)
	}
...
exports.insertCard = async (context, jellyfish, session, typeCard, options, object) => {
	...
	if (object.slug) {
		card = await jellyfish.getCardBySlug(
			context, session, `${object.slug}@${object.version}`, getOptions)
	}
	if (!card && object.id) {
		card = await jellyfish.getCardById(
			context, session, object.id, getOptions)
	}
