 ...
 * }
 */
const getInputCard = async (context, jellyfish, session, identifier, options) => {
	if (!options.type) {
...
exports.insertCard = async (context, jellyfish, session, typeCard, options, object) => {
	...
	if (
		...
		const timeline = _.compact(await Bluebird.map(attachedElements, (item) => {
			return jellyfish.getCardById(context, session, item.id, {
				type: item.type
			})
		}))
		...
		await Bluebird.map(options.triggers, async (trigger) => {
			...
			if (insertedCard.data.target &&
				...
				matchCard.data.target = await jellyfish.getCardById(
					context, session, insertedCard.data.target, {
						type: targetType
					})
			}
...
exports.run = async (jellyfish, session, context, library, request) => {
	...
	if (!skhema.isValid(argumentsSchema, request.arguments)) {
		throw new errors.WorkerSchemaMismatch('Arguments do not match', request.context)
	}
