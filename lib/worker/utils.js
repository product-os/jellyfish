...
exports.hasCard = async (jellyfish, session, object, options = {}) => {
	if (object.id && await jellyfish.getCardById(options.ctx, session, object.id, options)) {
		return true
	}

	if (object.slug && await jellyfish.getCardBySlug(options.ctx, session, object.slug, options)) {
		return true
	}
