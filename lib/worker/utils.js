 ...
 * }
 */
exports.hasCard = async (jellyfish, session, object, options) => {
	if (object.id && await jellyfish.getCardById(session, object.id, options)) {
		return true
	}

	if (object.slug && await jellyfish.getCardBySlug(session, object.slug, options)) {
		return true
	}
 ...
 *   target: '033d9184-70b2-4ec9-bc39-9a249b186422',
 *   actor: '57692206-8da2-46e1-91c9-159b2c6928ef'
 * })
 */
exports.getEventSlug = (type, data) => {
	const date = new Date(data.timestamp)
	return `${type}-${data.actor}-${date.valueOf()}`
}
