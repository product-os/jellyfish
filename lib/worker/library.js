...
module.exports = {
	...
	'action-create-session': async (session, context, card, request) => {
		...
		if (user.data.password && request.arguments.password.hash !== user.data.password.hash) {
			throw new errors.WorkerAuthenticationError('Invalid password')
		}
		...
		if (!sessionTypeCard) {
			throw new errors.WorkerNoElement('No such type: session')
		}
	...
	'action-create-user': async (session, context, card, request) => {
		return context.insertCard(session, card, {
			...
			slug: request.arguments.properties.username,
			data: {
				email: request.arguments.properties.email,
				roles: request.arguments.properties.roles,
				password: {
					hash: request.arguments.properties.hash
				}
			}
		})
	},
	...
	'action-update-card': async (session, context, card, request) => {
		...
		if (!typeCard) {
			throw new errors.WorkerNoElement(`No such type: ${updatedCard.type}`)
		}
