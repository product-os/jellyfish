...
module.exports = class Worker {
	...
	setTriggers (context, objects) {
		...
		this.triggers = objects.map((trigger) => {
			...
			if (trigger.mode && !_.isString(trigger.mode)) {
				throw new errors.WorkerInvalidTrigger(`Invalid mode: ${trigger.mode}`, context)
			}
	...
	async pre (session, request) {
		if (!this.library[request.action]) {
			const error = new errors.WorkerInvalidAction(
