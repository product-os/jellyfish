...
module.exports = class Worker {
	...
	constructor (jellyfish, session, actionLibrary) {
		...
		const execContext = logger.context.newWorkerContext()
		...
		this.context = {
			...
			getCardById: _.bind(function (lsession, id, options = {}) {
				...
				return this.jellyfish.getCardById(lsession, id, options)
			}, this),
			getCardBySlug: _.bind(function (lsession, slug, options = {}) {
				options.ctx = execContext
				return this.jellyfish.getCardBySlug(lsession, slug, options)
			}, this),
			query: _.bind(function (lsession, schema, options = {}) {
				options.ctx = execContext
				return this.jellyfish.query(lsession, schema, options)
			}, this),
		...
		if (!_.isNil(execCtx)) {
			return {
				...
				getCardById: _.bind(function (session, id, options = {}) {
					options.ctx = execCtx
					return this.jellyfish.getCardById(session, id, options)
				}, this),
				getCardBySlug: _.bind(function (session, slug, options = {}) {
					options.ctx = execCtx
					return this.jellyfish.getCardBySlug(session, slug, options)
				}, this),
				query: _.bind(function (session, schema, options = {}) {
					options.ctx = execCtx
					return this.jellyfish.query(session, schema, options)
				}, this),
	...
	insertCard (insertSession, typeCard, options, card, context) {
		const execCtx = logger.context.mergeContext(this.getExecutionContext().execContext, context)
	 ...
	 */
	setTriggers (objects) {
		logger.info(logger.context.systemContext, 'Setting triggers', {
	...
	async dequeue () {
		...
		logger.info(logger.context.systemContext, 'Dequeueing request', {
			request,
	...
	async enqueue (session, options) {
		logger.info(logger.context.systemContext, 'Enqueueing request', {
			request: options,
			length: await this.length()
		})

		const execCtx = logger.context.mergeContext(this.getExecutionContext().execContext, options.ctx)
		...
		const cards = await Bluebird.props({
			target: this.jellyfish.getCardBySlug(session, options.card, {
				type: options.type,
				ctx: execCtx
			}),
	...
	async execute (session, request) {
		logger.info(logger.context.systemContext, 'Executing request', {
			request,
			length: await this.length()
		})
		...
		const result = await executor.run(this.jellyfish, session,
			...
			logger.info(logger.context.systemContext, 'Execute success', {
				data
			})
	...
	async tick (session, options) {
		...
		logger.info(logger.context.systemContext, 'Processing tick request', {
