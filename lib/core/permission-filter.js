 ...
 */
const getUserMarkers = async (context, backend, user) => {
	...
	// TODO: Remove this once we drop RethinkDB support
	if (environment.database.type === 'rethinkdb' && user.links['is member of']) {
		for (const org of user.links['is member of']) {
			markers.push(org.slug)
		}
	}
...
exports.getQuery = async (context, backend, session, schema, options) => {
	...
	const userMarkers = await getUserMarkers(context, backend, user)
