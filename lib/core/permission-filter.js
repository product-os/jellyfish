 ...
 *
 * @example
 * const card = await permissionFilter.unsafeUpsertCard(backend, 'cards', {
 *   type: 'foo',
 *   links: [],
 *   tags: [],
 *   active: true,
...
exports.unsafeUpsertCard = (backend, table, card) => {
	skhema.validate(CARDS.card.data.schema, card)
	skhema.validate(CARDS[card.type].data.schema, card)
 ...
 * }
 */
exports.getViews = async (backend, table, roles) => {
...
exports.getQuery = async (backend, session, schema, options) => {
	...
	return skhema.merge(_.map(filters, (filter) => {
