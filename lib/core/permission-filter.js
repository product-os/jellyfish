...
exports.getSessionUser = async (backend, session, ctx) => {
	const sessionCard = await backend.getElementById(ctx, session, {
		type: 'session'
	})
	if (!sessionCard) {
	...
	const actor = await backend.getElementById(ctx, sessionCard.data.actor, {
		type: 'user'
	})
	if (!actor) {
...
exports.getViews = async (backend, roles, ctx) => {
	...
	const views = await Promise.all(roles.map((role) => {
		return backend.getElementBySlug(ctx, role, {
			type: CARD_TYPE
		})
	}))
