 ...
 * @param {Object} ctx - exectuion context
 * @returns {Object} card
 *
 * @example
 ...
 */
exports.createSession = (backend, actorId, title, ctx) => {
 ...
 */
exports.getSessionUser = async (backend, session, ctx) => {
 ...
 * }
 */
exports.getViews = async (backend, roles, ctx) => {
 ...
 * const markers = await getUserMarkers(user)
 *
 * for (const marker of markers) {
 *   console.log(marker)
 * }
 */
const getUserMarkers = async (backend, user) => {
	const markers = [ user.slug ]

	const userOrgs = _.get(user, [ 'links', 'is member of' ], [])

	await Bluebird.each(userOrgs, async (org) => {
		const orgCard = await backend.getElementById(org.id, {
			type: 'org'
		})

		// If the org does not exist or is unavailable, just ignore it
		if (!orgCard) {
			return
		}

		if (!orgCard.slug) {
			throw new Error(`Org ${org.id} does not have a slug`)
		}

		markers.push(orgCard.slug)
	})
