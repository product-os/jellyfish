...
module.exports = class Backend {
   ...
   * await backend.connect()
   *
   * const object = await backend.insertElement('foo', {
   ...
   * await backend.connect()
   *
   * const object = await backend.upsertElement('foo', {
   *   id: '4a962ad9-20b5-4dd8-a707-bf819593cc84',
   *   data: 'foo'
   * })
   */
   ...
   */
	async query (table, schema, options = {}) {
		...
		return jsonSchema.filter(schema, items)
	}
   ...
   */
	async stream (table, schema) {
		...
		cursor.each((error, change) => {
			...
			const newMatch = jsonSchema.filter(schema, change.new_val)
			const oldMatch = jsonSchema.filter(schema, change.old_val)
