...
module.exports = class Backend {
   ...
   */
	async createTable (name) {
		...
		if (await this.hasTable(name)) {
			const information = await rethinkdb
				.db(this.database)
				.table(name)
				.info()
				.run(this.connection)

			if (information.primary_key === 'slug') {
				return
			}

			const tmpName = `tmp${name}`
			console.log(`>>>>>> Creating temporary table ${tmpName}`)
			await rethinkdb
				.db(this.database)
				.tableCreate(tmpName, {
					primaryKey: 'slug'
			...
			await rethinkdb.db(this.database).wait().run(this.connection)
			console.log(`>>>>>> Waiting for table ${tmpName}`)
			await rethinkdb
				.db(this.database)
				.table(tmpName)
				.wait()
			...
			console.log(`>>>>>> Getting count from ${tmpName}`)
			...
			for (let index = 0; index < count; index += 300) {
				...
				console.log(`>>>>>> Inserting ${array.length} elements to ${name} (skip ${index}/${count})`)
				await rethinkdb
					.db(this.database)
					.table(tmpName)
					.insert(array)
			...
			console.log(`>>>>>> Creating table ${name} with slug primary key`)
			await rethinkdb
				.db(this.database)
				.tableCreate(name, {
					primaryKey: 'slug'
			...
			console.log(`>>>>>> Waiting for table ${name}`)
			await rethinkdb
				.db(this.database)
				.table(name)
				.wait()
			...
			console.log(`>>>>>> Dropping table ${tmpName}`)
			await rethinkdb
				.db(this.database)
				.tableDrop(tmpName)
				.run(this.connection)
