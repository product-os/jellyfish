...
const logger = require('../logger').getLogger('core:backend')
...
const generateRethinkInstance = (options, ctx) => {
	return rethinkdb(_.merge({
		...
		silent: true,
		log: (message) => {
			logger.debug(ctx, `RethinkDB Dash: ${message}`)
		}
	}, options))
}
...
const createTableStream = async (backend, table, schema, ctx) => {
	...
	logger.debug(ctx, 'Streaming from table', {
		table,
	...
	emitter.close = async () => {
		logger.debug(ctx, 'Closing stream', {
			table,
			database: backend.database
		})
		...
		return new Bluebird((resolve) => {
			cursor.close(() => {
				...
				logger.debug(ctx, 'Stream closed', {
					table,
					database: backend.database
				})
...
const queryTable = async (backend, table, schema, options) => {
	logger.debug(options.ctx, 'Querying from table', {
		table,
		database: backend.database,
		limit: options.limit,
		skip: options.skip,
	...
	logger.debug(options.ctx, 'Query database response', {
		table,
		database: backend.database,
		count: result.length
	})
...
const getElementsByIdFromTable = async (backend, table, ids, ctx) => {
	logger.debug(ctx, 'Batch get by id', {
		count: ids.length,
		table,
		database: backend.database
	})
...
const getElementByIdFromTable = async (backend, table, id, ctx) => {
	logger.debug(ctx, 'Getting element by id', {
		id,
		table,
		database: backend.database
	})
...
const getElementBySlugFromTable = async (backend, table, slug, ctx) => {
	logger.debug(ctx, 'Getting element by slug', {
		slug,
		table,
		database: backend.database
	})
...
const upsertObject = async (backend, table, object, options) => {
	...
	if (options.replace) {
		logger.debug(options.ctx, 'Upserting element', {
			table,
			slug: insertedObject.slug,
			database: backend.database
		})
	} else {
		logger.debug(options.ctx, 'Updating element', {
			table,
			slug: insertedObject.slug,
			database: backend.database
		})
...
module.exports = class Backend {
   ...
   */
	async reset (ctx) {
		...
		await Promise.all(tables.map((table) => {
			logger.debug(ctx, 'Dropping table', {
				table,
   ...
   */
	async destroy (ctx) {
		...
		logger.debug(ctx, 'Dropping database', {
			database: this.database
		})

		await rethinkInstance.dbDrop(this.database).run(connection)
		logger.debug(ctx, 'Database dropped', {
			database: this.database
		})
   ...
   */
	async connect (ctx) {
		...
		logger.debug(ctx, 'Listing databases')
		...
		if (!databases.includes(this.database)) {
			...
			logger.debug(ctx, 'Creating database', {
				database: this.database
			})
		...
		logger.debug(ctx, 'Waiting for database', {
			database: this.database
		})
		...
		for (const table of await this.getTables(ctx)) {
			logger.debug(ctx, 'Checking table for stale indexes', {
				table,
				database: this.database
			})
			...
			for (const index of indexes) {
				if (!_.includes(SECONDARY_INDEX, index)) {
					logger.debug(ctx, 'Dropping stale table index', {
						table,
						database: this.database,
						index
					})
		...
		for (const table of ALL_BUCKETS) {
			...
			logger.debug(ctx, 'Creating table indexes', {
				table,
				database: this.database
			})
			...
			for (const secondaryIndex of SECONDARY_INDEX) {
				if (!_.includes(indexes, secondaryIndex)) {
					logger.debug(ctx, 'Creating table index', {
						table,
						database: this.database,
					...
					logger.debug(ctx, 'Waiting for table indexes', {
						table,
						database: this.database
					})
	...
	async disconnect (ctx) {
		...
		if (streamConnection) {
			for (const [ id, stream ] of Object.entries(this.openStreams)) {
				logger.debug(ctx, 'Disconnecting stream', id)
				await stream.close()
   ...
   */
	getTables (ctx) {
		logger.debug(ctx, 'Listing tables', {
			database: this.database
		})
   ...
   */
	async createTable (name, ctx) {
		if (!await this.hasTable(name, ctx)) {
			logger.debug(ctx, 'Creating table', {
				table: name,
				database: this.database
			})
			...
			logger.debug(ctx, 'Waiting for table', {
				table: name,
				database: this.database
			})
