...
const queryTable = async (backend, table, schema, options) => {
	...
	// skhema.filter() doesn't seem to work very well
	// with arrays for some unknown reason.
	// We should be able simplify this a lot once we
	// get some time to dig into the root cause.
	return elements.reduce((accumulator, element) => {
		if (!element) {
			return accumulator
		}

		const filteredElement = skhema.filter(schema, element)
		if (filteredElement) {
			accumulator.push(filteredElement)
		}
...
const upsertObject = async (backend, table, object, current, options) => {
	...
	if (result.type === 'link') {
		const fn = result.active ? links.addLink : links.removeLink
		await Bluebird.map([
			result.data.from,
			result.data.to
		], async (id) => {
			...
			if (card) {
				await backend.upsertElement(fn(result, card))
			}
		})
	}
