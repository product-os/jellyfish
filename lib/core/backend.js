...
const upsertObject = async (backend, table, object, current, options) => {
	...
	if (insertedObject.links) {
		insertedObject.links = (current && current.links) || {}
	}

	if (current && current.id) {
		insertedObject.id = current.id

		// TODO: Remove this check once slugs become primary keys
		if (current.slug && current.slug !== insertedObject.slug) {
			throw new errors.JellyfishDatabaseError(`Cannot update slug: ${current.slug}`)
		}
	} else {
		insertedObject.id = uuid()
	}
	...
	if (insertedObject.type === 'link') {
		...
		await Bluebird.map([
			...
			options.unsetFromCache = true
			if (card) {
				await upsertObject(
					backend,
					getBucketForType(card.type),
					fn(insertedObject, card),
					card,
					options)
			}
		})
...
module.exports = class Backend {
   ...
   */
	async insertElement (object) {
		...
		if (!object.slug) {
			throw new errors.JellyfishNoSlug(`Missing slug in object with id ${object.id}`)
		}

		if (await this.getElementBySlug(object.slug)) {
