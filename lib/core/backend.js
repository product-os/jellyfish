...
const memoizedFilter = memoize((schema, element) => {
	return skhema.filter(schema, element)
})

// Memoization + cloning is faster than compiling JSON schemas
const filter = (schema, element) => {
	// Clonedeep is used to stop the memoize cache from being mutated
	// (not fun to debug!)
	const result = _.cloneDeep(memoizedFilter(schema, element))
