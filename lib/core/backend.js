   ...
   */
	async query (table, schema) {
		...
		if (_.isEqual(properties, [ 'id' ]) && schema.properties.id.const) {
			return jsonSchema.filter(schema, _.cloneDeep(await this.getElementById(table, schema.properties.id.const)))
		}

		if (_.isEqual(properties, [ 'slug' ]) && schema.properties.slug.const) {
			return jsonSchema.filter(schema, _.cloneDeep(await this.getElementBySlug(table, schema.properties.slug.const)))
		}
		...
		if (_.isEqual(properties, [ 'active', 'id' ]) && schema.properties.id.const && schema.properties.active.const) {
			...
			return jsonSchema.filter(schema, _.cloneDeep(element))
		}
