   ...
   */
	async query (table, schema, options = {}) {
		...
		if (_.isEqual(properties, [ 'id' ]) && schema.properties.id.const) {
			return _.compact([
				jsonSchema.filter(schema, _.cloneDeep(await this.getElementById(table, schema.properties.id.const)))
			])
		}

		if (_.isEqual(properties, [ 'slug' ]) && schema.properties.slug.const) {
			return _.compact([
				jsonSchema.filter(schema, _.cloneDeep(await this.getElementBySlug(table, schema.properties.slug.const)))
			])
		}
		...
		const filter = utils.jsonSchemaToReqlFilter(schema)

		debug(`Querying entries in table ${table}`)

		let query = await rethinkdb
			.db(this.database)
			.table(table)
			.filter(filter)
		...
		const results = jsonSchema.filter(schema, items)
