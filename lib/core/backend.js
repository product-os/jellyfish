...
const generateRethinkInstance = (context, options) => {
	return rethinkdb(_.merge({
		...
		silent: true,
		log: (message) => {
			logger.debug(context, `RethinkDB Dash: ${message}`)
		}
	}, options))
}
...
const queryTable = async (context, backend, table, schema, options) => {
	...
	const reqlSchemaOptions = {
		indexes: getSecondaryIndexesForQuery()
	}

	if (table === BUCKETS.requests) {
		reqlSchemaOptions.indexes = null
		reqlSchemaOptions.orderBy = {
			index: backend.connection.desc(TIMESTAMP_INDEX)
		}

		// Don't allow any other sorting/ordering
		options.sortBy = null
		options.sortDir = null
	}

	let query = backend.reqlSchema(backend.database, table, schema, reqlSchemaOptions)
	...
	} else if (table !== BUCKETS.requests) {
   ...
   */
	async connect (context) {
		...
		for (const table of ALL_BUCKETS) {
			...
			// To speed up action request result traversals
			if (table === BUCKETS.requests && !_.includes(indexes, TIMESTAMP_INDEX)) {
				await this.connection
					.db(this.database)
					.table(table)
					.indexCreate(TIMESTAMP_INDEX, this.connection.row('data')('payload')('timestamp'))
					.run()
			}
