 ...
 * @param {String[]} inputProperty - the formula input card property path
 * @returns {Any} expression result
 *
 * @example
 * const result = formulas.evaluate('UPPER(input)', {
 *   data: {
 *     text: 'foo'
 *   }
 * }, [ 'data', 'text' ])
 ...
 * > 'FOO'
 */
exports.evaluate = (expression, card, inputProperty) => {
	const input = _.get(card, inputProperty)

	if (_.isEmpty(card) || _.isNil(input)) {
		return null
	}
