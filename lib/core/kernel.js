...
const preUpsert = async (instance, context, session, card) => {
	...
	const [
		...
		// TODO: Update all cards in the system to point
		// to types using version suffixes
		instance.getCardBySlug(
			context, session, `${card.type}@1.0.0`),
   ...
   */
	async getCardBySlug (context, session, slug) {
		...
		assert.INTERNAL(context, version,
			errors.JellyfishInvalidVersion, 'Version is undefined')
   ...
   */
	async patchCardBySlug (context, session, slug, patch, options) {
		...
		const [
			...
			// to types using version suffixes
			this.getCardBySlug(
				context, session, `${fullCard.type}@1.0.0`),
