...
module.exports = class Kernel {
   ...
   */
	async insertCard (session, card, options = {}) {
		...
		const processedCard = this.evaluateFormulas(schema, card)
		jsonSchema.validate(schema, processedCard)

		const bucket = this.getBucketForType(processedCard.type)

		if (options.override) {
			return this.backend.upsertElement(bucket, processedCard)
		}

		return this.backend.insertElement(bucket, processedCard)
	}
   ...
   */
	...
	evaluateFormulas (schema, object) {
		for (const path of jsonSchema.getFormulasPaths(schema)) {
			...
			const result = jellyscript.evaluate(path.formula, {
				context: object,
				input: _.get(object, path.output)
			})
