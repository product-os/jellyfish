...
module.exports = class Kernel {
   ...
   */
	...
	evaluateFormulas (schema, object) {
		for (const property of jsonSchema.getFormulasFromSchema(schema)) {
			const context = _.get(object, property.path)
			const result = formulas.evaluate(property.formula, context)
			if (!_.isNull(result)) {
				// Mutates input object
				_.set(object, property.path, result)
			}
		}
