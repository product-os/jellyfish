...
const preUpsert = async (instance, context, session, card) => {
	...
	// TODO: Remove this once we completely migrate action-requests
	// to have versioned types in the "action" property.
	// We put this check here, before we type-check the
	// upsert, so we don't cause violations to the type
	// if the action has no versions.
	// See: https://github.com/product-os/jellyfish/pull/3009
	if (card.type === 'action-request@1.0.0' &&
		card.data &&
		card.data.action &&
		!_.includes(card.data.action, '@')) {
		card.data.action = `${card.data.action}@1.0.0`
	}
