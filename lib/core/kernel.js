...
module.exports = class Kernel {
   ...
   * @param {Boolean} [options.defaults=false] - set default properties of card
   ...
   */
	async insertCard (session, card, options = {}) {
		...
		const result = options.override
			? await this.backend.upsertElement(card)
			: await this.backend.insertElement(card)
		...
		if (_.isString(result.data.target)) {
			...
			await this.insertCard(session, this.defaults({
