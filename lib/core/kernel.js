...
module.exports = class Kernel {
   ...
   */
	async query (context, session, schema, options = {}) {
		...
		const queryOptions = {
			...
			// TODO: We should not need to pass a mask to the backend, which
			// only uses it in order to apply to it the custom pending requests
			// query.
   ...
   * await kernel.initialize()
	 *
	 * if (await kernel.lock('4a962ad9-20b5-4dd8-a707-bf819593cc84', {
	 *   slug: 'foobar',
	 *   ...
	 * })) {
	 ...
	 * }
	 */
	async lock (owner, card) {
		return this.backend.lock(owner, card)
	}
   ...
   * await kernel.initialize()
	 ...
	 *   console.log('Got the lock!')
	 *
	 *   if (await kernel.unlock('4a962ad9-20b5-4dd8-a707-bf819593cc84', {
	 *     slug: 'foobar',
	 *     ...
	 *   })) {
	 *     console.log('Unlock!')
	 ...
	 * }
	 */
	async unlock (owner, card) {
		return this.backend.unlock(owner, card)
	}

	async popRequest (context, request) {
		return this.backend.popRequest(context, request)
	}
}
