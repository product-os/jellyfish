...
module.exports = class Kernel {
   ...
   */
	async initialize () {
		...
		return this.pipeline.start(Object.values(this.buckets))
	}
   ...
   */
	async insertCard (session, card, options = {}) {
		...
		const result = jellyscript.evaluateObject(schema, card)
		jsonSchema.validate(schema, result.object)

		const bucket = this.getBucketForType(result.object.type)

		const id = options.override
			? await this.backend.upsertElement(bucket, result.object)
			: await this.backend.insertElement(bucket, result.object)

		// TODO: Generalize and refactor this on top of triggered actions
		for (const watcher of result.watchers) {
			this.pipeline.insertFunction(id, async (change) => {
				const expandedCard = _.cloneDeep(change.after)
				if (!change.after.data.target) {
					return Bluebird.resolve()
				}

				expandedCard.data.target = await this.getCardById(session, change.after.data.target)
				if (!jsonSchema.isValid(watcher.filter, expandedCard)) {
					return Bluebird.resolve()
				}

				const targetProperty = _.get(change.after, watcher.target)
				const target = await this.getCardById(session, targetProperty)
				if (!target) {
					return Bluebird.resolve()
				}
				...
				return this.insertCard(session, target, {
					override: true
				})
			})
		}
   ...
   */
	async stream (session, schema, options) {
		const filteredQuery = await getFilteredQuery(this, session, schema, options)
		return utils.mergeStreams(this.getBucketsForSchema(schema), (bucket) => {
			return this.backend.stream(bucket, filteredQuery)
		})
	}
