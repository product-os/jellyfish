...
module.exports = class Kernel {
   ...
   * await kernel.initialize()
   *
	 * const card = await kernel.patchCard(
	 *   '4a962ad9-20b5-4dd8-a707-bf819593cc84', { ... })
   * console.log(card.id)
   */
	async patchCard (context, session, object) {
		const card = this.defaults(object)
		const filter = await preUpsert(this, context, session, card)

		/*
		 * TODO: Replace this abomination with JSON Patch. This
		 * method should take a JSON Patch set of operations and
		 * apply those instead of doing this expensive merge logic.
		 */
