...
const logger = require('../logger').getLogger('core:kernel')
const {
	deepEqual
} = require('fast-equals')
 ...
 */
const getElementCreatedTime = async (backend, card) => {
	if (card.created_at) {
	...
	const [ createCard ] = await backend.query({
		type: 'object',
		properties: {
			type: {
				type: 'string',
...
module.exports = class Kernel {
   ...
   */
	async getCardsById (session, ids, options = {}) {
		...
		const result = await this.backend.getElementsById(ids, options)
	...
	async insertCard (session, object, options = {}) {
		...
		if (options.override) {
			...
			if (card.id || card.slug) {
				...
				if (element) {
					card.created_at = await getElementCreatedTime(this.backend, element)
   ...
   */
	async query (session, schema, options = {}) {
		...
		return this.backend.query(filteredQuery, {
			...
			subquery: async (ids, subqueryOptions) => {
				logger.debug(options.ctx, 'Executing subquery', {
					ids
				})

				subqueryOptions.ctx = options.ctx
