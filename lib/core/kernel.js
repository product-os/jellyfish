...
module.exports = class Kernel {
   ...
   */
	async getCardBySlug (context, session, slug) {
		...
		const [ base, suffix ] = slug.split('@')

		// TODO: Get rid of this default once the
		// whole system makes use of versions.
		const version = suffix || 'latest'
   ...
   */
	async query (context, session, schema, options = {}) {
		const finalSchema = schema.type === `${CARDS.view.slug}@${CARDS.view.version}` ||
				...
				// TODO: Get rid of this condition once the whole
				// system makes use of semver slug pointer.
				schema.type === CARDS.view.slug
   ...
   * await kernel.initialize()
	...
	async lock (context, session, owner, card) {
		if (!await this.getCardBySlug(
			context, session, `${card.slug}@${card.version}`)) {
			return null
		}
