...
module.exports = class Kernel {
   ...
   */
	async getCardById (session, id, options) {
		...
		const results = await this.query(session, {
			type: 'object',
			properties: {
				id: {
					type: 'string',
   ...
   */
	async query (session, schema, options = {}) {
		...
		const queries = await Promise.all(this.getBucketsForSchema(schema).map((bucket) => {
   ...
   */
	async stream (session, schema, options) {
		const filteredQuery = await getFilteredQuery(this, session, schema, options)
		const buckets = this.getBucketsForSchema(schema)
