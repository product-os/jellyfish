...
module.exports = class Kernel {
   ...
   * await kernel.initialize()
   */
	async initialize (context, holdLock = true) {
		if (holdLock) {
		...
		const unsafeUpsert = (card) => {
			...
			return permissionFilter.unsafeUpsertCard(this.backend, element, context)
		}
		...
		return Promise.all([
			...
			logger.debug(context, `Upserting core card ${card.slug}`)
   ...
   */
	async query (context, session, schema, options = {}) {
		...
		return this.backend.query(context, filteredQuery, {
			...
			subquery: async (subcontext, ids, subqueryOptions) => {
				logger.debug(context, 'Executing subquery', {
					ids
				})

				subqueryOptions.writeMode = options.writeMode
				return this.getCardsById(subcontext, session, ids, subqueryOptions)
			}
		})
	}
