...
const COMPOSITE_SLUG_VERSION_INDEX = 'slug_version'
...
exports.setup = async (context, connection, database, options = {}) => {
	...
	await connection.any(`
		...
		ALTER TABLE ${table} ADD COLUMN IF NOT EXISTS updated_at TEXT;
		ALTER TABLE ${table} ADD COLUMN IF NOT EXISTS linked_at JSONB;
		`)
	...
	if (!indexes.includes(COMPOSITE_SLUG_VERSION_INDEX)) {
		await connection.any(`
			CREATE UNIQUE INDEX ${COMPOSITE_SLUG_VERSION_INDEX} ON ${table} (slug, version)`)
	}
...
exports.getBySlug = async (context, connection, slug, options = {}) => {
	...
	const results = !version || version === 'latest'
		? await connection.any(`
			SELECT * FROM ${table} WHERE slug = $1 ORDER BY version DESC LIMIT 1;`,
		[ base ])
		: await connection.any(`
			SELECT * FROM ${table} WHERE slug = $1 AND version = $2 LIMIT 1;`,
		[ base, version ])
...
exports.materializeLink = async (context, errors, connection, card, options = {}) => {
	...
	try {
		await connection.any(`
			...
			WHERE id = '${card.id}';`, [ card.linked_at ])
	} catch (error) {
		if (/^duplicate key value/.test(error.message)) {
