...
exports.getUnexecutedRequestsCount = async (connection, options = {}) => {
	const table = options.table || exports.TABLE

	// An optimised custom query to get the count of non-executed requests
	// in an effective manner. We should ideally implement "COUNT" support
	// when querying using JSON Schema and use that instead.
	const LINK_TYPE = 'is executed by'
	const result = await connection.any(`
		SELECT COUNT (cards.id)
		FROM ${table}
		LEFT JOIN links AS link ON (
