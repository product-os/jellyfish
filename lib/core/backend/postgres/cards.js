...
exports.setup = async (context, connection, database, options = {}) => {
	...
	if (!_.includes(indexes, jsonschema2sql.createIndexName(table))) {
		await connection.any(jsonschema2sql.createSearchIndex(table))
	}
...
exports.upsert = async (context, errors, connection, object, options) => {
	...
	const elementId = options.id || await uuid()
