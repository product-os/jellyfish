...
exports.setup = async (context, connection, options = {}) => {
	 ...
	 */
	const views = _.map(await utils.query(connection, `
...
const refreshView = environment.isProduction()
	? debouncePromise((connection, view) => {
		 ...
		 */
		return utils.query(connection, `REFRESH MATERIALIZED VIEW CONCURRENTLY ${view}`)
	}, 500, {
		leading: true
	})
	...
	: async (connection, view) => {
		return utils.query(connection, `REFRESH MATERIALIZED VIEW ${view}`)
	}
...
exports.getUserMarkers = async (context, connection, user, options = {}) => {
	...
	return result[0].markers.map((marker) => {
		return {
			slug: marker,
			type: 'org'
		}
