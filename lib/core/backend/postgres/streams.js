...
class Streamer {
	...
	async attach (context, select, schema) {
		return new Stream(this, await uuid.random(), context, select, schema)
	}
...
class Stream extends EventEmitter {
	constructor (streamer, id, context, select, schema) {
		super()
	...
	async query (select, schema, options) {
		...
		// `this.seenCardIds`
		const selectsId = 'id' in this.select
		if (!selectsId) {
			this.select.id = {}
		}
	...
	async tryEmitEvent (payload) {
		...
		if (this.cardTypes && !this.cardTypes.includes(payload.type.split('@')[0])) {
			return false
		}
		...
		_.set(schema, [ 'properties', 'id', 'const' ], payload.id)
		try {
			const result = await this.streamer.backend.query(
				this.context,
				this.select,
				schema,
				{
