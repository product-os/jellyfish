...
class Streamer {
	...
	async attach (context, select, schema) {
		const stream = new Stream(this, await uuid.random(), context)
		await stream.setSchema(select, schema)
...
class Stream extends EventEmitter {
	constructor (streamer, id, context) {
		super()
	...
	async setSchema (select, schema) {
		...
		this.streamQuery = await this.streamer.backend.prepareQueryForStream(this.context, select, schema)
	}
