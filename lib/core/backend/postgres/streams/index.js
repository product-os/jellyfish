...
exports.setup = async (context, connection, table) => {
	...
	const lq = new PGLiveSelect(sco.client, `megatream-${table}`, context)

	const emitter = await mergeStreams([ table ], async (item) => {
		const handle = await lq.select(`SELECT * FROM ${item}`)
...
exports.attach = async (context, instance, schema, options) => {
	...
	emitter.close = async () => {
		...
		logger.info(context, 'Stream closed', {
			table: instance.table
		})
	}
