...
const upsertObject = async (context, backend, object, options) => {
	 ...
	 */
	if (baseType === 'action-request' &&
		insertedObject.active &&
		!options.replace) {
		await queue.add(context, backend.errors, backend.connection, insertedObject, {
			table: 'requests'
		})
	} else if (baseType === 'link' && insertedObject.active && !options.replace) {
		if (insertedObject.data.from.type.split('@')[0] === 'action-request' &&
			insertedObject.data.to.type.split('@')[0] === 'execute') {
			await queue.remove(
				context, backend.connection, insertedObject.data.from.id, {
					table: 'requests'
				})
		} else if (insertedObject.data.from.type.split('@')[0] === 'execute' &&
			insertedObject.data.to.type.split('@')[0] === 'action-request') {
			await queue.remove(
				context, backend.connection, insertedObject.data.to.id, {
					table: 'requests'
				})
		}
	} else if (baseType === 'execute') {
		await queue.remove(
...
module.exports = class PostgresBackend {
	...
	async query (context, schema, options = {}) {
		...
		if (schema.type === 'object' &&
			...
			schema.properties.type.const === 'action-request@1.0.0' &&
   ...
   * - closed: When the connection is closed after calling `.close()`
	...
	async unlock (context, owner, card) {
		...
		if (card.id &&
			...
			(card.type === 'action-request@1.0.0' || card.type === 'action-request')) {
			await queue.remove(
