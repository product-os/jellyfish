...
const SECONDARY_INDEX = [ 'slug', 'type', JSON_COLUMN ]
...
const queryTable = async (context, backend, table, schema, options) => {
	...
	const elements = await Bluebird.map(results, async (element) => {
		return resolveLinks(context, backend, schema, config, element[JSON_COLUMN])
	}, {
		concurrency: 3
	})
...
const getElementsByIdFromTable = async (context, backend, table, ids) => {
	...
	const results = await backend.connection.any(`
		SELECT ${JSON_COLUMN} FROM ${table} WHERE id IN (${quotedIds});
	`)
...
const getElementByIdFromTable = async (context, backend, table, id) => {
	...
	const results = await backend.connection.any(`
		SELECT ${JSON_COLUMN} FROM ${table} WHERE id = $1 LIMIT 1;
	`, [ id ])

	return (results[0] && results[0][JSON_COLUMN]) || null
}
...
const getElementBySlugFromTable = async (context, backend, table, slug) => {
	...
	const results = await backend.connection.any(`
		SELECT ${JSON_COLUMN} FROM ${table} WHERE slug = $1 LIMIT 1;
	`, [ slug ])
 ...
 *
 * ┌─────────┬─────────────┬────────────┐
 * │ UUID id │ STRING slug │ JSONB data │
 * └─────────┴─────────────┴────────────┘
 *
...
module.exports = class PostgresBackend {
	...
	async connect (context) {
		...
		await Bluebird.map(ALL_BUCKETS, async (table) => {
			...
			const indexes = _.map(await this.connection.any(`
				SELECT * FROM pg_indexes WHERE tablename = '${table}'`), 'indexname')
	...
	async createTable (context, name) {
		 ...
		 */
		await this.connection.any(`
			ALTER TABLE ${name} ALTER COLUMN ${JSON_COLUMN}
	...
	async lock (owner, slug) {
		...
		try {
			...
			const result = await this.connection.one(`
				INSERT INTO ${LOCK_TABLE} VALUES ($1, $2, $3, $4, $5)
				ON CONFLICT (slug)
				DO UPDATE
