...
const postProcessCard = (card, schema, errors) => {
	const postCard = {}
	if ('links' in card) {
		postCard.links = card.links
	}
	for (const [ key, value ] of Object.entries(card)) {
		if (key.startsWith('links.')) {
			if (!('links' in postCard)) {
				postCard.links = {}
			}
			const linkType = key.substring(6)
			postCard.links[linkType] = value.map((linked) => {
				return postProcessCard(linked, schema.$$links[linkType])
			})
			utils.filter(schema.$$links[linkType], postCard.links[linkType], errors)
		} else if (key !== 'links') {
			postCard[key] = value
		}
	}

	return utils.removeVersionFields(utils.convertDatesToISOString(postCard))
}
...
const queryTable = async (context, backend, table, schema, options) => {
	...
	let query = null
	try {
		query = jsonschema2sql(table, schema, config)
	} catch (error) {
