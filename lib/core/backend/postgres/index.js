...
const queryTable = async (context, backend, table, schema, options) => {
	...
	await backend.connection.txIf({
		...
		// Try out the new compiler for 1% of the queries and only in those cases
		// submit timing metrics to make sure we're comparing apples to apples
		if (testNewCompiler) {
			...
			const strippedResults = results.map((card) => {
				return getStrippedCard(card)
			})
			const newStrippedResults = newResults.map((card) => {
				return getStrippedCard(card)
			})
