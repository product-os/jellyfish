...
const userBelongsToOrgOptimizationIsApplicable = (schema) => {
...
const runQuery = async (context, schema, query, connection, errors, values) => {
	const queryStart = performance.now()
	const results = await connection.any(query, values).catch((error) => {
...
const queryTable = async (context, backend, table, select, schema, options) => {
	...
	const {
		query,
		queryGenTime
	} = compileSchema(table, select, schema, options, backend.errors)
...
module.exports = class PostgresBackend {
	...
	async prepareQueryForStream (context, select, schema) {
		if (userBelongsToOrgOptimizationIsApplicable(schema)) {
			return async (id) => {
				return markers.getUserMarkers(
					context,
   ...
   * - closed: When the connection is closed after calling `.close()`
	...
	async stream (context, select, schema) {
		...
		return this.streamClient.attach(context, select, schema)
	}
