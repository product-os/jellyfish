...
const queryTable = async (context, backend, table, schema, options) => {
	...
	const elements = results.map((element) => {
		for (const linkType of Object.keys(schema.$$links || {})) {
			...
			element.links[linkType] = filter(linkSchema, element[`links.${linkSlug}`])
...
const shouldTriggerQueueRefresh = (object, afterCard, options) => {
	/*
	 * When inserting an action-request for the first time. We
	 * don't care about updates such as updated_at, linked_at, etc.
	 */
	if (afterCard.type === 'action-request' && !options.replace) {
		return true
	}

	/*
	 * When there is a new link involving an action request
	 * ignoring any other update to link cards.
	 */
	if (afterCard.type === 'link' && !options.replace) {
		if (afterCard.data.from.type === 'action-request' ||
			afterCard.data.to.type === 'action-request') {
			return true
		}
	}
...
const upsertObject = async (context, backend, object, options) => {
	...
	/*
	 * Only update the queue view if needed, for performance reasons.
	 */
	if (shouldTriggerQueueRefresh(object, insertedObject, options)) {
		logger.info(context, 'Triggering queue refresh', {
			type: insertedObject.type,
			slug: insertedObject.slug,
			database: backend.database
		})

		await queue.refresh(context, backend.connection, {
			source: cards.TABLE
		})
	}
...
module.exports = class PostgresBackend {
	...
	async connect (context) {
		...
		await queue.setup(context, this.connection, {
			source: cards.TABLE,
			links: links.TABLE
		})
   ...
   * - closed: When the connection is closed after calling `.close()`
	 ...
	 */
	async lock (owner, slug, date = new Date()) {
		return locks.lock(this.connection, this.errors, owner, slug, date)
	}
	 ...
	 */
	async getPendingRequests (context, options = {}) {
		const results = await queue.getElements(context, this.connection, {
			source: cards.TABLE,
			limit: options.limit,
			skip: options.skip
		})

		return filter(options.mask || {
			type: 'object',
	...
	/*
	 * Get the markers that belong to a user card.
	 *
	 * Clients that require accessing the user markers are advised
	 * to use this function for performance reasons rather than doing
	 * a more generic JSON Schema query on the cards table.
	 */
	async getUserMarkers (context, user) {
		const orgs = await markers.getUserMarkers(
			context, this.connection, user, {
				source: cards.TABLE
			})

		return [ user.slug ].concat(orgs)
	}
}
