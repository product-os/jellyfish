...
const defaultAdditionalPropertiesFalse = (schema) => {
	// Default additionalProperties to false, to limit the ammount of returned
...
// Get a object with only the ID, slug, version, markers, and type of a card,
// with all links stripped as well and sorted by ID
const getStrippedCard = (card) => {
	const result = {
		id: card.id,
		slug: card.slug,
	...
	for (const [ key, value ] of Object.entries(card)) {
		if (key.startsWith('links.')) {
			const linkedCards = value.map((linked) => {
				return getStrippedCard(linked)
			})
			result[key] = _.sortBy(linkedCards, (linked) => {
				return linked.id
			})
		}
	}
...
const queryTable = async (context, backend, table, schema, options) => {
	...
	await backend.connection.txIf({
		...
		results = await tx.any(query).catch((error) => {
		...
		// Try out the new compiler for a percentage of the queries, and only
		// in those cases submit timing metrics to make sure we're comparing
		// apples to apples
		if (testNewCompiler) {
			...
			const newResults = await tx.any(newQuery).catch((error) => {
				assert.USER(context, !error.message.includes('statement timeout'),
					backend.errors.JellyfishDatabaseTimeoutError,
					() => {
						return `Schema query timeout with new compiler: ${JSON.stringify(schema)}`
					})
				assert.USER(context, !error.message.startsWith('invalid regular expression:'),
					backend.errors.JellyfishInvalidRegularExpression,
					() => {
						return `Invalid pattern in schema with new compiler: ${JSON.stringify(schema)}`
					})
				throw error
			})
			const newQueryEnd = performance.now()

			metrics.markSqlGenTime(queryGenEnd - queryGenStart, false)
			metrics.markSqlGenTime(newQueryGenEnd - newQueryGenStart, true)
			const queryTime = queryEnd - queryStart
			metrics.markQueryTime(queryTime, false)
			const newQueryTime = newQueryEnd - newQueryStart
			metrics.markQueryTime(newQueryTime, true)

			if (newQueryTime - queryTime > 300) {
				const explain = 'EXPLAIN (SETTINGS ON, SUMMARY ON)'
				logger.warn(context, 'Performance regression in new compiler', {
					table,
					schema,
					options,
					query,
					explain: await tx.any(`${explain} ${query}`),
					newQuery,
					newExplain: await tx.any(`${explain} ${newQuery}`)
				})
			}

			const strippedResults = _.sortBy(results, [ 'id' ]).map((card) => {
				return getStrippedCard(card)
			})
			const newStrippedResults = _.sortBy(newResults, [ 'id' ]).map((card) => {
				return getStrippedCard(card)
			})
			if (!_.isEqual(strippedResults, newStrippedResults)) {
				metrics.markCompilerMismatch()
				logger.warn(context, 'Result mismatch between old and new compilers', {
					table,
					schema,
					options,
					query,
