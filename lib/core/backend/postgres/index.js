 ...
 */
const getPendingRequests = async (context, backend, options = {}) => {
...
const queryTable = async (context, backend, table, schema, options) => {
	...
	const results = await utils.query(backend.connection, query)
...
module.exports = class PostgresBackend {
	...
	async connect (context) {
		...
		const databases = _.map(await utils.query(this.connection, `
		...
		if (!databases.includes(this.database)) {
			 ...
			 */
			await utils.query(this.connection, `
	...
	async query (context, schema, options = {}) {
		...
		if (schema.type === 'object' &&
			...
			return getPendingRequests(context, this, {
				limit: options.limit,
				skip: options.skip,
				mask: schema
			})
