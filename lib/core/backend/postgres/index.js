   ...
   * - closed: When the connection is closed after calling `.close()`
	...
	async stream (context, querySchema) {
		...
		const changeHandler = async (change) => {
			...
			/*
			 * Postgres restricts livefeed payloads to 8000 bytes. We have some
			 * cards that are particularly big, so we make Postgres send us
			 * the id and the type of the cards that changed, and then we fetch
			 * the whole thing here to workaround that size limitation that
			 * doesn't seem to be easily configurable.
			 */
			const cards = await Bluebird.props({
				after: change.after && this.getElementById(context, change.after.id, {
					type: change.after.type
				}),
				before: change.before && this.getElementById(context, change.before.id, {
					type: change.before.type
				})
			})

			let newMatch = filter(schema, cards.after)
			let oldMatch = filter(schema, cards.before)
