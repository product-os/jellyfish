...
exports.const = (value, path, walk, schema, context, keys, requiredPaths) => {
	if (
		...
		if (path.length === 2 && _.last(path) === 'version') {
			if (!_.isString(value)) {
				return {
					keys,
					filter: false
			...
			return {
				keys,
				filter: builder.and(
					`${builder.getProperty([ path[0], 'version_major' ])} = ${fragments[0]}`,
					`${builder.getProperty([ path[0], 'version_minor' ])} = ${fragments[1]}`,
					`${builder.getProperty([ path[0], 'version_patch' ])} = ${fragments[2]}`)
			}
		}
