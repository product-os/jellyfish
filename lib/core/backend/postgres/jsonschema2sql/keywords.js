...
exports.const = (value, path, walk, schema, context, keys, requiredPaths) => {
	...
	// A small optimisation to make const queries on type string friendlier to
	// indexing, as string comparison is by far the most common case
	// note: If path[0] is null, then this is being run inside a subselect
	if (!_.isNull(path[0]) && _.isString(value) && !schema.enum) {
		const propPath = builder.getProperty(path, {
			text: true
		})

		return {
			keys,
			filter: `${propPath} = ${builder.valueToPostgres(value)}`
		}
	}
