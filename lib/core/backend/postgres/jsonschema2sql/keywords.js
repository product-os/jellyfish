...
exports.properties = (value, path, walk, schema, context) => {
	return builder.and(...Object.entries(value).map(([ key, subschema ]) => {
		...
		return builder.or(
			...
			walk(subschema, path.concat([ key ]), context).sql)
	}))
}
...
exports.propertyNames = (value, path, walk, schema, context) => {
	if (value === true) {
		return true
	}

	const subexpression = walk(value, [ null, 'key' ], context)
	return builder.or(
		builder.isNotOfType(path, 'object'),
...
exports.contains = (value, path, walk, schema, context) => {
	return builder.or(
		builder.isNotOfType(path, 'array'),
		builder.someArray(path, value, walk, context))
}

exports.items = (value, path, walk, schema, context) => {
	if (!value) {
	...
	if (Array.isArray(value)) {
		...
		const subexpression =
			builder.and(...value.slice(0, length).map((expression, index) => {
				return walk(expression, path.concat([ index ]), context).sql
			}))
...
exports.anyOf = (value, path, walk, schema, context) => {
	return builder.or(...value.map((subschema) => {
		return walk(subschema, path, context).sql
	}))
}

exports.allOf = (value, path, walk, schema, context) => {
	return builder.and(...value.map((subschema) => {
		return walk(subschema, path, context).sql
	}))
}

exports.not = (value, path, walk, schema, context) => {
	return builder.not(walk(value, path, context).sql)
}

exports.patternProperties = (value, path, walk, schema, context) => {
	...
	const subexpression = builder.and(...Object.entries(value)
		.map(([ key, subschema ]) => {
			...
			return builder.or(
				...
				walk(subschema, [ null, 'value' ], context).sql)
		}))
...
exports.additionalProperties = (value, path, walk, schema, context) => {
	...
	return builder.or(
		...
		builder.noneObject(path, builder.and(
			subexpression,
			builder.not(walk(value, [ null, 'value' ], context).sql))))
}
