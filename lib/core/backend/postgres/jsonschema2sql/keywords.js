...
exports.properties = (value, path, walk, schema, context) => {
	return builder.and(...Object.entries(value).map(([ key, subschema ]) => {
		...
		return builder.or(
			...
			walk(subschema, path.concat([ key ]), context))
	}))
}
	...
	if (Array.isArray(value)) {
		...
		const subexpression =
			builder.and(...value.slice(0, length).map((expression, index) => {
				return walk(expression, path.concat([ index ]), context)
			}))
...
exports.anyOf = (value, path, walk, schema, context) => {
	return builder.or(...value.map((subschema) => {
		return walk(subschema, path, context)
	}))
}
...
exports.not = (value, path, walk, schema, context) => {
	return builder.not(walk(value, path, context))
}
	...
	const subexpression = builder.and(...Object.entries(value)
		.map(([ key, subschema ]) => {
			...
			return builder.or(
				...
				walk(subschema, [ null, 'value' ], context))
		}))
...
exports.additionalProperties = (value, path, walk, schema, context) => {
	...
	return builder.or(
		...
		builder.noneObject(path, builder.and(
			subexpression,
			builder.not(walk(value, [ null, 'value' ], context)))))
}
