	...
	toSqlInto (builder) {
		...
		if (this.orderBy.length > 0) {
			...
			for (const [ idx, [ path, isDescending ] ] of this.orderBy.entries()) {
				...
				builder.push(path.toSql())
				builder.push(isDescending ? ' DESC' : ' ASC')
				builder.push(' NULLS LAST')
			}
		}
		if (this.offset > 0) {
			builder.push('\nOFFSET ')
			builder.push(this.offset)
		}
		if (this.limit !== null) {
			builder.push('\nLIMIT ')
			builder.push(this.limit)
		}
	}
}
...
class SqlJoin {
	...
	toSqlInto (builder) {
		...
		if (_.isString(this.table)) {
			...
			builder.push('(\n')
			this.table.toSqlInto(builder)
			builder.push('\n)')
		}
		if (this.alias) {
			builder.push(' AS ')
			builder.push(this.alias)
		}
		builder.push('\nON ')
		this.filter.toSqlInto(builder)
	}
