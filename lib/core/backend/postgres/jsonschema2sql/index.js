	...
	toSqlSelect () {
		...
		if (this.additionalProperties) {
			columns = [
				...
				`${this.table}.type`,
				`${this.table}.active`,
				`${SqlPath.getVersionComputedField(this.table)} AS version`,
				`${this.table}.name`,
				`${this.table}.tags`,
				`${this.table}.markers`,
			...
			columns = Array.from(this.columns.values()).map((column) => {
				if (column === 'version') {
					return `${SqlPath.getVersionComputedField(this.table)} AS version`
				}
...
ON ${fragments.lateralAlias}.source = linked.id`)
	...
	getSqlOrderBy (tableOverride) {
		...
		if (this.options.sortBy) {
			const sortBy = _.clone(_.castArray(this.options.sortBy))
