...
/*
 * For the PostgreSQL index to be used the search vector used in a query must match
 * the one used for index creation.
 */
const createSearchVector = (table) => {
	return `
		(
			to_tsvector(
				'english',
				...
				coalesce(${table}.slug,'') || ' ' ||
				${table}.id || ' '
				|| array_to_tsvector(${table}.tags)
			)
			|| to_tsvector('english', ${table}.data)
		)
	`
}

const createSearchQuery = (table, term) => {
	return `${createSearchVector(table)} @@ to_tsquery(quote_literal(${builder.valueToPostgres(term)}) || ':*')`
}

exports.createIndexName = (table) => {
	return `${table}_textsearch_idx`
}

exports.createSearchIndex = (table) => {
	return `
		CREATE INDEX CONCURRENTLY ${exports.createIndexName(table)} ON ${table} USING GIN (${createSearchVector(table)})
	`
}

exports.createQuery = (table, schema, options = {}) => {
	...
	if (schema.$$search) {
		query.push('AND (')
		query.push(createSearchQuery(table, schema.$$search))

		// If links are specified, the search query should match the head card OR
		// the linked card
		if (link && link.schema) {
			query.push('OR')
			query.push(createSearchQuery(linkedCard, schema.$$search))
		}
