...
class SqlFilter {
	...
	WHERE ${SqlFilter.sqlfyFilter(filter)}
)`)
	}
	...
	static materialConditional (condition, implicated) {
		return (new SqlFilter(condition)).negate().or(implicated)
	}

	static maybeJsonLiteral (query, value) {
		if (query.isProcessingJsonProperty()) {
			if (_.isString(value)) {
				const literal = pgFormat.literal(value)
				...
				const contents = value.map((item) => {
					let literal = SqlFilter.maybeJsonLiteral(query, item)
					if (literal.endsWith('::jsonb')) {
						literal = literal.substring(0, literal.length - 7)
					}
					if (literal[0] === '\'') {
	...
	static sqlfyFilter (filter, op) {
		if (_.isString(filter)) {
			return filter
		}

		// Assume it's an instance of this object
	...
	constructor (filter) {
		if (!_.isUndefined(filter)) {
			this.expr = [ AND, SqlFilter.sqlfyFilter(filter) ]
		}
	}

	and (...filters) {
		return this.applyBinaryOperator(AND, filters)
	}

	or (...filters) {
		return this.applyBinaryOperator(OR, filters)
	}

	applyBinaryOperator (op, filters) {
		if (this.expr) {
			if (this.expr[0] !== op) {
				this.expr = [ op, this.toSql() ]
			}
		} else {
			this.expr = [ op ]
		}

		this.expr.push(...filters.map((filter) => {
			return SqlFilter.sqlfyFilter(filter, op)
		}))
	...
	toSql () {
		return SqlFilter.sqlfyFilter(this)
	}
}
...
class SqlQuery {
	...
	static fromSchema (tableOrParent, schema, options) {
		...
		if (schema === false) {
			...
			if (!_.isUndefined(schema.required)) {
	...
	addRequired (required) {
		assert.INTERNAL(null, Array.isArray(required), Error,
			`value for '${this.formatJsonPath('required')}' must be a string`)

		const requiredFilter = new SqlFilter()
		this.path.push(null)
		for (const key of required) {
			...
			if (isProcessingColumn) {
				if (_.get(CARD_FIELDS, [ key, 'nullable' ])) {
		...
		this.path.pop()
		this.filter.and(this.ifTypesThen('object', requiredFilter))
	}

	setAdditionalProperties (schema) {
		// TODO: technically this can be a schema too
		assert.INTERNAL(null, _.isBoolean(schema), Error,
			`value for '${this.formatJsonPath('additionalProperties')}' must be a boolean`)
	...
	visit (key, value) {
		assert.INTERNAL(null, _.isString(key), Error,
			`key for '${this.formatJsonPath(key)}' must be a string`)
		...
		assert.INTERNAL(null, this[visitor], Error,
			`invalid key: ${this.formatJsonPath(key)}`)
	...
	$$linksVisitor (linkMap) {
		...
		assert.INTERNAL(null, _.isPlainObject(linkMap), Error,
			`value for '${this.formatJsonPath('$$links')}' must be a map`)

		for (const [ linkType, linkSchema ] of Object.entries(linkMap)) {
			assert.INTERNAL(null, _.isString(linkType), Error,
				`key for '${this.formatJsonPath([ '$$links', linkType ])}' must be a string`)
	...
	allOfVisitor (branches) {
		assert.INTERNAL(null, Array.isArray(branches), Error,
			`value for '${this.formatJsonPath('allOf')}' must be an array`)
	...
	anyOfVisitor (branches) {
		assert.INTERNAL(null, Array.isArray(branches), Error,
			`value for '${this.formatJsonPath('anyOf')}' must be an array`)
	...
	containsVisitor (schema) {
		...
		this.filter.and(this.ifTypesThen('array', filter))
	}

	enumVisitor (values) {
		assert.INTERNAL(null, Array.isArray(values), Error,
			`value for '${this.formatJsonPath('enum')}' must be an array`)
	...
	formatVisitor (format) {
		assert.INTERNAL(null, _.isString(format), Error,
			`value for '${this.formatJsonPath('format')}' must be a string`)

		const regex = REGEXES.format[format]
		if (regex) {
			const filter = SqlFilter.matchesPattern(this, regex)
			this.filter.and(this.ifTypesThen('string', filter))
		} else {
	...
	exclusiveMaximumVisitor (limit) {
		assert.INTERNAL(null, _.isNumber(limit), Error,
			`value for '${this.formatJsonPath('exclusiveMaximum')}' must be a number`)

		const filter = SqlFilter.valueIs(this, '<', limit)
		this.filter.and(this.ifTypesThen('number', filter))
	}

	exclusiveMinimumVisitor (limit) {
		assert.INTERNAL(null, _.isNumber(limit), Error,
			`value for '${this.formatJsonPath('exclusiveMinimum')}' must be a number`)
	...
	arrayContentsMustMatch (schema) {
		...
		this.filter.and(this.ifTypesThen('array', filter.negate()))
	}
	...
	maximumVisitor (limit) {
		assert.INTERNAL(null, _.isNumber(limit), Error,
			`value for '${this.formatJsonPath('maximum')}' must be a number`)
	...
	maxLengthVisitor (limit) {
		assert.INTERNAL(null, _.isNumber(limit), Error,
			`value for '${this.formatJsonPath('maxLength')}' must be a number`)
	...
	maxItemsVisitor (limit) {
		assert.INTERNAL(null, _.isNumber(limit), Error,
			`value for '${this.formatJsonPath('maxItems')}' must be a number`)
	...
	maxPropertiesVisitor (limit) {
		assert.INTERNAL(null, _.isNumber(limit), Error,
			`value for '${this.formatJsonPath('maxProperties')}' must be a number`)

		const filter = SqlFilter.propertyCountIs(this, '<=', limit)
		this.filter.and(this.ifTypesThen('object', filter))
	}

	minimumVisitor (limit) {
		assert.INTERNAL(null, _.isNumber(limit), Error,
			`value for '${this.formatJsonPath('minimum')}' must be a number`)
	...
	minLengthVisitor (limit) {
		assert.INTERNAL(null, _.isNumber(limit), Error,
			`value for '${this.formatJsonPath('minLength')}' must be a number`)
	...
	minItemsVisitor (limit) {
		assert.INTERNAL(null, _.isNumber(limit), Error,
			`value for '${this.formatJsonPath('minItems')}' must be a number`)
	...
	minPropertiesVisitor (limit) {
		assert.INTERNAL(null, _.isNumber(limit), Error,
			`value for '${this.formatJsonPath('minProperties')}' must be a number`)
	...
	multipleOfVisitor (multiple) {
		assert.INTERNAL(null, _.isNumber(multiple), Error,
			`value for '${this.formatJsonPath('multipleOf')}' must be a number`)
	...
	patternVisitor (pattern) {
		assert.INTERNAL(null, _.isString(pattern), Error,
			`value for '${this.formatJsonPath('pattern')}' must be a string`)
	...
	propertiesVisitor (propertiesMap) {
		assert.INTERNAL(null, _.isPlainObject(propertiesMap), Error,
			`value for '${this.formatJsonPath('properties')}' must be a map`)
	...
	regexpVisitor (value) {
		...
		assert.INTERNAL(null, isString || _.isPlainObject(value), Error,
			`value for '${this.formatJsonPath('regexp')}' must be a string or a map`)
	...
	typeVisitor (value) {
		...
		assert.INTERNAL(null, isArray || isString, Error,
			`value for '${this.formatJsonPath('type')}' must be a string or an array`)
	...
	ifTypesThen (types, filter) {
		const typeFilter = this.getIsTypesFilter(_.castArray(types))
		if (typeFilter === null) {
			return filter
		} else if (typeFilter === false) {
			return (new SqlFilter()).makeUnsatisfiable()
		}
	...
	buildQueryFromSubSchema (schema, suffix) {
		...
		options.parentJsonPath = _.concat(options.parentJsonPath, _.castArray(suffix))

		return SqlQuery.fromSchema(this, schema, options)
	}
	...
	buildQueryFromCorrelatedSchema (table, schema, suffix) {
		...
		return SqlQuery.fromSchema(table, schema, {
...
module.exports = (table, schema, options = {}) => {
	...
	console.log(generateQuery(table, schema, options))
	const query = SqlQuery.fromSchema(table, schema, options).toSqlSelect()
	console.log('----------')
	console.log(query)
