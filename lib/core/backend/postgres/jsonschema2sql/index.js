...
const CARD_COLUMNS = _.assign({},
	CARD_TOP_LEVEL_REQUIRED_FIELD_TYPES,
	CARD_TOP_LEVEL_OPTIONAL_FIELD_TYPES)
...
const getTypeExpression = (property, value, options, toplevel) => {
	const row = getRowFromProperty(property, true)

	if (property.length === 0 || toplevel) {
		return null
	}

	if (property.length === 1 && options.assumeValidCard) {
		const propertyName = _.isArray(property[0]) ? property[0][0] : property[0]
		if (CARD_TOP_LEVEL_REQUIRED_FIELD_TYPES[propertyName] &&
				CARD_TOP_LEVEL_REQUIRED_FIELD_TYPES[propertyName] === value) {
			return null
		}

		if (CARD_TOP_LEVEL_OPTIONAL_FIELD_TYPES[propertyName] &&
				CARD_TOP_LEVEL_OPTIONAL_FIELD_TYPES[propertyName] === value) {
			return null
		}
	}
...
const getContainsExpression = (property, value, options, toplevel) => {
	const row = getRowFromProperty(property, true)
	if (_.isBoolean(value)) {
		if (value) {
	...
	if (value.minimum) {
		...
		if (property.length === 1 && options.assumeValidCard) {
			const propertyName = _.isArray(property[0]) ? property[0][0] : property[0]
			if (CARD_TOP_LEVEL_REQUIRED_FIELD_TYPES[propertyName] &&
					CARD_TOP_LEVEL_REQUIRED_FIELD_TYPES[propertyName] === 'array') {
				conjuncts.push(`(${subquery})`)
			}

			if (CARD_TOP_LEVEL_OPTIONAL_FIELD_TYPES[propertyName] &&
					CARD_TOP_LEVEL_OPTIONAL_FIELD_TYPES[propertyName] === 'array') {
				conjuncts.push(`(${subquery})`)
			}
		} else {
	...
	if (value.const) {
		...
		} else if (toplevel || property.length === 1) {
			conjuncts.push(`(${row} @> ARRAY[${format(value.const)}])`)
		} else {
			conjuncts.push(`(${row} ? ${format(value.const)})`)
		}
	}
...
const getFilterExpression = (operator, property, value, schema, root, options, toplevel) => {
	let row = getRowFromProperty(property)
	switch (operator) {
		case 'type': {
			return getTypeExpression(property, value, options, toplevel)
		}
		case 'const': {
			if (options.assumeValidCard && property.length === 1) {
				const propertyName = _.isArray(property[0]) ? property[0][0] : property[0]
				if (CARD_COLUMNS[propertyName]) {
					return `${propertyName} = ${format(value)}`
				}
			}
		...
		case 'pattern': {
			...
			if (options.assumeValidCard && property.length === 1) {
				const propertyName = _.isArray(property[0]) ? property[0][0] : property[0]
				if (CARD_TOP_LEVEL_REQUIRED_FIELD_TYPES[propertyName] ||
						CARD_TOP_LEVEL_OPTIONAL_FIELD_TYPES[propertyName]) {
					if (CARD_TOP_LEVEL_REQUIRED_FIELD_TYPES[propertyName] === 'string' ||
							CARD_TOP_LEVEL_OPTIONAL_FIELD_TYPES[propertyName] === 'string') {
						if (CARD_COLUMNS[propertyName]) {
							return `(${propertyName} ~* ${format(value)})`
						}
			...
			if (toplevel) {
				return `(${getRowFromProperty(property, true)} ~* ${format(value)})`
			}
		...
		case 'items': {
			...
			if (_.isBoolean(value)) {
				if (options.assumeValidCard && property.length === 1) {
					...
					if (CARD_TOP_LEVEL_REQUIRED_FIELD_TYPES[propertyName] ||
							CARD_TOP_LEVEL_OPTIONAL_FIELD_TYPES[propertyName]) {
						if (CARD_TOP_LEVEL_REQUIRED_FIELD_TYPES[propertyName] === 'array' ||
								CARD_TOP_LEVEL_OPTIONAL_FIELD_TYPES[propertyName] === 'array') {
							if (value) {
								return null
							}
			...
			if (options.assumeValidCard && property.length === 1) {
				...
				if (CARD_TOP_LEVEL_REQUIRED_FIELD_TYPES[propertyName] ||
					...
					if (CARD_TOP_LEVEL_REQUIRED_FIELD_TYPES[propertyName] === 'array' ||
							CARD_TOP_LEVEL_OPTIONAL_FIELD_TYPES[propertyName] === 'array') {
						const subexpression = parseFragment(value, options, [ [ 'unnest', true ] ], root, true)
							.join('\n')

						return [
							`(NOT EXISTS (SELECT 1 FROM UNNEST(${row})`,
		...
		case 'minimum': {
			...
			if (options.assumeValidCard && property.length === 1) {
				...
				if (CARD_TOP_LEVEL_REQUIRED_FIELD_TYPES[propertyName] ||
						CARD_TOP_LEVEL_OPTIONAL_FIELD_TYPES[propertyName]) {
					if (CARD_TOP_LEVEL_REQUIRED_FIELD_TYPES[propertyName] === 'number' ||
							CARD_TOP_LEVEL_OPTIONAL_FIELD_TYPES[propertyName] === 'number') {
		...
		case 'maxProperties': {
			...
			if (options.assumeValidCard && property.length === 1) {
				...
				if (CARD_TOP_LEVEL_REQUIRED_FIELD_TYPES[propertyName] ||
						CARD_TOP_LEVEL_OPTIONAL_FIELD_TYPES[propertyName]) {
					if (CARD_TOP_LEVEL_REQUIRED_FIELD_TYPES[propertyName] === 'object' ||
							CARD_TOP_LEVEL_OPTIONAL_FIELD_TYPES[propertyName] === 'object') {
		...
		case 'enum': {
			...
			if (options.assumeValidCard && property.length === 1) {
				...
				if (CARD_TOP_LEVEL_REQUIRED_FIELD_TYPES[propertyName] ||
					...
					const quotedProperties = value.map((item) => {
						return `'${item}'`
					})

					return `${propertyName} IN (${quotedProperties.join(', ')})`
				}
			}

			const cases = value.map((item) => {
				if (toplevel) {
					return `${row} = ${format(JSON.stringify(item))}`
				}
		...
		case 'contains': {
			return getContainsExpression(property, value, options, toplevel)
		}
		case 'multipleOf': {
			row = getRowFromProperty(property, true)
...
const parseFragment = (fragment, options, name = [], root, toplevel = false) => {
	if (name.length === 1 && name[0] === 'data') {
		name[0] = [ name[0], true ]
	}
	...
	if (fragment.type === 'object' || _.has(fragment, [ 'properties' ])) {
		...
		if (fragment.type === 'object') {
			const typeExpression = getTypeExpression(name, 'object', options, toplevel)
			if (typeExpression) {
			...
			for (const property of Object.keys(fragment.properties)) {
				...
				if (_.isBoolean(value)) {
					if (!value) {
						...
						if (options.assumeValidCard && path.length === 1) {
							const propertyName = _.isArray(path[0]) ? path[0][0] : path[0]
							if (CARD_TOP_LEVEL_REQUIRED_FIELD_TYPES[propertyName]) {
								return [ 'false' ]
							}
						}
				...
				if (value.not && _.isEmpty(value.not)) {
					...
					if (options.assumeValidCard &&
							path.length === 1 &&
							CARD_TOP_LEVEL_REQUIRED_FIELD_TYPES[propertyName]) {
					...
					const expression = parseFragment(value, options, path, root, toplevel)
					if (expression && expression.length) {
						result = getConjunction(result, expression)
					}
				} else {
					const parsed = parseFragment(value, options, path, root, toplevel)
		...
		if (fragment.required && fragment.required.length) {
			const requirements = fragment.required.reduce((accumulator, property) => {
				...
				const propertyName = _.isArray(path[0]) ? path[0][0] : path[0]
				if (path.length === 1 && options.assumeValidCard) {
					if (CARD_TOP_LEVEL_REQUIRED_FIELD_TYPES[propertyName]) {
						return accumulator
					}
				}
			...
			const propertyName = _.isArray(name[0]) ? name[0][0] : name[0]
			if ((name.length !== 1 && name.length !== 0) || !options.assumeValidCard) {
				if (name.length > 1 ||
						(CARD_TOP_LEVEL_REQUIRED_FIELD_TYPES[propertyName] !== 'object' &&
							CARD_TOP_LEVEL_OPTIONAL_FIELD_TYPES[propertyName] !== 'object')) {
		...
		if (fragment.not) {
			...
			const expression = _.flattenDeep([
				'NOT',
				'(',
				parseFragment(fragment.not, options, name, root, toplevel),
				')'
			])
			result = getConjunction(result, expression)
		}
		...
		if (fragment.allOf) {
			const subresult = fragment.allOf.map((conjunct) => {
				...
				const exprs = parseFragment(copy, options, name, root, toplevel)
		...
		if (fragment.anyOf) {
			const subresult = fragment.anyOf.map((disjunct) => {
				...
				if (disjunct.anyOf) {
					copy.anyOf = _.cloneDeep(disjunct.anyOf)
				} else {
		...
		if (
			...
			if (fragment.patternProperties) {
				const patternProperties = _.map(fragment.patternProperties, (subSchema, pattern) => {
					...
					const conjunct = parseFragment(
						subSchema, options, [ [ 'value', true ] ], root, toplevel)
					if (!conjunct.length) {
						return `(key !~ ${format(pattern)})`
					}
			...
			if (options.assumeValidCard && name.length === 1) {
				const propertyName = _.isArray(name[0]) ? name[0][0] : name[0]
				if (CARD_TOP_LEVEL_REQUIRED_FIELD_TYPES[propertyName] ||
						CARD_TOP_LEVEL_OPTIONAL_FIELD_TYPES[propertyName]) {
					if (CARD_TOP_LEVEL_REQUIRED_FIELD_TYPES[propertyName] === 'object' ||
		...
		if (!isExcludedKeyword(options, 'additionalProperties') &&
			...
			const filter = parseFragment(
				fragment.additionalProperties, options, [ [ 'value', true ] ], root, toplevel)
		...
		if (property === 'not' && value !== false) {
			...
			if (value === true) {
				...
				if (!options.assumeValidCard ||
					...
					!CARD_TOP_LEVEL_REQUIRED_FIELD_TYPES[propertyName]) {
					expression.push(`${getRowFromProperty(name, true)} IS NULL OR`)
				}

				expression.push('NOT (')
				expression.push(...parseFragment(fragment.not, options, name, root, toplevel))
				expression.push(')')
				expression.push(')')
				result = getConjunction(result, expression)
			}
			...
			if (
				...
				const operatorResult = value.map((disjunct) => {
					return `(${parseFragment(disjunct, options, name, root, toplevel).join('\n')})`
				}).filter((disjunct) => {
					return disjunct !== '()'
				})
			...
			if (!root && _.isString(name[0])) {
				name[0] = [ name[0], true ]
			}

			const expression = getFilterExpression(
				property, name, value, fragment, root, options, toplevel)
			if (expression) {
				result = getConjunction(result, expression)
			}
		}
...
module.exports = (table, schema, options = {}) => {
	options.assumeValidCard = true
	const ROOT = options.assumeValidCard ? null : 'data'
	...
	const path = ROOT ? [ [ ROOT, true ] ] : []
	const filter = parseFragment(schema, options, path, ROOT)
	...
	if (options.sortBy) {
		...
		const expression = ROOT
			...
			: getRowFromProperty([ [ order[0], true ] ].concat(order.slice(1)))
