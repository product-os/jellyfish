...
const createSearchQuery = (table, term) => {
	return `${createSearchVector(table)} @@ plainto_tsquery(${builder.valueToPostgres(term)})`
}
...
exports.createSearchIndex = (table) => {
	return `
		CREATE INDEX ${exports.createIndexName(table)} ON ${table} USING GIN (${createSearchVector(table)})
	`
}
