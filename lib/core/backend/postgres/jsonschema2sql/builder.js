...
exports.getProperty = (property, options = {}) => {
	...
	const root = property[0]
		? `${property[0]}.${pgFormat.ident(property[1])}`
...
exports.notExists = (path, requiredPaths) => {
	if (exports.isRequiredColumn(path) || requiredPaths.has(path.toString())) {
		return false
	}

	const property = exports.getProperty(path)
	if (!property) {
		return true
	}
...
exports.everyArray = (property, schema, walk) => {
	if (exports.isColumn(property)) {
		const expression = exports.not(
			walk(schema, [ null, 'unnest' ], CARD_FIELDS[property[1]]).filter)
		return [
	...
	const expression = exports.not(walk(schema, [ null, 'value' ]).filter)
	return [
