...
exports.someArray = (property, schema, walk, onKey) => {
	if (exports.isColumn(property)) {
		const expression = walk(
			schema, [ null, 'unnest' ], CARD_FIELDS[property[1]], onKey)
		return [
	...
	const expression = walk(schema, [ null, 'value' ], null, onKey)
	return [
...
exports.everyArray = (property, schema, walk, onKey) => {
	if (exports.isColumn(property)) {
		const expression = exports.not(
			walk(schema, [ null, 'unnest' ], CARD_FIELDS[property[1]]).sql, onKey)
		return [
	...
	const expression = exports.not(walk(schema, [ null, 'value' ], null, onKey).sql)
	return [
