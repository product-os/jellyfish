...
const createTableStream = async (context, backend, table, schema) => {
	...
	cursor.each(async (error, change) => {
		...
		if (newMatch || oldMatch) {
			emitter.emit('data', {
				type: change.old_val ? 'update' : 'insert',
...
const getElementsByIdFromTable = async (context, backend, table, ids) => {
	...
	const elements = await backend.connection
		.db(backend.database)
		.table(table)
		.getAll(...ids, {
			index: 'id'
...
const getTablesSafe = async (connection, database, errors, retries = 10) => {
	if (retries === 0) {
	...
	try {
		...
		if (error.name === 'ReqlOpFailedError' &&
			...
			return getTablesSafe(connection, database, errors, retries - 1)
		}
...
const createTableSafe = async (connection, database, errors, name, retries = 10) => {
	if (retries === 0) {
	...
	try {
		...
		if (error.name === 'ReqlOpFailedError' &&
				error.msg === `Database \`${database}\` does not exist.`) {
			await Bluebird.delay(100)
			return createTableSafe(connection, database, errors, name, retries - 1)
		}
...
const checkUpsertErrors = (object, results, errors) => {
	if (results.errors !== 0) {
...
const upsertObject = async (context, backend, table, object, options) => {
	if (!object.slug) {
		throw new backend.errors.JellyfishDatabaseError(
			'Missing primary key')
	}
	...
	checkUpsertErrors(object, results, backend.errors)
	...
	if (_.isNil(insertedObject.name)) {
		insertedObject.name = null
	}
	...
	if (insertedObject.type === 'link') {
		...
		await Bluebird.map([
			...
			const linkResults = await backend.connection
				.db(backend.database)
				.table(getBucketForType(updatedCard.type))
				.get(updatedCard.slug)
				.update({
					links: updatedCard.links
				})
				.run()

			checkUpsertErrors(updatedCard, linkResults)
...
module.exports = class Backend {
   ...
   * const backend = new Backend(cache, errors, {
   ...
   */
	async getTables (context) {
		...
		return getTablesSafe(this.connection, this.database, this.errors)
	}
   ...
   */
	async createTable (context, name) {
		if (!await this.hasTable(context, name)) {
			...
			await createTableSafe(this.connection, this.database, this.errors, name)
   ...
   */
	async getElementsById (context, ids, options) {
		if (!options.type) {
			throw new this.errors.JellyfishNoIdentifier(
				`No types when getting elements by id: ${ids}`)
		}
   ...
   * await backend.connect()
	...
	async lock (owner, slug) {
		...
		if (response.errors > 0) {
			...
			throw new this.errors.JellyfishDatabaseError(response.first_error)
		}
