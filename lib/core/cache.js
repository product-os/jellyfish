...
const logger = require('../logger').getLogger('jellyfish:cache')
...
module.exports = class Cache {
	 ...
	 * @param {String} options.database - database name, will be used as key prefix
	...
	async connect (ctx) {
		if (this.options.mock) {
			logger.debug(ctx, 'Using mocked cache')
	...
	generateKey (table, category, key) {
		return `${this.options.database}:${table}:${category}:${key}`
	}
