...
ava('CardField renders a primative item with a label', async (test) => {
	const props = {
		field: 'id',
		payload: {
			id: 'test'
...
ava('CardField recursively renders an object as CardField components inside a Collapsed component', async (test) => {
	const props = {
		field: 'details',
		payload: {
			details: {
	...
	const component = shallowCardField(props)
	const collapsibleComponent = component.find('Collapsible').dive()

	const titleComponent = findTitleComponent(collapsibleComponent, props.field)
	test.is(titleComponent.props().children, props.field)

	const cardFieldComponents = collapsibleComponent.find('CardField')
	let cardField = cardFieldComponents.at(0).dive()

	let labelComponent = findLabelComponent(cardField, 'id')
	test.is(labelComponent.props().children, 'id')

	let valueComponent = findValueComponent(cardField, 'id')
	test.is(valueComponent.props().fieldValue, 'test')

	cardField = cardFieldComponents.at(1).dive()

	labelComponent = findLabelComponent(cardField, 'name')
	test.is(labelComponent.props().children, 'name')

	valueComponent = findValueComponent(cardField, 'name')
	test.is(valueComponent.props().fieldValue, 'hello')
})
