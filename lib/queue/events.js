 ...
 */
exports.EXECUTION_EVENT_VERSION = '1.0.0'
...
exports.post = async (context, jellyfish, session, options, results) => {
	...
	const contents = {
		type: `${EXECUTION_EVENT_TYPE}@${exports.EXECUTION_EVENT_VERSION}`,
		slug: exports.getExecuteEventSlug({
			id: options.id
		}),
		version: exports.EXECUTION_EVENT_VERSION,
		active: true,
		links: {},
		markers: [],
		tags: [],
		...
		properties: {
			...
			type: {
				...
				enum: [
					...
					`${EXECUTION_EVENT_TYPE}@${exports.EXECUTION_EVENT_VERSION}`
				]
			},
			data: {
				type: 'object',
...
exports.wait = async (context, jellyfish, session, options) => {
	...
	return new Bluebird((resolve, reject) => {
		...
		jellyfish.getCardBySlug(
			context, session, `${slug}@${exports.EXECUTION_EVENT_VERSION}`).then((card) => {
			if (!card) {
				return
			}
