	...
	return new Bluebird((resolve, reject) => {
		const timeout = setTimeout(() => {
			jellyfish.getCardBySlug(context, session, slug, {
				...
				if (card) {
					result = result || card
					logger.warn(context, 'Found results after stalled stream', {
						slug: result.slug,
				...
				stream.close()
				return reject(new errors.QueueStreamTimeout(
					'Did not receive results in time'))
			}).catch((error) => {
				logger.exception(context, 'Stream error after timeout', error)
				stream.removeAllListeners()
				stream.close()
				return reject(error)
			})
