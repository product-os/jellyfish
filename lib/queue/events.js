...
exports.post = async (context, jellyfish, session, options, results) => {
	...
	return jellyfish.insertCard(context, session, contents).catch((error) => {
		// TODO: Remove once the RabbitMQ migration is done
		if (error instanceof jellyfish.errors.JellyfishElementAlreadyExists) {
			return jellyfish.getCardBySlug(context, session, contents.slug, {
				type: contents.type
			})
		}
