...
const rabbitEnqueue = async (channel, queue, data, retries = 10) => {
	...
	return new Promise((resolve, reject) => {
		...
		if (error.name === 'IllegalOperationError' &&
			...
			retries > 0) {
			return rabbitEnqueue(channel, queue, data, retries - 1)
		}
...
module.exports = class Queue extends EventEmitter {
   ...
   */
	...
	async enqueue (actor, session, options) {
		...
		await rabbitEnqueue(
			this.rabbit.channel, this.options.name, Buffer.from(slug))
