...
const claimRequest = async (context, jellyfish, session, actor, request) => {
	...
	if (!await jellyfish.lock(actor, request)) {
		return null
	}
	...
	if (executeCard) {
		...
		await jellyfish.popRequest(context, request)
		await jellyfish.unlock(actor, request)
		return null
	}
...
module.exports = class Queue extends EventEmitter {
   ...
   */
	...
	async postResults (actor, context, actionRequest, results) {
		...
		await this.jellyfish.unlock(actor, actionRequest)
		return eventCard
	}
