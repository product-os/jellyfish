...
const pickNextUnexecutedRequest = async (context, jellyfish, session, actor, skip = 0) => {
	...
	if (await jellyfish.lock(actor, actionRequest.slug)) {
		...
		logger.debug(context, 'Locking non executed request success', {
			slug: actionRequest.slug,
			time: lockEndDate.getTime() - claimStartDate.getTime()
		})
		...
		const foundEndDate = new Date()
		logger.debug(context, 'Non executed request found', {
			slug: actionRequest.slug,
			time: foundEndDate.getTime() - claimStartDate.getTime()
		})

		return actionRequest
	}

	const lockFailEndDate = new Date()

	logger.debug(context, 'Locking non executed request failure', {
		slug: actionRequest.slug,
		time: lockFailEndDate.getTime() - claimStartDate.getTime()
	})
