...
const claimRequest = async (context, instance, actor, request) => {
	 ...
	 */
	const executeSlug =
		`${events.getExecuteEventSlug(request)}@${events.EXECUTION_EVENT_VERSION}`
	const executeCard = await instance.jellyfish.getCardBySlug(
		context, instance.session, executeSlug)
	if (executeCard) {
	...
	const linkSlug = `${getExecuteLinkSlug(request)}@${EXECUTE_LINK_VERSION}`
	const executeLink = await instance.jellyfish.getCardBySlug(
		context, instance.session, linkSlug)
	if (executeLink) {
...
const pickNextUnexecutedRequest = async (instance, context, actor, skip = 0) => {
	...
	const hasPriority =
		instance.options.enablePriorityBuffer && instance.priority.length > 0
	...
	const results = hasPriority
		...
		// implementation. Make sure it remains fast if you
		// change it.
   ...
   */
	...
	async enqueue (actor, session, options) {
		...
		const cards = await Bluebird.props({
			...
			// TODO: Require users to be explicit on the action version
			// they want to execute.
			action: this.jellyfish.getCardBySlug(
				options.context, session, `${options.action}@latest`),
