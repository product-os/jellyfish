...
module.exports = class Queue extends EventEmitter {
   ...
   */
	...
	async enqueue (actor, session, options) {
		...
		if (!cards.action || !this.library[cards.action.slug]) {
		...
		const actionArgs = jellyscript.evaluateObject({
			type: 'object',
		...
		// The `pre` method can return modified arguments or null. If modified
		// arguments are returned, they should be used instead of the original
		// arguments declared previously
		const modifiedActionArgs = await this.library[cards.action.slug]
			.pre(options.context, session, cards, actionArgs)
		...
		return this.jellyfish.insertCard(options.context, this.session, {
			...
			data: {
				...
				arguments: modifiedActionArgs || actionArgs
			}
		})
	}
}
