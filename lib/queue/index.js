...
const rabbitConnect = async (instance) => {
...
const rabbitEnqueue = async (instance, data, retries = 10) => {
	// The .sendToQueue() method does not return
	// a promise but uses a callback, even in
	// promise mode.
	return new Promise((resolve, reject) => {
		instance.rabbit.channel.sendToQueue(instance.options.name, data, {
			persistent: true
		}, (error, ok) => {
			if (error) {
				return reject(error)
		...
		if (error.name === 'IllegalOperationError' &&
			...
			retries > 0) {
			return rabbitConnect(instance).then(() => {
				return rabbitEnqueue(instance, data, retries - 1)
			})
		}
...
module.exports = class Queue extends EventEmitter {
   ...
   */
	...
	async enqueue (actor, session, options) {
		...
		await rabbitEnqueue(this, Buffer.from(slug))
