...
const rabbitConnect = async (context, instance, retries = 10) => {
	try {
		logger.info(context, 'Connecting to RabbitMQ',
			_.omit(instance.options, [ 'password' ]))
...
const rabbitReconnectHandler = async (context, instance, fn, retries = 10) => {
	try {
		await fn(instance.rabbit.channel, instance.options.name)
	} catch (error) {
		if (error.name === 'IllegalOperationError' &&
			...
			await rabbitReconnectHandler(context, instance, fn, retries - 1)
			return
		}
...
module.exports = class Queue extends EventEmitter {
   ...
   */
	...
	async postResults (actor, context, actionRequest, results) {
		...
		await rabbitReconnectHandler(context, this, async (channel) => {
	...
	async enqueue (actor, session, options) {
		...
		await rabbitReconnectHandler(options.context, this, (channel, name) => {
