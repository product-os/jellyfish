...
const pickNextUnexecutedRequest = async (context, jellyfish, session, actor, skip = 0) => {
	...
	const results = await jellyfish.query(
		context, session, SCHEMA_NON_EXECUTED_REQUESTS, {
			...
			// Get the oldest non executed request we can get. This ensures
			// that the queue is served in a FIFO fashion, and that the oldest
			// requests don't starve if we get loads of requests.
			sortBy: 'created_at',
			sortDir: 'asc'
		})
	...
	if (await jellyfish.lock(actor, actionRequest.slug)) {
		...
		// heal itself.
		if (executeCard) {
			logger.debug(context, 'Omitting non executed request because of execute card', {
				slug: actionRequest.slug,
