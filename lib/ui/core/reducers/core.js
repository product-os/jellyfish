...
exports.actionCreators = {
	...
	setSendCommand: (command) => {
		return async function (dispatch, getState) {
			try {
				const user = await sdk.sdk.auth.whoami()
				_.set(user, [ 'data', 'profile', 'sendCommand' ], command)
				const result = await sdk.sdk.card.update(user.id, user)

				dispatch(exports.actionCreators.setUser(result))
				dispatch(exports.actionCreators.addNotification('success', `Successfully set "${command}" as send command`))
			} catch (error) {
				dispatch(exports.actionCreators.addNotification('danger', error.message || error))
			}
		}
	}
}
