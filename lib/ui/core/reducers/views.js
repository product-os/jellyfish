		...
		return function (dispatch, getState) {
			...
			return sdkHelpers.loadSchema(query)
				.then((schema) => {
					...
					streams[viewId] = core.subscribeToCoreFeed('update', (response) => {
						const {
							after, before
						} = response
						...
						// If after is null, the item has been removed from the result set
							...
							// eslint-disable-next-line consistent-return
							return dispatch(exports.actionCreators.upsertViewData(query, after))
						}
						if (!before && afterValid) {
							// Otherwise, if before is null, this is a new item
							// eslint-disable-next-line consistent-return
							return dispatch(exports.actionCreators.appendViewData(query, after))
						}
					})
				})
				.catch((error) => {
