...
exports.viewSelectors = {
	...
	getUsersViewLens: (state, viewId) => {
		const user = core.coreSelectors.getCurrentUser(state)
	...
	setViewLens: (viewId, lensSlug) => {
		return function (dispatch, getState) {
			...
			sdk.card.update(user.id, {
				...
				.then((result) => {
					dispatch(store.actionCreators.setUser(result))
				})
				.catch((error) => {
					dispatch(store.actionCreators.addNotification('danger', error.message || error))
				})
...
exports.views = (state, action) => {
	...
	switch (action.type) {
		...
		case actions.UPSERT_VIEW_DATA_ITEM: {
			console.log('before', _.cloneDeep(state.viewData[action.value.id]))
			...
			console.log('after', _.cloneDeep(state.viewData[action.value.id]))
			return state
		}
