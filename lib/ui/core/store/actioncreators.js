...
export default class ActionCreator {
	...
	getActor (id) {
		return async (dispatch, getState) => {
			...
			// TODO: Check if the user is part of the balena org instead of this
			// heuristic
			if (actor.type === 'user' && actor.data.email !== 'new@change.me') {
				name = actor.slug.replace('user-', '')
				email = actor.data.email
			} else {
				proxy = true
				let handle = actor.data.handle
				if (!handle) {
					handle = actor.data.email === 'new@change.me' || !actor.data.email
