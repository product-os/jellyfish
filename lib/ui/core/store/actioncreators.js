...
const checkForNotification = (user, card) => {
	if (card.type !== 'whisper' && card.type !== 'message') {
		return false
	}
	...
	if (
		(message.includes(`@${user.slug.slice(5)}`) || message.includes(`!${user.slug.slice(5)}`)) &&
		!_.includes(card.data.readBy, user.slug)
	) {
		console.log('got notification', message, card)
		return true
	}
			...
			return Bluebird.props({
				...
				.then(({
					...
					stream.on('update', async (payload) => {
						...
						if (update.after && !update.before && checkForNotification(user, update.after)) {
							// Do stuff
						}
						if (update.after) {
