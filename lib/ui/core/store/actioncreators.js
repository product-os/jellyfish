...
export default class ActionCreator {
	...
	getLinks (card, targetType) {
		const verb = _.get(this.sdk.LINKS, [ card.type, targetType ], null)
		if (!verb) {
			throw new Error(`No link definition found between ${card.type} and ${targetType}`)
		}
	...
	createLink (fromCard, toCard, options = {}) {
		const verb = _.get(this.sdk.LINKS, [ fromCard.type, toCard.type ], null)
		if (!verb) {
