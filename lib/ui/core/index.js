...
load()
	.then(async () => {
		...
		if (token) {
			...
			await exports.store.dispatch(store.actionCreators.loginWithToken(token))
		} else {
			await exports.store.dispatch(store.actionCreators.setStatus('unauthorized'))
		}

		// Only try and load the URL route if the app is authorized
		if (store.selectors.getStatus(exports.store.getState()) === 'authorized') {
