...
const logger = (store) => {
	return (next) => {
		return (action) => {
			helpers.debug('DISPATCHING REDUX ACTION', action)
			// eslint-disable-next-line callback-return
			const result = next(action)
			helpers.debug('NEXT REDUX STATE', store.getState())
			return result
		}
	}
}
...
exports.createJellyfishStore = () => {
	return redux.createStore(reducerWrapper, redux.applyMiddleware(logger, reduxThunk.default))
}
