...
const getActorKey = async (jellyfish, session, actorId) => {
	...
	const key = await jellyfish.getCardBySlug(
		logger.context.systemContext, session, keySlug, {
			type: 'session'
		})
	...
	logger.info(logger.context.systemContext, 'Create worker key', {
		slug: keySlug,
		actor: actorId
	})

	return jellyfish.insertCard(
		logger.context.systemContext, session, jellyfish.defaults({
			slug: keySlug,
			version: '1.0.0',
			type: 'session',
			data: {
...
const tick = (worker, session, onError) => {
	...
	tickingWorker = worker.tick(null, session, {
...
exports.start = async (jellyfish, session, onRequest, onError) => {
	...
	const triggerStream = await jellyfish.stream(null, session, SCHEMA_ACTIVE_TRIGGERS)
	triggerStream.on('error', onError)

	let refreshingTriggers = null

	const refreshTriggers = async () => {
		refreshingTriggers = jellyfish.query(null, session, SCHEMA_ACTIVE_TRIGGERS)
		const triggers = await refreshingTriggers

		logger.info(logger.context.systemContext, 'Refreshing triggers', {
			triggers: triggers.length
		})

		worker.setTriggers(null, triggers.map((trigger) => {
...
exports.flush = async (jellyfish, session, worker) => {
	...
	logger.info(logger.context.systemContext, 'Flushing queue', {
		length
	})
