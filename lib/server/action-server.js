...
exports.start = async (jellyfish, session, onRequest, onError) => {
	...
	worker.stop = async () => {
		...
		// TODO: Investigate why there are still card insertions happening
		// after `worker.stop()` is called.
		// Even after the worker has stopped, there are sometimes "in-flight"
		// card insertions running that will throw errors if you disconnect
		// immediately. This delay band-aids the problem and lets the
		// "dust settle", allowing db queries to finish before disconnecting.
