...
exports.start = async (jellyfish, session, onRequest, onError) => {
	...
	worker.stop = async () => {
		...
		await exports.flush(worker, jellyfish, session)
...
exports.insertCard = async (instance, session, typeCard, card, options) => {
	return instance.insertCard(session, typeCard, options, card)
}

exports.enqueue = async (instance, session, params, context) => {
	return instance.enqueue(session, params, context)
}

exports.waitResults = async (instance, session, id) => {
	return instance.waitResults(session, id)
}

exports.stop = async (instance) => {
	return instance.stop()
}

exports.flush = async (instance, jellyfish, session) => {
	await tickingWorker
	await processingQueue

	if (await instance.length() === 0) {
		return
	}

	const request = await instance.dequeue()
	const result = await instance.execute(session, request)

	if (result.error) {
		const Constructor = instance.errors[result.data.type] ||
	...
	await exports.flush(instance, jellyfish, session)
}
