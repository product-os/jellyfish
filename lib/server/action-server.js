...
const processQueue = (context, jellyfish, worker, session, onRequest, onError) => {
	if (!run) {
		return
	}
	...
	processingQueue.then(() => {
		...
		setTimeout(() => {
			processQueue(context, jellyfish, worker, session, onRequest, onError)
		}, 10)
	})
}

exports.start = async (context, jellyfish, session, onRequest, onError) => {
	...
	worker.stop = async (workerContext) => {
		...
		await exports.flush(workerContext, jellyfish, session, worker)
...
exports.flush = async (context, jellyfish, session, worker) => {
	...
	await exports.flush(context, jellyfish, session, worker)
}
