...
exports.bindRoutes = (jellyfish, worker, app, socketServer) => {
	...
	app.all('/api/v2/hooks/:provider/:type*?', (request, response) => {
		...
		logger.info(request.ctx, `Webhook from ${hostname} for provider ${request.params.provider}`)

		if (!sync.isValidExternalEventRequest(hostname, request.params.provider)) {
