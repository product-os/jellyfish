...
exports.bindRoutes = (jellyfish, worker, queue, app, socketServer) => {
	...
	app.use(responseTime((request, response, time) => {
		logger.info(request.ctx, 'HTTP request end', {
			uri: request.originalUrl,
			time
		})
	}))
	...
	app.post('/api/v2/action', authorize, upload.any(), async (request, response) => {
		...
		return worker.enqueue(request.sessionToken, action).then((id) => {
			return queue.waitResults(ctx, id)
		}).then(async (results) => {
			...
			if (results.error) {
				errorReporter.reportException(ctx, errio.fromObject(results.data))
			}
			...
			errorReporter.reportException(ctx, error)
			return response.status(500).json({
				error: true,
	...
	app.get('/status', (request, response) => {
		const currentDate = new Date()
		return queue.length().then(async (length) => {
			const kernelStatus = await jellyfish.getStatus()
			return response.status(200).json({
				kernel: kernelStatus,
				workers: [
					{
	...
	*/
	app.get('/ping', (request, response) => {
		...
		return worker.enqueue(session, {
			...
			return queue.waitResults(request.ctx, id)
		}).then(async (results) => {
			const code = results.error ? 500 : 200
			return response.status(code).json(results)
		}).catch((error) => {
			...
			logger.error(request.ctx, 'Ping error', {
				error: errorObject
			})
