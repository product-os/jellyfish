...
exports.bindRoutes = (jellyfish, worker, app, socketServer) => {
	...
	app.all('/api/v2/hooks/:provider/:type*?', (request, response) => {
		...
		if (!integrationToken || !integrationToken.api) {
			return response.status(401).json({
				error: true,
				data: 'Webhook rejected as there is no token for this provider'
			})
		}

		// For development purposes
		if (process.env.NODE_ENV === 'production' &&
				!sync.isValidExternalEventRequest(integrationToken,
					request.params.provider, request.rawBody, request.headers)) {
