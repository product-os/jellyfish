...
exports.bindRoutes = (jellyfish, instance, app, socketServer) => {
	...
	app.post('/api/v2/action', authorize, upload.any(), async (request, response) => {
		...
		return actionServer.enqueue(instance, request.sessionToken, action, ctx).then((id) => {
			return actionServer.waitResults(instance, request.sessionToken, id)
		}).then(async (results) => {
