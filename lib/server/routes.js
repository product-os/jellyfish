...
exports.bindRoutes = (jellyfish, worker, app, socketServer) => {
	...
	const createContext = async (request, response, next) => {
		const ctx = context.newPrefixContext('REQUEST')
		logger.silly(ctx, 'Request started')
		const start = Date.now()

		request.ctx = ctx
		response.on('end', () => {
			const end = Date.now()
			logger.debug(ctx, `Request took: ${end - start} ms`)
		})
		return next()
	}
	...
	app.all('/api/v2/hooks/:provider/:type*?', (request, response) => {
		logger.info(request.ctx, `Webhook from ${request.hostname} for provider ${request.params.provider}`)

		if (!sync.isValidExternalEventRequest(request.hostname, request.params.provider)) {
			logger.error(request.ctx, 'Webhook rejected')
	...
	app.post('/api/v2/action', authorize, upload.any(), async (request, response) => {
		...
		logger.debug(ctx, `Got action ${JSON.stringify(action)}`)
		...
		return worker.enqueue(request.sessionToken, action, ctx).then((id) => {
			...
			if (results.error) {
				logger.error(ctx, `Result error! ${JSON.stringify(results.data)}`)
			}
			return response.status(code).json(results)
		}).catch((error) => {
			const currentDate = new Date()
			logger.error(ctx, 'Unexpected error!', error)
			return response.status(500).json({
				error: true,
