...
module.exports = (application, jellyfish, worker, queue) => {
	...
	application.get('/status', (request, response) => {
		return Bluebird.props({
			...
			logger.error(request.context, 'Status error', {
				error: errorObject
			})

			errorReporter.reportException(request.context, error)
	...
	application.get('/ping', (request, response) => {
		...
		return queue.enqueue(worker.getId(), session, {
			...
			return queue.waitResults(request.context, id)
		}).then(async (results) => {
			...
			logger.error(request.context, 'Ping error', {
				error: errorObject
			})
	...
	application.post('/api/v2/action', upload.any(), async (request, response) => {
		...
		return queue.enqueue(worker.getId(), request.sessionToken, action).then((id) => {
			return queue.waitResults(request.context, id)
		}).then(async (results) => {
			...
			if (results.error) {
				errorReporter.reportException(
					request.context, errio.fromObject(results.data))
			}
			...
			logger.error(request.context, 'HTTP unexpected error', {
				error: errorObject
			})
