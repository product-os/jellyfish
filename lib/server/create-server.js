...
exports.createServer = async (context, options) => {
	...
	const queue = new Queue(workerContext, jellyfish, jellyfish.sessions.admin)
	...
	const worker = await actionServer.start(
		...
		(error) => {
			...
			console.error(error)
			errorReporter.reportException(workerContext, error)
	...
	await actionServer.flush(workerContext, jellyfish, jellyfish.sessions.admin, worker, queue)

	bindRoutes(jellyfish, worker, queue, app, socketServer)
	...
	return {
		...
		close: async () => {
			...
			await worker.stop(workerContext)
