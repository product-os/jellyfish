...
exports.createServer = async (options) => {
	...
	const instance = new ActionServer(jellyfish, jellyfish.sessions.admin)
	...
	await Bluebird.each([
		...
		if (existingCard) {
			return ActionServer.enqueue(jellyfish.sessions.admin, {
	...
	return {
		...
		waitResults: async (session, id) => {
			return ActionServer.waitResults(session, id)
		},
		close: async () => {
