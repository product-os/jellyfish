...
const systemCtx = logger.context.systemContext
...
exports.createServer = async (options) => {
	...
	const jellyfish = await core.create(systemCtx, {
		backend: {
			host: config.dbHost,
			port: config.dbPort,
			user: config.dbUser,
	...
	await Bluebird.each([
		...
		const existingCard = await jellyfish.getCardBySlug(systemCtx, jellyfish.sessions.admin, card.slug, {
			type: card.type
		})

		if (deepEqual(_.omit(card, 'links'), _.omit(existingCard, [ 'id', 'links' ]))) {
			logger.info(systemCtx, 'Skipping unchanged default card', {
				slug: card.slug,
				type: card.type
			})
		...
		logger.info(systemCtx, 'Inserting default card', {
			slug: card.slug,
			type: card.type
		})

		return jellyfish.insertCard(systemCtx, jellyfish.sessions.admin, card, {
			override: true
		})
	})
	...
	const guestUserSession = await jellyfish.insertCard(systemCtx, jellyfish.sessions.admin, jellyfish.defaults({
		slug: 'session-guest',
		version: '1.0.0',
		type: 'session',
		data: {
	...
	logger.info(systemCtx, 'Done setting up guest session')
	...
	logger.info(systemCtx, 'Loading default cards using worker')
	...
	await Bluebird.each([
		...
		const typeCard = await jellyfish.getCardBySlug(systemCtx, jellyfish.sessions.admin, card.type, {
			type: 'type'
		})
		...
		if (deepEqual(_.omit(card, 'links'), _.omit(existingCard, [ 'id', 'links' ]))) {
			logger.info(systemCtx, 'Inserting default card using worker', {
				slug: card.slug,
				type: card.type
			})
	...
	logger.info(systemCtx, 'Flushing worker queue')
	...
	logger.info(systemCtx, 'HTTP app listening', {
	...
	return {
		...
		close: async () => {
			...
			await worker.stop()
			await jellyfish.disconnect(systemCtx)
		}
	}
}
