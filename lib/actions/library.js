...
const insertCard = async (jellyfish, worker, card, context, options) => {
	...
	// TODO: Don't create "create" events for action requests for now.
	// The proper solution is to use virtual roles to let users attach
	// events and modify action requests that belong to them.
	if (properties.type === 'action-request') {
		return id
	}

	if (options.upsert && hasCard) {
		await worker.executeAction(context.session, 'action-create-event', id, {
			type: 'update',
...
module.exports = {
	'action-create-session': async (jellyfish, worker, card, context, options) => {
		...
		return worker.executeAction(context.session, 'action-create-card', 'session', {
			properties: {
				data: {
					actor: card.id,
