...
module.exports = {
	...
	'action-create-todo-from-message': async (jellyfish, worker, card, context, options) => {
		const content = card.data.payload.message.replace(/#todo(\s+)?/g, '').trim()
		if (content.length === 0) {
			throw new Error(`Empty todo message: ${content}`)
		}

		const todoTypeCard = await jellyfish.getCardBySlug(context.session, 'todo')
		if (!todoTypeCard) {
			throw new Error('The todo type card does not exist')
		}

		return insertCard(jellyfish, worker, todoTypeCard, context, {
			properties: {
				slug: `todo-${card.id}`,
				data: {
					actor: context.actor,
