...
const insertCard = async (jellyfish, actions, card, context, options) => {
	...
	const hasCard = identifier && Boolean(await jellyfish.getCard(identifier, {
		inactive: true
	}))
	...
	const id = await jellyfish.insertCard(properties, {
		override: options.upsert
	})

	if (options.upsert && hasCard) {
		await actions.executeAction(jellyfish, 'action-create-event', id, {
			type: 'update',
...
module.exports = {
	'action-user-login': async (jellyfish, actions, card, context, options) => {
		if (options.hash !== card.data.password.hash) {
			throw new Error('Invalid password')
		}

		return actions.executeAction(jellyfish, 'action-create-card', 'session', {
			properties: {
				data: {
	...
	'action-update-card': async (jellyfish, actions, card, context, options) => {
		...
		const typeCard = await jellyfish.getCard(updatedCard.type)
