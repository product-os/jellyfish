...
module.exports = {
	...
	'action-create-card-from-tag': async (jellyfish, worker, card, context, options) => {
		const content = _.get(card, options.sourcePath).replace(new RegExp(`${options.tag}(\\s+)?`), '').trim()

		if (content.length === 0) {
			throw new Error(`Empty content: ${content}`)
		}

		const typeCard = await jellyfish.getCardBySlug(context.session, options.type)
		if (!typeCard) {
			throw new Error(`The ${options.type} type card does not exist`)
		}

		// The actor, origin and content values are avaialble as template variables
		const properties = objectTemplate.compile(options.properties, {
			actor: context.actor,
			origin: card,
			content
		}, {
