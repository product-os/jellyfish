...
module.exports = {
	'action-create-session': async (jellyfish, worker, card, context, options) => {
		...
		return worker.executeAction(context.session, 'action-create-card', sessionTypeCard.id, {
			properties: {
				data: {
					actor: card.id,
	...
	'action-create-event': async (jellyfish, worker, card, context, options) => {
		const sessionCard = await jellyfish.getCardById(context.session, context.session)

		if (!sessionCard) {
			throw new jellyfish.errors.JellyfishNoElement('Session does not exist')
		}
