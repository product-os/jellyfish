...
class ActionRequestWorker {
	...
	async executeAction (userSession, actionId, targetId, args) {
		...
		const cardSchema = this.jellyfish.getSchema(await this.jellyfish.getCardBySlug(this.session, 'card', queryOptions))
		...
		const actionSchema = this.jellyfish.getSchema(await this.jellyfish.getCardBySlug(this.session, 'action', queryOptions))
