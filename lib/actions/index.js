...
class ActionRequestWorker {
	...
	async watch () {
		const view = await this.jellyfish.getCard(this.session, 'view-non-executed-action-requests')
		const schema = await this.jellyfish.getSchema(view)
	...
	async createRequest (userSession, options) {
		const target = await this.jellyfish.getCard(userSession, options.targetId)

		if (!target) {
			throw new Error(`No such target: ${options.targetId}`)
		}

		const actor = await this.jellyfish.getCard(userSession, options.actorId, {
			writeMode: true
		})
		...
		const actionCard = await this.jellyfish.getCard(userSession, options.action, {
			writeMode: true
		})
		...
		return this.executeAction(userSession, 'action-create-card', 'action-request', _.omitBy({
			properties: {
	...
	async executeAction (userSession, actionId, targetId, args) {
		...
		const actionCard = await this.jellyfish.getCard(userSession, actionId)
		if (!actionCard) {
			throw new this.jellyfish.errors.JellyfishNoAction(`Unknown action: ${actionId}`)
		}

		const targetCard = await this.jellyfish.getCard(userSession, targetId)
		if (!targetCard) {
			throw new this.jellyfish.errors.JellyfishNoElement(`Unknown target: ${targetId}`)
		}

		const cardSchema = this.jellyfish.getSchema(await this.jellyfish.getCard(this.session, 'card', queryOptions))
		...
		const actionSchema = this.jellyfish.getSchema(await this.jellyfish.getCard(this.session, 'action', queryOptions))
...
module.exports.setup = async (jellyfish, session, options) => {
	...
	const sessionCard = await jellyfish.getCard(session, session, queryOptions)
	if (!sessionCard) {
	...
	if (!await jellyfish.getCard(session, `user-${options.username}`, queryOptions)) {
		...
		const signupRequest = await jellyfish.getCard(session, signupRequestId, queryOptions)
		await adminWorker.processRequest(signupRequest)
	}

	const actor = await jellyfish.getCard(session, `user-${options.username}`, queryOptions)
	if (!actor) {
	...
	const loginRequest = await jellyfish.getCard(session, loginRequestId, queryOptions)
	await adminWorker.processRequest(loginRequest)
	const completedLoginRequest = await jellyfish.getCard(session, loginRequestId, queryOptions)
