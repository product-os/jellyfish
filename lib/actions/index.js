...
class ActionRequestWorker {
	...
	async executeAction (userSession, actionId, targetId, args) {
		...
		const actionCard = await this.jellyfish.getCard(userSession, actionId, queryOptions)
		if (!actionCard) {
			throw new this.jellyfish.errors.JellyfishNoAction(`Unknown action: ${actionId}`)
		}

		const targetCard = await this.jellyfish.getCard(userSession, targetId, queryOptions)
		if (!targetCard) {
