...
class ActionRequestWorker {
	...
	async createRequest (userSession, options) {
		const target = await this.jellyfish.getCard(this.session, options.targetId)
		if (!target) {
	...
	async executeAction (userSession, actionId, targetId, args) {
		...
		const cardSchema = this.jellyfish.getSchema(await this.jellyfish.getCard(this.session, 'card'))
		...
		const actionSchema = this.jellyfish.getSchema(await this.jellyfish.getCard(this.session, 'action'))
		...
		return actionFunction(this.jellyfish, this, targetCard, {
			...
			actor: await this.jellyfish.getCard(this.session, 'user-admin'),
			timestamp: time.getCurrentTimestamp(),
