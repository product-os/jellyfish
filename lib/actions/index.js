...
class ActionRequestWorker {
	...
	async start (options = {}) {
		...
		this.orchestrator.on('request', (request) => {
			...
			return this.orchestrator.getActorSession(request.data.actor).then((session) => {
				return this.processRequest(session.id, request).then(() => {
	...
	async flushPendingRequests () {
		...
		for (const request of pendingRequests) {
			const session = await this.orchestrator.getActorSession(request.data.actor)
			await this.processRequest(session.id, request)
		}
