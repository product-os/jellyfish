...
exports.processRequest = async (jellyfish, request) => {
	...
	const results = await exports.executeAction(jellyfish, data.action, data.target, data.arguments)
	debug(`Action request executed: ${request.id}`)

	const id = await exports.executeAction(jellyfish, 'action-update-card', request.id, {
		properties: {
			data: {
				executed: true,
				result: {
