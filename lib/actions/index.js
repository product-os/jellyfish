...
const processRequest = (worker, session, request) => {
	...
	return Bluebird.resolve(worker.executeAction(session, {
		...
		return worker.executeAction(session, {
...
class ActionRequestWorker {
	...
	async flushPendingRequests () {
		...
		for (const request of pendingRequests) {
			const key = await getActorKey(this.jellyfish, this.session, request.data.actor)
			await processRequest(this, key.id, request)
		}
