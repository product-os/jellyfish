...
exports.processRequest = async (jellyfish, request) => {
	...
	return Bluebird.resolve(exports.executeAction(jellyfish, data.action, data.target, data.arguments)).then((results) => {
		debug(`Action request executed: ${request.id}`)

		return exports.executeAction(jellyfish, 'action-update-card', request.id, {
			properties: {
				data: {
					executed: true,
					result: {
	...
	}).catch(jellyfish.errors.JellyfishSchemaMismatch, (error) => {
		return exports.executeAction(jellyfish, 'action-update-card', request.id, {
			properties: {
				data: {
					executed: true,
...
exports.createRequest = async (jellyfish, options) => {
	const target = await jellyfish.getCard(options.targetId)
	if (!target) {
		throw new Error(`No such target: ${options.targetId}`)
	}

	const actor = await jellyfish.getCard(options.actorId)
	if (!actor) {
	...
	return exports.executeAction(jellyfish, 'action-create-card', 'action-request', _.omitBy({
		properties: {
 ...
 *
 * const id = await actions.executeAction(jellyfish, 'action-create-card', 'user', {
 *   properties: {
 *     slug: 'johndoe',
 *     data: {
 ...
 * })
 */
exports.executeAction = async (jellyfish, actionId, targetId, args) => {
	const actionCard = await jellyfish.getCard(actionId)
	if (!actionCard) {
		throw new jellyfish.errors.JellyfishNoAction(`Unknown action: ${actionId}`)
	}

	const targetCard = await jellyfish.getCard(targetId, {
	...
	const cardSchema = jellyfish.getSchema(await jellyfish.getCard('card'))
	...
	const actionSchema = jellyfish.getSchema(await jellyfish.getCard('action'))
	...
	if (superActionSlug) {
		...
		return exports.executeAction(jellyfish, superActionSlug, targetId, compiledArguments)
	}
	...
	return actionFunction(jellyfish, exports, targetCard, {
		...
		actor: await jellyfish.getCard('user-admin'),
		timestamp: time.getCurrentTimestamp()
	}, compiledArguments)
}
