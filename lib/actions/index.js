...
class ActionRequestWorker {
	constructor (jellyfish, session) {
		...
		this.pipeline = new Pipeline(jellyfish)
	}

	async start () {
		const emitter = new EventEmitter()
		const watcher = await this.watch()
		await this.pipeline.start(this.session)
		...
		watcher.on('request', (request) => {
			// TODO: Ugly hack to process actions with the user's session
			this.jellyfish.query(this.session, {
				type: 'object',
				properties: {
					id: {
						type: 'string'
					...
					data: {
						...
						properties: {
							actor: {
								type: 'string',
								const: request.data.actor
							}
						}
					}
				}
			}).then((results) => {
				return this.processRequest(results[0].id, request).then(() => {
	...
	async executeTriggers (session, card) {
		const viewSlug = 'view-active-triggered-actions'
		const view = await this.jellyfish.getCardBySlug(this.session, viewSlug)
		if (!view) {
			throw new Error(`Cannot execute triggers: no such card ${viewSlug}`)
		}

		const triggers = await this.jellyfish.query(this.session, view)
		...
		const requests = await Bluebird.map(triggers, (trigger) => {
			const compiledTrigger = objectTemplate.compile(trigger.data, {
				source: card
			}, {
				delimiters: [ '\\[', '\\]' ]
			})

			if (!this.jellyfish.matchesSchema(compiledTrigger.filter, expandedCard)) {
				return null
			}
