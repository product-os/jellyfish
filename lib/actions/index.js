...
class ActionRequestWorker {
	...
	async createRequest (userSession, options) {
		let target = await this.jellyfish.getCardById(userSession, options.targetId)

		if (!target) {
			target = await this.jellyfish.getCardBySlug(userSession, options.targetId)
		}
	...
	async executeAction (userSession, actionId, targetId, args) {
		...
		const cardCard = await this.jellyfish.getCardBySlug(this.session, 'card')
		...
		const actionTypeCard = await this.jellyfish.getCardBySlug(this.session, 'action')
...
module.exports.setup = async (jellyfish, session, options) => {
	...
	if (!await jellyfish.getCardBySlug(session, `user-${options.username}`, queryOptions)) {
		const signupRequestId = await adminWorker.createRequest(session, {
			...
			arguments: {
				...
				hash: `{{ HASH({ string: "${options.password}", salt: properties.data.arguments.username }) }}`
			}
		})
	...
	if (!_.includes(actor.data.roles, 'user-actions')) {
	...
	const loginRequestId = await adminWorker.createRequest(session, {
		...
		arguments: {
			password: {
				hash: `{{ HASH({ string: "${options.password}", salt: "${actor.slug}" }) }}`
			}
		}
	})
