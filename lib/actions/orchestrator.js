...
module.exports = class Orchestrator extends EventEmitter {
   ...
   * const orchestrator = new Orchestrator(...)
	...
	/**
	 * @summary Get the "actions" API key of an actor
	 * @function
	 * @private
	 *
	 ...
	 * @example
   * const orchestrator = new Orchestrator(...)
	 * const session = await orchestrator.getActorSession('4a962ad9-20b5-4dd8-a707-bf819593cc84')
	 * console.log(session.id)
	 */
	async getActorSession (actorId) {
		const sessionSlug = `session-action-${actorId}`
		const session = await this.jellyfish.getCardBySlug(this.session, sessionSlug, {
			type: 'session'
		})

		if (session && session.data.actor === actorId) {
			return session
		}
