...
const FUNCTIONS = Object.assign({
	GENERATESALT: () => {
		return credentials.generateSalt()
	},
	HASH: (string, salt) => {
		return credentials.hash(string, salt)
	}
}, formula)
...
const compile = (card, root, breadcrumb, onInterpolation) => {
	return deepMapValues(card, (value, key) => {
		...
		if (
			(root || card).interpolateValues ||
			_.get((root || card), [ 'properties', 'data', 'interpolateValues' ]) ||
			_.get((root || card), [ 'payload', 'data', 'interpolateValues' ])
		) {
