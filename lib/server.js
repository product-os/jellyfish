...
sdk.create({
	...
	return actions.listen(jellyfish, ACTION_SERVER_SESSION).then((actionServer) => {
		...
		app.post('/api/v1/signup', (request, response) => {
			...
			return utils.signup(jellyfish, jellyfish.sessions.guest.uuid, actions, {
		...
		app.post('/api/v1/login', (request, response) => {
			...
			return utils.login(jellyfish, jellyfish.sessions.guest.uuid, actions, {
				username: request.body.username,
				password: request.body.password
			}).then((data) => {
				response.status(200).json({
		...
		app.post('/api/v1/action', (request, response) => {
			...
			return utils.executeAndWaitAction(jellyfish, session, actions, {
