...
module.exports = Promise.resolve(sdk.create({
	tables: {
		cards: 'cards',
		requests: 'requests',
		sessions: 'sessions'
	...
	return Promise.each([
		...
		require('../default-cards/contrib/view-read-user-default.json'),
					...
					return Promise.try(() => {
						if (_.isString(input)) {
							...
							return jellyfish.getCardById(token, input)
								...
								.then((viewCard) => {
									return jellyfish.getViewSchema(viewCard)
								})
						}

						if (input.type === 'view') {
							return jellyfish.getViewSchema(input)
						}
				...
				app.post('/api/v1/action', authorize, (request, response) => {
					...
					const handler = utils.executeAndWaitAction(
						...
						worker, {
							...
							interpolateValues: request.body.interpolateValues
						}
					)
