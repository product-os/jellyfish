...
const createServer = () => {
	...
	return Promise.resolve(core.create({
		...
		// remaining cards will be inserted using the actions server itself
		return Promise.each([
		...
		return jellyfish.insertCard(jellyfish.sessions.admin, require('../default-cards/contrib/user-guest.json'), {
			...
			return ActionRequestWorker.setup(jellyfish, jellyfish.sessions.admin, {
				...
				return Promise.props({
					worker1: new ActionRequestWorker(jellyfish, actionsSession.id),
					worker2: actionServer.start(jellyfish, jellyfish.sessions.admin, _.noop,
						(error) => {
							throw error
						})
				})
				...
				return Promise.map([
					...
					return jellyfish.getCardBySlug(jellyfish.sessions.admin, card.type)
						.then((typeCard) => {
							debug(`Inserting ${card.slug}`)
							return workers.worker2.insertCard(
					...
					.then(() => {
						...
						return actionServer.flush(jellyfish, jellyfish.sessions.admin, workers.worker2)
					})
					.then(() => {
						return workers.worker1.start()
					})
					.then((actionEmitter) => {
						actionEmitter.on('error', (error) => {
							throw error
						})

						actionEmitter.on('request', (request) => {
						...
						const authorize = (request, response, next) => {
							...
							request.sessionToken = token || guestUserSession.id
						...
						const configHandler = (request, response) => {
							response.send({
								version,
								changelog
							})
						...
						app.post('/api/v1/query', authorize, queryHandler)
						app.post('/api/v2/query', authorize, queryHandler)
						...
						app.post('/api/v2/action', authorize, (request, response) => {
							...
							return workers.worker2.enqueue(request.sessionToken, request.body).then((id) => {
								return workers.worker2.waitResults(request.sessionToken, id)
							}).then((results) => {
