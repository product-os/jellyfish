...
const authorize = (request, response, next) => {
	...
	request.sessionToken = _.last(_.split(authorization, ' '))
	...
	return ActionRequestWorker.setup(jellyfish, jellyfish.sessions.admin, {
		...
		worker.start().then((actionServer) => {
			...
			const loadSchema = (input, token) => {
				return Promise.try(() => {
					if (_.isString(input)) {
						return jellyfish.getCard(token, input)
							.then((viewCard) => {
			 ...
			 * Accepts either a JSON schema, view id or view card as the `query` parameter.
			 */
			app.get('/api/v1/query', authorize, (request, response) => {
				...
				const query = utils.deserializeQuery(request.query)
