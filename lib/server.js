...
sdk.create({
	...
	return actions.listen(jellyfish, ACTION_SERVER_SESSION).then((actionServer) => {
		...
		app.post('/api/v1/signup', (request, response) => {
			...
			return actions.createRequest(jellyfish, jellyfish.sessions.guest.uuid, {
				targetId: 'user',
				actorId: 'user-admin',
				...
				return utils.waitForRequestResults(jellyfish, jellyfish.sessions.guest.uuid, id).then((results) => {
					response.status(200).json({
						error: false,
						data: {
							request: id,
		...
		app.post('/api/v1/login', (request, response) => {
			...
			return jellyfish.getCard(jellyfish.sessions.guest.uuid, `user-${request.body.username}`).then((user) => {
				if (!user) {
					return response.status(400).json({
						error: true,
		...
		app.post('/api/v1/action', (request, response) => {
			...
			return actions.createRequest(jellyfish, session, {
				...
				return utils.waitForRequestResults(jellyfish, session, id).then((results) => {
					response.status(200).json({
						error: false,
						data: {
							request: id,
