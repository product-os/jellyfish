...
sdk.create({
	...
	return actions.listen(jellyfish).then((actionServer) => {
		...
		app.get('/api/v1/query', (request, response) => {
			...
			return jellyfish.query(schema).then((results) => {
		...
		app.get('/api/v1/query-view/:id', (request, response) => {
			...
			return utils.queryView(jellyfish, id).then((results) => {
				response.status(200).json({
					error: false,
					data: results
				})
		...
		app.post('/api/v1/login', (request, response) => {
			...
			return jellyfish.getCard(`user-${request.body.username}`).then((user) => {
				if (!user) {
					return response.status(400).json({
						error: true,
