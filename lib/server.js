...
// TODO: Replace these dummy stubs with the real things
const ACTION_SERVER_SESSION = '4a962ad9-20b5-4dd8-a707-bf819593cc84'
const USER_SESSION = '4a962ad9-20b5-4dd8-a707-bf819593cc84'
...
sdk.create({
	...
	return actions.listen(jellyfish, ACTION_SERVER_SESSION).then((actionServer) => {
		...
		app.get('/api/v1/query', (request, response) => {
			...
			return jellyfish.query(USER_SESSION, schema).then((results) => {
		...
		app.get('/api/v1/query-view/:id', (request, response) => {
			...
			return utils.queryView(jellyfish, USER_SESSION, id).then((results) => {
				response.status(200).json({
					error: false,
					data: results
				})
		...
		app.post('/api/v1/signup', (request, response) => {
			...
			return actions.createRequest(jellyfish, USER_SESSION, {
				targetId: 'user',
				actorId: 'user-admin',
				...
				return utils.waitForRequestResults(jellyfish, USER_SESSION, id).then((results) => {
					response.status(200).json({
						error: false,
						data: {
							request: id,
		...
		app.post('/api/v1/login', (request, response) => {
			...
			return jellyfish.getCard(USER_SESSION, `user-${request.body.username}`).then((user) => {
				if (!user) {
					return response.status(400).json({
						error: true,
						data: `Invalid user: ${request.body.username}`
					})
				}
