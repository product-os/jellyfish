		...
		return jellyfish.insertCard(jellyfish.sessions.admin, require('../default-cards/contrib/user-guest.json'), {
			...
			return ActionRequestWorker.setup(jellyfish, jellyfish.sessions.admin, {
				...
				return Promise.props({
					...
					worker2: actionServer.start(jellyfish, jellyfish.sessions.admin, (request) => {
						console.log('server.js ONREQUEST', request)
					}, (error) => {
						throw error
					})
				})
			}).then((workers) => {
				return workers.worker1.start().then((actionEmitter) => {
