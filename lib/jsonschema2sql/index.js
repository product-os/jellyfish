...
const getFilterExpression = (operator, property, value, schema, options) => {
		...
		case 'items': {
			...
			if (_.isArray(value)) {
				...
				value.forEach((subschema, index) => {
					...
					const subexpression = parseFragment(subschema, options, subproperty)
						.join('\n')
			...
			const subexpression = parseFragment(value, options, [ [ 'value', true ] ])
				.join('\n')
			return [
				'(',
...
const parseFragment = (fragment, options, name = []) => {
	if (fragment.type === 'object' || _.has(fragment, [ 'properties' ])) {
		...
		if (!isExcludedKeyword(options, 'additionalProperties') &&
			...
			const filter = parseFragment(
				fragment.additionalProperties, options, [ [ 'value', true ] ])
	...
	for (const property of Object.keys(fragment)) {
		...
		if (property === 'not' && value !== false) {
			...
			const expression = getFilterExpression(property, name, value, fragment, options)
			if (expression) {
				result = getConjunction(result, expression)
			}
		}
