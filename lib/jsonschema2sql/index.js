...
const REGEXP_FORMAT_URI =
	'^((?:[a-z][a-z0-9+-.]*:)(?:\\/?\\/)?|(\\/\\/))[^\\s]*$'
const REGEXP_FORMAT_HOSTNAME =
	'^[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?(?:\\.[a-z0-9](?:[-0-9a-z]{0,61}[0-9a-z])?)*$'

// Optimized http://stackoverflow.com/questions/53497/regular-expression-that-matches-valid-ipv6-addresses
const REGEXP_FORMAT_IPV6 =
	'^\\s*(?:(?:(?:[0-9a-f]{1,4}:){7}(?:[0-9a-f]{1,4}|:))|(?:(?:[0-9a-f]{1,4}:){6}(?::[0-9a-f]{1,4}|(?:(?:25[0-5]|2[0-4]\\d|1\\d\\d|[1-9]?\\d)(?:\\.(?:25[0-5]|2[0-4]\\d|1\\d\\d|[1-9]?\\d)){3})|:))|(?:(?:[0-9a-f]{1,4}:){5}(?:(?:(?::[0-9a-f]{1,4}){1,2})|:(?:(?:25[0-5]|2[0-4]\\d|1\\d\\d|[1-9]?\\d)(?:\\.(?:25[0-5]|2[0-4]\\d|1\\d\\d|[1-9]?\\d)){3})|:))|(?:(?:[0-9a-f]{1,4}:){4}(?:(?:(?::[0-9a-f]{1,4}){1,3})|(?:(?::[0-9a-f]{1,4})?:(?:(?:25[0-5]|2[0-4]\\d|1\\d\\d|[1-9]?\\d)(?:\\.(?:25[0-5]|2[0-4]\\d|1\\d\\d|[1-9]?\\d)){3}))|:))|(?:(?:[0-9a-f]{1,4}:){3}(?:(?:(?::[0-9a-f]{1,4}){1,4})|(?:(?::[0-9a-f]{1,4}){0,2}:(?:(?:25[0-5]|2[0-4]\\d|1\\d\\d|[1-9]?\\d)(?:\\.(?:25[0-5]|2[0-4]\\d|1\\d\\d|[1-9]?\\d)){3}))|:))|(?:(?:[0-9a-f]{1,4}:){2}(?:(?:(?::[0-9a-f]{1,4}){1,5})|(?:(?::[0-9a-f]{1,4}){0,3}:(?:(?:25[0-5]|2[0-4]\\d|1\\d\\d|[1-9]?\\d)(?:\\.(?:25[0-5]|2[0-4]\\d|1\\d\\d|[1-9]?\\d)){3}))|:))|(?:(?:[0-9a-f]{1,4}:){1}(?:(?:(?::[0-9a-f]{1,4}){1,6})|(?:(?::[0-9a-f]{1,4}){0,4}:(?:(?:25[0-5]|2[0-4]\\d|1\\d\\d|[1-9]?\\d)(?:\\.(?:25[0-5]|2[0-4]\\d|1\\d\\d|[1-9]?\\d)){3}))|:))|(?::(?:(?:(?::[0-9a-f]{1,4}){1,7})|(?:(?::[0-9a-f]{1,4}){0,5}:(?:(?:25[0-5]|2[0-4]\\d|1\\d\\d|[1-9]?\\d)(?:\\.(?:25[0-5]|2[0-4]\\d|1\\d\\d|[1-9]?\\d)){3}))|:)))(?:%.+)?\\s*$' // eslint-disable-line max-len

// Optimized https://www.safaribooksonline.com/library/view/regular-expressions-cookbook/9780596802837/ch07s16.html
const REGEXP_FORMAT_IPV4 =
...
const getFilterExpression = (operator, property, value, schema) => {
	...
	switch (operator) {
		...
		case 'minimum': {
			const minOp = schema.exclusiveMinimum ? '>' : '>='
			row = getRowFromProperty(property, true)
			return `(jsonb_typeof(${row}) != 'number' OR (${row})::text::numeric ${minOp} ${format(value)})`
		}
		case 'maximum': {
			const maxOp = schema.exclusiveMaximum ? '<' : '<='
			row = getRowFromProperty(property, true)
			return `(jsonb_typeof(${row}) != 'number' OR (${row})::text::numeric ${maxOp} ${format(value)})`
		}
		case 'maxLength': {
			row = getRowFromProperty(property, true)
			const maxTargetRow = property.length === 1
				? getRootJSONB(property)
				: getRowFromProperty(property)
			return `(jsonb_typeof(${row}) != 'string' OR char_length((${maxTargetRow})::text) <= ${format(value)})`
		}
		case 'minLength': {
			row = getRowFromProperty(property, true)
			const minTargetRow = property.length === 1
				? getRootJSONB(property)
				: getRowFromProperty(property)
			return `(jsonb_typeof(${row}) != 'string' OR char_length((${minTargetRow})::text) >= ${format(value)})`
		}
		case 'enum': {
			row = getRowFromProperty(property, true)
		...
		case 'format': {
			...
			if (value === 'uuid') {
				return `${row} ~* '${REGEXP_FORMAT_UUID}'`
			}

			if (value === 'email') {
				return `${row} ~* '${REGEXP_FORMAT_EMAIL}'`
			}

			if (value === 'date-time') {
				return `${row} ~* '${REGEXP_FORMAT_DATE_TIME}'`
			}

			if (value === 'uri') {
				return `${row} ~* ${format(REGEXP_FORMAT_URI)}`
			}

			if (value === 'hostname') {
				return `${row} ~* ${format(REGEXP_FORMAT_HOSTNAME)}`
			}

			if (value === 'ipv6') {
				return `${row} ~* ${format(REGEXP_FORMAT_IPV6)}`
			}

			if (value === 'ipv4') {
				return `${row} ~* ${format(REGEXP_FORMAT_IPV4)}`
			}
...
const parseFragment = (fragment, options, name = []) => {
	if (fragment.type === 'object' || _.has(fragment, [ 'properties' ])) {
		...
		if (fragment.properties) {
			for (const property of Object.keys(fragment.properties)) {
				...
				if (value.not && _.isEmpty(value.not)) {
					...
					const expression = parseFragment(value, options, path)
					if (expression && expression.length) {
						result = getConjunction(result, expression)
					}
				} else {
					const parsed = parseFragment(value, options, path)
		...
		if (fragment.not) {
			...
			const expression = _.flattenDeep([
				'NOT',
				'(',
				parseFragment(fragment.not, options, name),
				')'
			])
			result = getConjunction(result, expression)
		}
		...
		if (fragment.allOf) {
			const subresult = fragment.allOf.map((conjunct) => {
				...
				const exprs = parseFragment(copy, options, name)
		...
		if (
			...
			result = getConjunction(
				result,
				`ARRAY[${keys.map(format).join(', ')}] @> ARRAY(SELECT jsonb_object_keys(${row}))`
			)
		}
		...
		if (property === 'not' && value !== false) {
			...
			if (
				...
				const operatorResult = value.map((disjunct) => {
					return `(${parseFragment(disjunct, options, name).join('\n')})`
				})
			...
			const subexpression = _.map(value, (subSchema, pattern) => {
				...
				const conjunct = parseFragment(subSchema, options, [ [ 'value', true ] ])
			...
			const expression = getFilterExpression(property, name, value, fragment)
			if (expression) {
				result = getConjunction(result, expression)
			}
		}
...
module.exports = (table, schema, options = {}) => {
	...
	const filter = parseFragment(schema, options, [ [ ROOT, true ] ])
