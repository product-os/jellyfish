...
module.exports = {
	format: {
		...
		uritemplate: '^(?:(?:[^\\x00-\\x20"\\\'<>%\\^`{|}]|%[0-9a-f]{2})|\\{[+#./;?&=,!@|]?(?:[a-z0-9_]|%[0-9a-f]{2})+(?::[1-9][0-9]{0,3}|\\*)?(?:,(?:[a-z0-9_]|%[0-9a-f]{2})+(?::[1-9][0-9]{0,3}|\\*)?)*\\})*$',
		urireference: '^(?:(?:[a-z][a-z0-9+-.]*:)?\\/?\\/)?(?:[^\\\\s#][^\\s#]*)?(?:#[^\\\\s]*)?$',
		jsonpointer: '^(?:/(?:[^~/]|~0|~1)*)*$',
		email: '^\\S+@\\S+\\.\\S+$',
		// eslint-disable-next-line max-len
		datetime: '^\\d\\d\\d\\d-[0-1]\\d-[0-3]\\d[t\\s](?:[0-2]\\d:[0-5]\\d:[0-5]\\d|23:59:60)(?:\\.\\d+)?(?:z|[+-]\\d\\d:\\d\\d)$',
