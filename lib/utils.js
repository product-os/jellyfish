...
exports.waitForRequestResults = async (jellyfish, session, requestId) => {
	const request = await jellyfish.getCard(session, requestId)
	if (!request) {
		return null
	}
...
exports.getTimeline = async (jellyfish, session, id, options) => {
	const card = await jellyfish.getCard(session, id, options)
	if (!card) {
...
exports.queryView = async (jellyfish, session, viewId) => {
	const viewCard = await jellyfish.getCard(session, viewId)
	if (!viewCard || viewCard.type !== 'view') {
		throw new jellyfish.errors.JellyfishNoView(`Unknown view: ${viewId}`)
	}

	return jellyfish.query(session, jellyfish.getSchema(viewCard))
}

exports.executeAndWaitAction = async (jellyfish, session, actions, options) => {
	// TODO: We're requesting the actor that corresponds to the session using
	// the admin user. We should be able to do this using the user itself
	// once we have support for virtual roles (ie I can access my own session)
	const sessionCard = await jellyfish.getCard(jellyfish.sessions.admin.uuid, session)
	if (!sessionCard) {
		throw new jellyfish.errors.JellyfishNoElement('Invalid session')
	}

	options.actorId = sessionCard.id

	const id = await actions.createRequest(jellyfish, session, options)
...
exports.login = async (jellyfish, session, actions, options) => {
	...
	return exports.executeAndWaitAction(jellyfish, session, actions, {
		targetId: actor.id,
...
exports.signup = async (jellyfish, session, actions, options) => {
	return exports.executeAndWaitAction(jellyfish, session, actions, {
		targetId: 'user',
