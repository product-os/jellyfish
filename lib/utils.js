...
exports.executeAndWaitAction = async (jellyfish, session, worker, options) => {
	...
	const actor = await jellyfish.getSessionUser(session)
...
exports.login = async (jellyfish, session, worker, options) => {
	const user = await jellyfish.getCard(jellyfish.sessions.admin, `user-${options.username}`)
	if (!user) {
...
exports.signup = async (jellyfish, session, worker, options) => {
	return exports.executeAndWaitAction(jellyfish, session, worker, {
		targetId: 'user',
