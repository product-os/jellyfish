...
exports.login = async (jellyfish, session, worker, options) => {
	...
	return exports.executeAndWaitAction(jellyfish, session, worker, {
		...
		arguments: {
			password: {
				hash: `{{ HASH(properties.transient.password, '${slug}') }}`
			}
		}
	})
}
...
/**
 * @summary Given a schema, generates an array of paths to values that are
 * formatted as UUIDs
 * @function
 * @public
 *
 * @param {*} schema - A JSON schema
 * @param {String[]} [pathPieces=[]] - current search path as an array of keys
 * @param {Array.<String[]>} [paths=[]] - An array of paths to uuid fields
 * as arrays of keys
 *
 * @returns {Array.<String[]>} - An array of paths to the UUID values, each one
 * represented as an array of keys
 *
 * @example
 * const schema = {
 * 	type: 'object',
 ...
 * }
 *
 * const paths = findUuidPaths(schema)
 *
 * console.log(paths)
 * // --> [ [ 'id' ], [ 'data', 'actor' ] ]
 *
 */
exports.findUuidPaths = (schema, pathPieces = [], paths = []) => {
	let newPaths = paths
	if (schema.type === 'object') {
		_.forEach(schema.properties, (subSchema, key) => {
			newPaths = exports.findUuidPaths(subSchema, pathPieces.concat(key), newPaths)
		})
	}
	if (schema.format === 'uuid') {
		newPaths.push(pathPieces)
	}
