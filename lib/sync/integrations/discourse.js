...
const getEventCard = (mirrorId, data, options) => {
	 ...
	 */
	const dom = cheerio.load(data.cooked)
	dom('.lightbox .meta').remove()
	const html = dom.html()

	const message = utils.parseHTML(html, {
		 ...
		 */
		.replace(/https:\/\/discourse-cdn-(\w+)\.com\/business\d/gi, options.baseUrl)
...
module.exports = class DiscourseIntegration {
	...
	async translate (event) {
		...
		const posts = topicData.post_stream
			? await Bluebird.map(topicData.post_stream.posts, async (post) => {
			...
			: []
		if (isPostEvent(event) && !_.find(posts, {
