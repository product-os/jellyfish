...
const processTopicData = async (sequence, mirrorId, topic, options) => {
	...
	if (!_.isEqual(threadCard, newThreadCard)) {
		const card = _.cloneDeep(newThreadCard)
		card.data.translateDate = topicUpsertDate.toISOString()
...
module.exports = class DiscourseIntegration {
	...
	async translate (event) {
		...
		for (const post of posts) {
			...
			if (!eventCard) {
				postCard.data.translateDate = post.created_at
			...
			if (!_.isEqual(eventCard, patchedCard)) {
				patchedCard.data.translateDate = post.updated_at
				sequence.push({
