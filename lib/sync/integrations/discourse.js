...
const httpDiscourse = async (context, integrationOptions, method, url, options, retries = 15) => {
	...
	if (result.code >= 500) {
		...
		throw new integrationOptions.errors.SyncExternalRequestError(
...
module.exports = class DiscourseIntegration {
	...
	async translate (event) {
		...
		const eventActorId = await this.context.getActorId(eventActor.slug)
		...
		const topicActorId = await this.context.getActorId(topicActor.slug)
		...
		const posts = remoteTopicData.post_stream
			? await Bluebird.map(remoteTopicData.post_stream.posts, async (post) => {
				...
				post.actorId = await this.context.getActorId(postActor.slug)
