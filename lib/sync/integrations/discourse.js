...
const getDiscourseTopicById = async (context, actor, options, baseUrl, id) => {
	return getDiscourseResource(
		context, actor, Object.assign({}, options, {
			query: {
...
const isEvent = (object) => {
	return object.type === 'external-event@1.0.0' &&
		object.data &&
		object.data.headers &&
		object.data.payload
}
...
module.exports = class DiscourseIntegration {
	...
	async mirror (card, options) {
		...
		if (card.type === 'support-thread@1.0.0' && discourseUrl) {
		...
		if (card.type === 'message@1.0.0' || card.type === 'whisper@1.0.0') {
			...
			if (!thread || thread.type !== 'support-thread@1.0.0' || !thread.active) {
				return []
			}
			...
			const response = await httpDiscourse(
				...
				`${this.baseUrl}/posts.json`, {
					...
					body: {
						...
						whisper: card.type === 'whisper@1.0.0' ? 'true' : 'false'
					}
				})
