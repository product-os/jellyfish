...
const getActor = async (context, actor, options, baseUrl, payload) => {
	...
	const nameWords = remoteUser.name.split(' ')
	...
	async mirror (card, options) {
		...
		if (card.type === 'message' || card.type === 'whisper') {
			...
			assert.USER(null,
				...
				[
					`The Discourse user ${username} does not have enough permissions`,
					'to post or see whispers. Please ask the ops team to make you a',
					'moderator in Discourse and try again.'
				].join(' '))
