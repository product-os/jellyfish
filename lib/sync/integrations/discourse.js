...
const httpDiscourse = async (context, token, method, url, options = {}) => {
	return new Bluebird((resolve, reject) => {
		...
		request({
			...
			qs: {
				api_key: token.api,
				api_username: options.username || token.username
			}
		}, (error, response, body) => {
			if (error) {
				return reject(error)
			...
			if (response.statusCode >= 500) {
				...
				return reject(new Error(
					`Discourse API error ${response.statusCode}: ${description}`))
			}
...
const getDiscourseResource = async (context, token, baseUrl, path) => {
	const result = await httpDiscourse(context, token, 'GET', path, {
		baseUrl
	})
	...
	if (result.code !== 200) {
		throw new Error(
			`Couldn't get resource ${path}: ${JSON.stringify(result.body, null, 2)}`)
	}
 ...
 */
const getDiscourseUserById = async (context, token, baseUrl, id) => {
	return getDiscourseResource(
		context, token, baseUrl, `/admin/users/${id}.json`)
}
 ...
 */
const getDiscourseTopicById = async (context, token, baseUrl, id) => {
	return getDiscourseResource(
		context, token, baseUrl, `/t/${id}.json`)
}
 ...
 */
const getActor = async (context, token, baseUrl, payload) => {
	if (payload.post) {
		return {
...
const getRemoteTopicData = async (context, token, event) => {
	...
	const data = await getDiscourseTopicById(
		context, token, getBaseUrl(event), topicId)
	if (!data) {
		throw new Error(
...
module.exports = class DiscourseIntegration {
	...
	async translate (event) {
		...
		const eventActor = await getActor(
			this.context, this.options.token, baseUrl, event.data.payload)
		...
		const remoteTopicData = await getRemoteTopicData(
			this.context, this.options.token, event)
		const topicActor = isTopicEvent(event)
			? eventActor
			: await getActor(this.context, this.options.token, baseUrl, remoteTopicData)
		...
		const posts = remoteTopicData.post_stream
			? await Bluebird.map(remoteTopicData.post_stream.posts, async (post) => {
				...
				const postActor = await getActor(
					this.context, this.options.token, baseUrl, {
						post
					})
