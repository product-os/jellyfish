...
const getProspectByEmail = async (context, actor, errors, baseUrl, emails) => {
	...
	for (const email of _.castArray(emails)) {
		...
		assert.INTERNAL(null, searchResult.code === 200 || searchResult.code === 404,
...
const upsertProspect = async (context, actor, errors, baseUrl, card, retries = 5) => {
	...
	const outreachUrl = _.get(prospect, [ 'links', 'self' ]) ||
...
module.exports = class OutreachIntegration {
	...
	async translate (event) {
		...
		const eventCard =
			await this.context.getElementByMirrorId('email-sequence@1.0.0', url)
 ...
 */
module.exports.whoami = async (credentials, context, options) => {
	const user = await context.getElementBySlug(options.slug)

	assert.INTERNAL(context, user,
		options.errors.SyncNoMatchingUser,
		`Failed to find user with slug: ${options.slug}`)

	return {
		slug: `${user.slug}@${user.version}`,
		emails: []
	}
}
