...
module.exports = class OutreachIntegration {
	...
	async translate (event, options) {
		...
		const actorResult = await this.context.request(options.actor, {
			method: 'GET',
			json: true,
			uri: `${this.baseUrl}/api/v2/users/${data.relationships.creator.data.id}`
		}).catch((error) => {
			if (error.expected && error.name === 'SyncOAuthNoUserError') {
				return null
			}
		...
		const actorId = await this.context.getActorId({
			active: true,
			handle: actorResult.data.attributes.username,
			name: actorResult.data.attributes.name,
			email: actorResult.data.attributes.email,
			title: actorResult.data.attributes.title
		})

		const sequenceCard = getSequenceCard(data.links.self, data.attributes, {
			id: data.id,
			orgId
		})
		...
		return [
			{
				time: new Date(data.attributes.createdAt),
				actor: actorId,
				card: sequenceCard
			}
		]
