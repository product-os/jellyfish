...
module.exports = class FrontIntegration {
	...
	async translate (event) {
		...
		const remoteComments = await this.front.conversation.listComments({
			conversation_id: _.last(threadCard.data.mirrors[0].split('/')),
			limit: 100
		})

		for (const root of remoteComments._results) {
			const comment = await getMessage(this.context, cards, root, this.options.actor, threadCard.id)
