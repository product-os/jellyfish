...
const logger = require('../../logger').getLogger('sync:front')
...
const getMessageFromEvent = (context, event, options) => {
	...
	if (message) {
		logger.debug(ctx, 'Inserting message', {
			data: message.data
		})
...
module.exports = class FrontIntegration {
	...
	async mirror (card) {
		...
		if (card.type === 'support-thread' && frontUrl) {
			...
			if (conversation.subject.replace(/^Re:\s/, '') !== card.name ||
				...
				logger.debug(ctx, 'Updating front thread', {
					conversation: id,
					status: newStatus,
					tags: card.tags
				})
		...
		if (card.type === 'message' || card.type === 'whisper') {
			...
			if (card.type === 'whisper') {
				...
				logger.debug(ctx, 'Creating front whisper', {
					conversation,
					author: author.id,
					body: message
				})
			...
			if (card.type === 'message') {
				...
				logger.debug(ctx, 'Creating front message', {
					conversation,
					author: author.id,
					body: message
				})
	...
	async recreateThreadFromEvent (event) {
		...
		logger.debug(ctx, 'Inserting thread', {
			data
		})
