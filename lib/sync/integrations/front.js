...
module.exports = class FrontIntegration {
	...
	async translate (event) {
		...
		for (const remoteMessage of remoteMessages) {
			const comment = await getMessage(
				this.context, cards, remoteMessage, actor, threadCard.id)
	...
	async getLocalUser (event) {
		...
		if (event.data.payload.target && event.data.payload.target.data.author) {
			return this.context.getActorId(
				'user', event.data.payload.target.data.author.username)
		}
		...
		if (utils.isEmail(from.handle)) {
			return this.context.getActorId('account', from.handle)
		}
		...
		if (!from._links.related.contact) {
			return this.context.getActorId('account', `${from.handle}@intercom.io`)
		}
		...
		if (utils.isEmail(contact.name)) {
			return this.context.getActorId('account', contact.email)
		}
		...
		return this.context.getActorId('account', `${name}@intercom.io`)
	}
}
