...
const getConversationLastMessage = async (
	context, front, sequence, event, targetCard) => {
...
module.exports = class FrontIntegration {
	...
	async getLocalUser (event) {
		...
		if (event.data.target &&
			...
			event.data.target.data.type === 'custom' &&
			event.data.target.data.recipients.length > 1) {
			/*
			 * In forum messages the "from" is really "to" (?)
			 */
			const from = _.find(event.data.target.data.recipients, {
				role: 'to'
			})
			 ...
			 */
			if (from) {
				event.data.target.data.recipients = [ from ]
				event.data.target.data.recipients[0].role = 'from'
			}
		}
	...
	async getThreadActor (event) {
		if (event.data.payload.conversation &&
			...
			event.data.payload.conversation.recipient.role === 'to') {
			// eslint-disable-next-line no-underscore-dangle
		...
		if (event.data.target &&
			// eslint-disable-next-line no-underscore-dangle
			event.data.target._meta &&
			// eslint-disable-next-line no-underscore-dangle
			event.data.target._meta.type === 'message' &&
			event.data.target.data &&
			!event.data.target.data.is_inbound) {
			const target = _.find(event.data.target.data.recipients, {
				role: 'to'
			})
