...
module.exports = class FrontIntegration {
	...
	async getFile (file) {
		...
		/*
		 * We had cases where Front responds with an error
		 * status code (504), but still returns the buffer
		 * that corresponds to the attachment (?)
		 *
		 * We add some error handling so that if the status
		 * code is not success but we still see the body,
		 * then we proceed as if nothing happened.
		 */
		let response = null
		try {
