...
module.exports = class FrontIntegration {
	...
	async translate (event) {
		...
		if (event.data.payload.type === 'reminder') {
			...
			if (event.data.payload.conversation && event.data.payload.conversation.last_message) {
				// eslint-disable-next-line no-underscore-dangle
				const commentMirrorId = event.data.payload.conversation.last_message._links.self
	...
	async recreateThreadFromEvent (event) {
		...
		if (event.data.payload.conversation.status === 'archived') {
			cards.push({
				time: new Date(getEpochDate(event.data.payload.conversation.created_at + 1)),
				card: updateCardFromSequence(cards, 0, {
					data: {
						status: 'closed'
					}
				})
