 ...
 * @private
 */
const ALL_THREAD_TYPES = [ 'sales-thread', 'support-thread' ]
...
const getMessageActor = async (context, front, intercom, payload) => {
	...
	if (payload.author) {
		return context.getActorId({
			handle: payload.author.username,
	...
	if (from && payload.type === 'intercom') {
		const intercomUser = await getIntercomUser(intercom,
			from.handle)
		if (intercomUser) {
			return context.getActorId({
				handle: intercomUser.user_id,
				email: intercomUser.email,
				title: intercomUser.custom_attributes['Account Type'],
				company: intercomUser.custom_attributes.Company,
				country: intercomUser.location_data.country_name,
				city: intercomUser.location_data.city_name,
				name: {
					first: intercomUser.custom_attributes['First Name'],
					last: intercomUser.custom_attributes['Last Name']
				}
			})
		}
	}
 ...
 */
const getMessage = async (instance, front, intercom, sequence, payload, threadId, emittedDate) => {
...
const getConversationLastMessage = async (
	instance, front, intercom, sequence, event, targetCard) => {
 ...
 * @returns {Array} new sequence upserts
 */
const getEventMessage = async (instance, front, intercom, sequence, event, targetCard) => {
 ...
 * @returns {Object} user
 */
const getIntercomUser = async (intercom, id, retries = 10) => {
	return new Bluebird((resolve, reject) => {
		...
		if (error.statusCode === 503 || error.statusCode === 500) {
			return Bluebird.delay(2000).then(() => {
				return getIntercomUser(intercom, id, retries - 1)
			})
		}
...
module.exports = class FrontIntegration {
	...
	async translate (event) {
		...
		const eventMessage = await getEventMessage(
			this, this.front, this.intercom, cards, event, threadCard)
			...
			if (event.data.payload.conversation.recipient._links &&
				...
				if (contactUrl) {
					...
					if (contact) {
						...
						if (intercomData) {
							const intercomUser = await getIntercomUser(this.intercom,
								intercomData.handle)
							if (intercomUser) {
								return this.context.getActorId({
									handle: intercomUser.user_id,
