...
const getEventInbox = async (context, front, event) => {
	...
	if (event.data.payload.source._meta.type === 'inboxes') {
		...
		const publicInboxes = _.map(_.filter(event.data.payload.source.data, {
			is_private: false
		}), 'name')
 ...
 */
const findValidInbox = (inboxes) => {
	return _.find(inboxes, (inbox) => {
...
const getThreadWhispers = async (front, context, conversationId) => {
	return frontPaginate(context, (options) => {
		context.log.info('Front API request', {
