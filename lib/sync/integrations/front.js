...
const getMessage = (context, payload, options) => {
	...
	return {
		time: date,
		card: {
			slug: context.getEventSlug(options.type),
			type: options.type,
			version: '1.0.0',
			active: true,
			tags: [],
...
const getMessageFromEvent = (context, event, options) => {
	...
	if (message) {
		logger.debug(logger.context.systemContext, 'Inserting message', {
			data: message.data
		})
...
module.exports = class FrontIntegration {
	...
	async mirror (card) {
		...
		logger.debug(logger.context.systemContext, 'Mirroring', {
			type: card.type
		})
		...
		if (card.type === 'support-thread' && frontUrl) {
			...
			if (conversation.subject.replace(/^Re:\s/, '') !== card.name ||
				...
				logger.debug(logger.context.systemContext, 'Updating front thread', {
					conversation: id,
					status: newStatus,
					tags: card.tags
				})
		...
		if (card.type === 'message' || card.type === 'whisper') {
			const thread = await this.context.getCardById(
			...
			const actor = await this.context.getCardById(
			...
			if (card.type === 'whisper') {
				...
				logger.debug(logger.context.systemContext, 'Creating front whisper', {
					conversation,
					author: author.id,
					body: message
				})
			...
			if (card.type === 'message') {
				...
				logger.debug(logger.context.systemContext, 'Creating front message', {
					conversation,
					author: author.id,
					body: message
				})
	...
	async translate (event) {
		logger.debug(logger.context.systemContext, 'Translating', {
	...
	async recreateThreadFromEvent (event) {
		...
		logger.debug(logger.context.systemContext, 'Inserting thread', {
			data
		})
		...
		if (message.length > 0 && event.data.payload.type !== 'comment') {
			cards.push(...message)
		}
