 ...
 */
const getMessage = async (context, payload, actorId, threadId) => {
...
const getConversationLastMessage = async (context, sequence, event, targetCard, options) => {
	...
	const message = await getMessage(context, root, options.actor, targetCard.id)
...
module.exports = class FrontIntegration {
	...
	async translate (event) {
		if ([ 'comment', 'inbound', 'outbound', 'out_reply' ].includes(event.data.payload.type)) {
			const thread = await this.context.getElementByMirrorId('support-thread', getConversationMirrorId(event))
			if (!thread) {
				return this.recreateThreadFromEvent(event)
			}

			return getEventMessage(this.context, [], event, thread, {
		...
		if (event.data.payload.type === 'reminder') {
			const thread = await this.context.getElementByMirrorId('support-thread', getConversationMirrorId(event))
			if (!thread) {
				return this.recreateThreadFromEvent(event)
			}

			return getConversationLastMessage(this.context, [], event, thread, {
				actor: this.options.actor
			})
		}
		...
		if (!thread) {
			...
			cards.push({
				time: utils.getDateFromEpoch(event.data.payload.emitted_at),
				card: utils.patchObject(cards[0].card, utils.patchObject(delta, {
					id: {
						$eval: 'cards[0].id'
					}
				}))
		...
		return [
			{
				time: utils.getDateFromEpoch(event.data.payload.emitted_at),
				card: utils.patchObject(thread, delta)
			}
		]
	}
	...
	async recreateThreadFromEvent (event) {
		...
		const thread = await this.context.getElementByMirrorId('support-thread', mirrorId)
		const inbox = await getEventInbox(this.front, event)
		...
		const head = thread || cards[0].card
		const id = head.id || {
			$eval: 'cards[0].id'
		}

		if (event.data.payload.conversation.status === 'archived' && head.data.status !== 'closed') {
			cards.push({
				time: new Date(utils.getDateFromEpoch(event.data.payload.conversation.created_at + 1)),
				card: utils.patchObject(head, {
					data: {
						status: 'closed'
					}
				})
		...
		if (event.data.payload.target) {
			const eventMessages = await getEventMessage(this.context, [], event, utils.patchObject(head, {
				id
			}), {
				actor: this.options.actor
			})

			message1.push(...eventMessages)
		}

		const message2 = await getConversationLastMessage(this.context, message1, event, utils.patchObject(head, {
			id
		}), {
			actor: this.options.actor
		})
