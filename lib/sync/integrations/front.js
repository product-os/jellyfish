...
const getMessageText = (payload) => {
	 ...
	 */
	return _.trim(payload.body ? htmlToText.fromString(
		payload.body.replace(/<br>\n/g, '<br><br>'), {
 ...
 */
const frontPaginate = async (context, fn, args, page = 1) => {
	const limit = 100
	const response = await handleRateLimit(context, () => {
	...
	const next = await frontPaginate(context, fn, args, page + 1)
