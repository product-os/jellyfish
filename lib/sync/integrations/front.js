...
module.exports = class FrontIntegration {
	...
	async mirror (card) {
		...
		if (card.type === 'message' || card.type === 'whisper') {
			const thread = await this.context.getElementById(
				this.options.session, 'support-thread', card.data.target)
			...
			const actor = await this.context.getElementById(
				this.options.session, 'user', this.options.actor)
	...
	async getSupportThreadByMirrorId (id) {
		return this.context.getElementByMirrorId(this.options.session, 'support-thread', id)
	}

	async getCommentByMirrorId (id) {
		return this.context.getElementByMirrorId(this.options.session, 'message', id)
	}

	async getWhisperByMirrorId (id) {
		return this.context.getElementByMirrorId(this.options.session, 'whisper', id)
	}
...
module.exports.hosts = [ /frontapp\.com$/ ]
