...
const setCard = (payload, preExistingCard, context) => {
	...
	if (payload.targetCard === 'user') {
		...
		card.slug = getUserCardSlug(payload.targetCard, payload.payload.username, false)
		card.data = {
...
const mergedCardWithPayload = (card, payload) => {
	...
	card.data.mirrors = (card.data.mirrors || []).filter((mirror) => {
		console.log(mirror, !mirror.startsWith(baseUrl(payload.source)))

		return !mirror.startsWith(baseUrl(payload.source))
	})
...
module.exports = class BalenaAPIIntegration {
	...
	async translate (event) {
		...
		const payload = await decryptPayload(this.options.token, event.data.payload)

		// 2. Ignore empty or useless payloads
		this.context.log.info('Balena-API Translate: 2. Ignore empty or useless payloads')

		// Return early when payload is empty
		if (!payload) return []

		// Log translate payload
		this.context.log.info('Balena-API Translate: Translating Balena API payload', {
			payload
		})
		...
		if (payload.type === 'create') {
			...
			if (userCreateContainsUsername(payload)) {
				return []
			}
		}
		...
		// 4. Initialize base card object
		this.context.log.info('Balena-API Translate: 4. Initialize base card object')
		...
		// 6. Set Actor of translate event
		this.context.log.info('Balena-API Translate: 6. Set Actor of translate event')
		...
		// 7. build up translate sequence
		this.context.log.info('Balena-API Translate: 7. build up translate sequence')
		const sequence = buildSquence(mergedCard, payload, actor)

		this.context.log.info('Balena-API Translate: Returning Balena API updates', {
			sequence
		})
