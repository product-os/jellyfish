...
const getPreExistingCard = async (context, payload, resource, cardType) => {
	// Get the pre-existing card by mirror id
	const mirrorId = getMirrorId(payload.source, payload.payload.id, resource)
...
const mergeCardWithPayload = (preExistingCard, payload, resource, cardType) => {
	...
	if (cardType === 'account@1.0.0') {
		...
		console.log('sluggggs', card.slug)
		...
		// Const subscription = payload.payload.subscription[0]
		// const plan = payload.payload.plan[0]
...
module.exports = class BalenaAPIIntegration {
	...
	async translate (event) {
		...
		const preExistingCard = await getPreExistingCard(this.context, payload, resource, cardType)

		console.log('subs 04', JSON.stringify(preExistingCard, null, 4))

		// 5. Merge card with payload
		this.context.log.info('Balena-API Translate: Merge card with payload')

		const newCard = mergeCardWithPayload(preExistingCard, payload, resource, cardType)
		...
		console.log('subs 05', JSON.stringify(sequence, null, 4))
