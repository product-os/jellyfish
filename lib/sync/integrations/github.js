...
const getEventRoot = (event) => {
	return event.data.payload.comment || event.data.payload.issue || event.data.payload.pull_request
}
...
module.exports = class GitHubIntegration {
	...
	async mirror (card, options) {
		...
		if ((card.type === 'issue' || card.type === 'pull-request') && card.data.repository) {
			...
			const getOptions = {
				...
				repo: urlFragments[4],
				number: _.parseInt(_.last(urlFragments))
			}
	...
	async createIssueComment (event, actor) {
		...
		if (await this.getCardByMirrorId(getEventMirrorId(event), 'comment')) {
			return []
		}
	...
	async editIssueComment (event, actor) {
		...
		const commentMirrorId = getEventMirrorId(event)
		const comment = await this.getCardByMirrorId(commentMirrorId, 'comment')
		if (comment) {
		...
		for (const item of result) {
			...
			if (!await this.getCardByMirrorId(githubUrl, 'comment')) {
				sequence.push(item)
			}
		}
	...
	async getCommentsFromIssue (context, event, target, mirrorBlacklist, options) {
		...
		return Bluebird.reduce(response, async (accumulator, payload) => {
			...
			const card = await this.getCardFromEvent(mirrorId, 'comment')
			const data = {
