...
module.exports = class GitHubIntegration {
	...
	async getEventInfo (event) {
		...
		const card = await this.getCardByMirrorId(mirrorID)
		const root = getEventRoot(event)
	...
	async createPRIfNotExists (event) {
		...
		const openCard = getPRFromEvent(event, {
			status: 'open'
		})
	...
	async createPRWithConnectedIssues (event) {
		const cards = this.createPRIfNotExists(event)
		if (_.isEmpty(cards)) {
			return []
		}
		const pr = cards[0]
	...
	async labelEventPR (event, action) {
		...
		if (pr) {
			...
			if (!_.isEqual(_.sortBy(card.tags), _.sortBy(pr.tags))) {
				card.id = pr.id
				return [
					{
	...
	async editIssueComment (event) {
		...
		if (index === -1) {
			const upserts = getCommentFromEvent(this.context, event, {
				...
				target,
				targetCard: card || sequence[0].card
			})
