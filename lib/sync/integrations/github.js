...
module.exports = class GitHubIntegration {
	...
	async closePR (event) {
		...
		const prOpened = getPRFromEvent(event, {
			status: 'open'
		})

		const prClosed = getPRFromEvent(event, {
			status: 'closed',
			id: {
				$eval: 'cards[0].id'
			}
	...
	async createPRIfNotExists (event) {
		...
		const card = getPRFromEvent(event, {
			status: 'open'
		})
	...
	async updateIssue (event, action) {
		...
		if (issue) {
			const issueCard = getIssueFromEvent(event, {
				id: issue.id,
				status: 'open'
			})
		...
		const issueCard = getIssueFromEvent(revertEventChanges(event, 'issue'), {
			status: 'open'
		})
		...
		if (action === 'reopened') {
			...
			const closedCard = getIssueFromEvent(revertEventChanges(event, 'issue'), {
				status: 'closed',
				id: {
					$eval: 'cards[0].id'
				}
		...
		const openCard = getIssueFromEvent(event, {
			status: 'open',
			id: {
				$eval: 'cards[0].id'
			}
