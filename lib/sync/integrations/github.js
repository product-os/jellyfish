...
module.exports = class GitHubIntegration {
	...
	async mirror (card, options) {
		...
		if ((card.type === 'issue@1.0.0' || card.type === 'pull-request@1.0.0') && card.data.repository) {
			...
			this.context.log.debug(GITHUB_API_REQUEST_LOG_TITLE, {
				category: card.type === 'pull-request@1.0.0' ? 'pulls' : 'issues',
				action: 'get'
			})
			...
			const result = card.type === 'pull-request@1.0.0'
			...
			if (result.data.state !== card.data.status ||
				...
				if (card.type === 'pull-request@1.0.0') {
					this.context.log.debug(GITHUB_API_REQUEST_LOG_TITLE, {
						category: 'pulls',
						action: 'update'
					})
		...
		if (card.type === 'message@1.0.0') {
			...
			if (!issue || (issue.type !== 'issue@1.0.0' && issue.type !== 'pull-request@1.0.0')) {
				return []
			}
