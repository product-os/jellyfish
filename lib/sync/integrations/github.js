...
module.exports = class GitHubIntegration {
	...
	async mirror (card) {
		...
		const actorCard = await this.context.getElementById(
		...
		if (card.type === 'message') {
			const issue = await this.context.getElementById(
				this.options.session, 'issue', card.data.target)
	...
	async translate (event) {
		...
		if (type === 'issue_comment' && action === 'created') {
				...
				return [
					{
						...
						card: getCommentFromEvent(this.context, event, {
			...
			return sequence.concat(await this.getCommentsFromIssue(this.context, event, {
				$eval: 'cards[0].id'
		...
		if (type === 'issue_comment' && [
			...
			return sequence.concat([
				{
					time: new Date(event.data.payload.comment.updated_at),
					card: index === -1 ? _.merge(getCommentFromEvent(this.context, event, {
						actor: this.options.actor,
						active: true,
						target
	...
	async getIssueByMirrorId (id) {
		return await this.context.getElementByMirrorId(this.options.session, 'issue', id) ||
			this.context.getElementByMirrorId(this.options.session, 'pull-request', id)
	}
