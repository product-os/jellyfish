...
module.exports = class FlowdockIntegration {
	...
	async translate (event) {
		...
		const threadCard = await this.context.getElementByMirrorId('thread', flowThreadMirrorId)
...
const slugify = (event, type) => {
	const baseSlug = `${type}-flowdock-${event.data.payload.thread_id.toLowerCase().replace(/_/g, '-')}`
	return type === 'thread' ? baseSlug : `${baseSlug}-${event.data.payload.id}`
}
...
const getMessageType = (event) => {
	return getRawContent(event).trimStart().startsWith('%') ? 'message' : 'whisper'
}
