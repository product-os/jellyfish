...
const getActor = async (localUser, context) => {
	const username = localUser.type === 'user'
		? localUser.username
		: localUser.email
	const id = await context.getActorId(localUser.type, username)
	if (!id) {
		throw new errors.SyncNoActor(`Not actor id for ${JSON.stringify(localUser)}`)
	}
...
exports.translate = async (card, options) => {
	...
	const actor = await getActor(localUser, options.context)
