...
exports.mirror = async (name, session, context, card, request) => {
	...
	if (!token) {
		logger.warn(logger.context.systemContext, 'Ignoring mirror as there is no token', {
			integration: name
		})
	...
	if (!integration) {
		logger.warn(logger.context.systemContext, 'Ignoring mirror as there is no compatible integration', {
			integration: name
		})
...
exports.translate = async (session, context, card, request) => {
	...
	if (!token) {
		logger.warn(logger.context.systemContext, 'Ignoring translate as there is no token', {
			integration: source
		})
