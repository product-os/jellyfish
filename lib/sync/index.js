 ...
 * @param {String[]} options.scopes - access scopes
...
exports.getAssociateUrl = (integration, token, slug, options) => {
	...
	return oauth.getAuthorizeUrl(Integration.OAUTH_BASE_URL, options.scopes, slug, {
		appId: token.appId,
 ...
 * @param {String} integration - integration name
 * @param {Object} token - token details
 * @param {Any} state - OAuth callback state
 ...
 */
exports.associate = async (integration, token, state, context, options) => {
	const Integration = INTEGRATIONS[integration]
	if (!Integration) {
		context.log.warn(
