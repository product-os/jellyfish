...
const getIssueMirrorIdFromEvent = (event) => {
	return event.data.payload.issue.html_url
}
...
const getIssueFromEvent = (event, options) => {
	const issue = {
		name: event.data.payload.issue.title,
		type: 'issue',
		active: true,
		tags: event.data.payload.issue.labels.map((label) => {
			return label.name
		}),
		links: {},
		data: {
			...
			mentionsUser: [],
			alertsUser: [],
			description: event.data.payload.issue.body,
...
module.exports = class GitHubIntegration {
	...
	async translate (event, options) {
		...
		if (type === 'issues' && _.includes([ 'labeled', 'unlabeled' ], action)) {
			...
			if (issue) {
				const card = getIssueFromEvent(event, {
					status: event.data.payload.issue.state
				})
				...
				if (!_.isEqual(_.sortBy(card.tags), _.sortBy(issue.tags))) {
					...
					return [
						{
							time: new Date(event.data.payload.issue.updated_at),
							card
						}
					]
				}
			...
			if (action === 'labeled') {
				if (event.data.payload.issue.created_at === event.data.payload.issue.updated_at) {
					return [
						{
							time: new Date(event.data.payload.issue.created_at),
							card
						}
					]
				}
				...
				if (event.data.payload.issue.state === 'closed') {
					sequence.push({
						time: new Date(event.data.payload.issue.closed_at),
	...
	async getCommentsFromIssue (event, target, mirrorBlacklist) {
		const response = await this.queryComments(
			...
			event.data.payload.issue.number)
