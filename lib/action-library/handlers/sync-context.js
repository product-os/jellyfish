...
exports.fromWorkerContext = (provider, workerContext, context, session) => {
	...
	const contextObject = {
		...
		upsertElement: async (type, object, options) => {
			...
			// We get the current card the needs to be upserted
			let current = await workerContext.getCardBySlug(
				session, `${object.slug}@${object.version}`)

			// If the object and the current card has mirrors,
			// we use Mirrors as upsert reference
			if (_.has(object, [ 'data', 'mirrors' ])) {
				console.log(' ')
				console.log(' ')
				console.log('object', object.data.mirrors[0])
				const mirrorIdCard = await contextObject.getElementByMirrorId(
					object.type, `${object.data.mirrors[0]}`)
				current = mirrorIdCard || current
				console.log('cardRetrievedByMirrorId', current)
				console.log(' ')
				console.log(' ')
			}
