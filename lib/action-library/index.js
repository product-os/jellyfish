...
module.exports = {
	...
	'action-create-session': {
		...
		pre: async (session, context, request) => {
			const user = isUUID(request.card)
				? await context.getCardById(context.privilegedSession, request.card, {
					type: 'user'
				})
				: await context.getCardBySlug(context.privilegedSession, request.card, {
					type: 'user'
				})

			if (user && _.get(user.data, [ 'password', 'hash' ]) && !user.data.disallowLogin) {
				const authorised = await bcrypt.compare(
					request.arguments.password.hash,
					user.data.password.hash
				)

				request.arguments.password.hash = 'OMITTED'
			...
			throw new errors.AuthenticationError(
				'Incorrect username or password', request.context)
		},
		handler: async (session, context, card, request) => {
	...
	'action-create-user': {
		...
		pre: async (session, context, request) => {
			// If the password is falsey, set it to a value that will never be
			// generated by bcrypt, to prevent login
			if (request.arguments.password) {
				...
				request.arguments.password = 'NO_PASSWORD_SET'
			}
