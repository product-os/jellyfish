...
const mirror = async (type, session, context, card, request) => {
	...
	const cards = await sync.mirror(
		type,
		environment.getIntegrationToken(type),
		card,
		syncContext.fromWorkerContext(context, request.context, session),
		{
			actor: request.actor
		})
...
module.exports = {
	'action-integration-import-event': {
		...
		handler: async (session, context, card, request) => {
			const cards = await sync.translate(
				card.data.source,
				environment.getIntegrationToken(card.data.source),
				card,
				syncContext.fromWorkerContext(context, request.context, session),
				{
					actor: request.actor,
	...
	'action-upsert-card': {
		...
		pre: _.noop,
		handler: async (session, context, card, request) => {
			if (skhema.isValid(context.cards.event.data.schema,
				request.arguments.properties)) {
				throw new Error('You may not use card actions to create an event')
			}
