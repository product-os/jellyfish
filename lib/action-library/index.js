...
module.exports = {
	...
	'action-get-gravatar': {
		...
		handler: async (session, context, card, request) => {
			...
			if (
				!email || _.has(card, [ 'data', 'profile', 'gravatar' ])
			) {
				return {
					id: card.id,
					slug: card.slug,
			...
			const typeCard = await context.getCardBySlug(
				session, 'user', typeGetOptions)

			assert.INTERNAL(request.context, typeCard,
				context.errors.WorkerNoElement, 'No such type: contact')

			logger.info(request.context, 'Patching user gravatar', {
				slug: card.slug,
				patch
			})
