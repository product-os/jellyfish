...
module.exports = {
	...
	// TODO: Get rid of this action, as clients
	// should either insert, or patch
	'action-upsert-card': {
		...
		handler: async (session, context, card, request) => {
			...
			const object = await context.getCardBySlug(
				session, request.arguments.properties.slug, {
					type: card.slug
				})
			...
			if (!object) {
				const result = await context.insertCard(
					session, card, options, request.arguments.properties)
			...
			// TODO: Expose a JSON patch interface in the action
			// so we don't have to do this diff comparison thing.
			const updatedCard = _.mergeWith(
				{}, object, request.arguments.properties, (objectValue, sourceValue) => {
			...
			const patch = jsonpatch.compare(
				...
				context.defaults(Object.assign({}, updatedCard, {
					type: card.slug,
					name: typeof updatedCard.name === 'string'
						? updatedCard.name
						: null
				})))

			const result = await context.patchCard(
				session, card, options, object, patch)
			if (!result) {
				return null
			}
	...
	'action-update-card': {
		...
		handler: async (session, context, card, request) => {
			const updatedCard = _.mergeWith(
				{}, card, request.arguments.properties, (objectValue, sourceValue) => {
					if (_.isArray(objectValue)) {
						return sourceValue
					}
			...
			if (fastEquals.deepEqual(card, updatedCard)) {
				return {
					id: card.id,
					type: card.type,
					slug: card.slug
