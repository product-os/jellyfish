...
const jsonSchema = require('../core/json-schema')
...
const assertNotEvent = (card) => {
	if (jsonSchema.isValid(CARDS.event.data.schema, card)) {
		throw new Error('You may not use card actions to create an event')
	}
}

module.exports = {
	'action-integration-import-event': sync.translate,
	'action-integration-github-mirror-event': _.partial(sync.mirror, 'github'),
	'action-integration-front-mirror-event': _.partial(sync.mirror, 'front'),
	...
	'action-create-event': async (session, context, card, request) => {
		...
		return context.insertCard(session, typeCard, {
			...
			slug: request.arguments.slug || context.getEventSlug(typeCard.slug, data),
			version: '1.0.0',
