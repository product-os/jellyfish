...
module.exports = {
	...
	'action-create-session': {
		...
		pre: async (session, context, request) => {
			const target = isUUID(request.card)
			...
			if (request.arguments.password.hash) {
				// Convert the plaintext password into a hash so that we don't have
				// a plain password stored in the DB
				request.arguments.password.hash.string = hashPBKDF({
					string: request.arguments.password.hash.string,
					salt: request.arguments.password.hash.salt
				})
			}
	...
	'action-create-user': {
		...
		pre: async (session, context, request) => {
			// Convert the plaintext password into a hash so that we don't have
			// a plain password stored in the DB
			request.arguments.password.string = hashPBKDF({
				string: request.arguments.password.string,
				salt: request.arguments.password.salt
			})
			...
			try {
				const result = await context.insertCard(session, card, {
					...
					data: {
						...
						password: {
							hash: request.arguments.password.string
						}
					}
				})
