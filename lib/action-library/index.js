...
const mirror = async (type, session, context, card, request) => {
	...
	if (request.originator) {
		...
		if (originator &&
			originator.type === 'external-event' &&
...
module.exports = {
	...
	'action-google-meet': {
		...
		handler: async (session, context, card, request) => {
			...
			if (!credentialsEnvVar) {
				throw new Error(
					'Google meet creadentials environment variable was not found!'
				);
			}
			const credentials = JSON.parse(credentialsEnvVar);

			const auth = new google.auth.JWT({
				email: credentials.client_email,
				key: credentials.private_key,
				scopes: ['https://www.googleapis.com/auth/calendar'],
				// `Subject` required to impersonate real account using service account (neccessary for creating events with meet URLs)
			...
			const event = await calendarAPI.events.insert(
				{
					...
					requestBody: {
						...
						conferenceData: {
							createRequest: {
								requestId: now.valueOf().toString(),
	...
	'action-maintain-contact': {
		...
		handler: async (session, context, card, request) => {
			...
			const originCard = card.data.origin && (uuid.isUUID(card.data.origin)
				...
				: await context.getCardBySlug(session, `${card.data.origin}@latest`))
	...
	'action-set-password': {
		...
		pre: async (session, context, request) => {
			...
			const loginResult = isFirstTimePassword
				...
				: await module.exports['action-create-session']
					.pre(session, context, {
						card: request.card,
	...
	'action-create-event': {
		...
		handler: async (session, context, card, request) => {
			const typeCard = await context.getCardBySlug(
				session, `${request.arguments.type}@latest`)
	...
	'action-increment-tag': {
		...
		handler: async (session, context, card, request) => {
			...
			return Bluebird.map(names, async (item) => {
				...
				try {
					return await module.exports['action-create-card'].handler(
						session,
						context,
						card,
						createOptions)
