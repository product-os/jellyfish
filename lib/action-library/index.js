...
const assertNotEvent = (card) => {
	if (skhema.isValid(CARDS.event.data.schema, card)) {
		throw new Error('You may not use card actions to create an event')
	}
}

const getIntegrationContext = (context) => {
	return {
		insertCard: context.insertCard,
		getElementById: async (session, type, id) => {
			return context.getCardById(session, id, {
				type
			})
		},
		getElementBySlug: async (session, type, slug) => {
			return context.getCardBySlug(session, slug, {
				type
			})
		},
		getElementByMirrorId: async (session, type, mirrorId) => {
			const results = await context.query(session, {
				type: 'object',
...
module.exports = {
	'action-integration-import-event': async (session, context, card, request) => {
		return sync.translate(session, getIntegrationContext(context), card, request)
	},
	'action-integration-github-mirror-event': async (session, context, card, request) => {
		return sync.mirror('github', session, getIntegrationContext(context), card, request)
	},
	'action-integration-front-mirror-event': async (session, context, card, request) => {
		return sync.mirror('front', session, getIntegrationContext(context), card, request)
	},
	'action-create-card': async (session, context, card, request) => {
		assertNotEvent(request.arguments.properties)
