...
exports.fromWorkerContext = (workerContext, context, session) => {
	return {
		...
		getActorId: async (type, username) => {
			...
			const translatedUsername = ACTOR_TRANSLATE_MAP[type][username] || username
			const slug = `${type}-${translatedUsername.toLowerCase().replace(/[^a-z0-9-]/g, '-')}`
			...
			if (type === 'account') {
				data.handle = translatedUsername.split('@')[0]
				if (translatedUsername.includes('@')) {
					data.email = translatedUsername
				}
			} else {
			...
			if (!result) {
				const existentUser = await workerContext.getCardBySlug(session, slug, {
					type: typeCard.slug
				})
