...
const processDataSets = (items = []) => {
	...
	// Group the items by repo
	const repos = _.groupBy(items, 'data.repository')

	const sets = _.map(repos, (sourceData, repo) => {
		const set = {
			label: repo,
			backgroundColor: colorHash(repo),
			data: []
		}
...
export default React.memo((props) => {
	...
	const chartOptions = {
		title: {
			display: true,
			text: `Pull requests created over the last 30 days (${total} total)`
		},
		scales: {
			xAxes: [
				{
