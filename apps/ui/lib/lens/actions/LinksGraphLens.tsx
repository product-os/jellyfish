...
import { CloseButton, Column, helpers } from '@balena/jellyfish-ui-components';
...
export class LinksGraphLens extends React.Component<any, any> {
	...
	constructor(props) {
		...
		const { card } = this.props.channel.data.head;
	...
	async loadNodeLinks(card) {
		_.map(card.links, async (linkedCards, verb) => {
			for (const item of linkedCards) {
				...
				const fullCard: any = await sdk.card.get(item.id);
	...
	componentDidMount() {
		...
		const drawNode = (node) => {
			...
			const color =
				node.id === cardId ? '#2297DE' : helpers.colorHash(node.type);
			context.beginPath();
...
const mapDispatchToProps = (dispatch) => {
	return {
		actions: {
			removeChannel: redux.bindActionCreators(
				actionCreators.removeChannel,
				dispatch,
			),
		},
	};
