...
import { Box, Steps, Step, Button, Txt, Flex } from 'rendition';
...
export default function AuditPanel(props) {
	...
	useEffect(() => {
		...
		(Bluebird as any)
			.map(actorIds, props.actions.getActor)
			.then((actors: any[]) => {
				setAgents(
					_.filter(actors, {
						proxy: false,
	...
	useEffect(() => {
		sdk.card.getWithLinks(props.card.id, ['is source for']).then((result) => {
			if (result) {
	...
	const createFeedback = (feedback) => {
		...
		sdk.card
			...
			.then((result: any) => {
				...
				return (Bluebird as any).all([
					sdk.card.link(props.card, result, 'is source for'),
					sdk.card.link(result, agent, 'is feedback for'),
				]);
			})
			.catch((error) => {
		...
		>
			<Steps m={3} titleText="Audit">
				...
				<Step
					...
					onClick={step > 1 ? () => setStep(1) : (null as any)}
				>
					Provide agent feedback
				</Step>
				<Step
					status={step > 2 ? 'completed' : 'pending'}
					onClick={step > 2 ? () => setStep(2) : (null as any)}
				>
					Finished
				</Step>
			</Steps>
			...
			>
				{step === 0 && (
					<React.Fragment>
						<Box>
							<Txt>
								Is there a product feature that can reduce this friction in the
								future?
							</Txt>
							<Txt>
								Can we improve documentation to remove friction for our users?
							</Txt>
						</Box>
				...
				{step === 1 && (
					<React.Fragment>
							...
							<Box>
								...
								{_.map(agents, (agent) => {
									return (
										...
										>
											<Txt>
												<strong>{agent.card.slug.slice(5)}</strong>
											</Txt>
						...
						<Flex mt={4}>
							<Button data-test="skip-step" onClick={skipStep}>
								Done
							</Button>
						</Flex>
					</React.Fragment>
			...
			{feedbackFor && <Feedback user={feedbackFor} done={createFeedback} />}
		</Box>
	);
}
