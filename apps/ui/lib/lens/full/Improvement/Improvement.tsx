...
export default class Improvement extends React.Component<Props, State> {
	...
	async componentDidMount() {
		...
		this.cursor = sdk.getCursor(milestoneQuery, { sortBy: 'created_at' });
	...
	render() {
		...
		return (
			<TabbedContractLayout card={card} channel={channel}>
				<CardFields card={card} type={type} viewMode={UI_SCHEMA_MODE.fields} />
				<Heading.h2>Milestones</Heading.h2>
				{Boolean(snippetLens) && (
					<Box mx={-3}>
						{milestones.map((c) => {
							return (
								<snippetLens.data.renderer card={c} types={this.props.types} />
							);
						})}

						<ViewFooter types={[helpers.getType(milestones[0].type, types)]} />
					</Box>
				)}
