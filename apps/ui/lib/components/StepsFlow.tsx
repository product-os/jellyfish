...
import { Box, Button, Divider, Heading, Flex, Steps, Step } from 'rendition';
...
const FlowStepFlex = styled(Flex)`
	...
	overflow-y: ${(props: any) => {
		return props.scrollable ? 'auto' : 'unset';
	}};
`;

const nonePending = (stepStatuses) => {
	return _.every(stepStatuses, (status) => {
 ...
 */
const FlowStep = ({ stepIndex, label, title, status, children, ...rest }) => {
	return (
		<FlowStepFlex
			pt={4}
			px={4}
		...
		>
			<Heading.h5 data-test={`flow-step-${stepIndex}__heading`} mb={2}>
				{title || label}
			</Heading.h5>
			{children}
		</FlowStepFlex>
 ...
 */
const StepsFlow: React.FunctionComponent<any> & { Step: any } = ({
	title,
	titleIcon,
	initialStep,
	onClose,
	...
	const steps: any[] = _.compact(children);
	...
	return (
		...
		>
				...
				>
					{React.Children.map(steps, (step, stepIndex) => {
						...
						if (!VALID_STEP_STATUSES[step.props.status]) {
							throw new Error(`Invalid step status: ${step.props.status}`);
						}
			...
			>
				...
				{activeStepIndex === steps.length - 1 && (
					<Button
						icon={busy ? <Icon name="cog" spin /> : (null as any)}
