...
export default function NewOwnerStep({
	...
	const setNewOwner = (owner) => {
		...
		setFlow({
			newOwner: owner,
			userError:
				currentOwner && _.get(owner, ['id']) === _.get(currentOwner, ['id'])
					? `${currentOwner.slug} already owns this ${cardTypeName}`
					: null,
		});
	};
	...
	return (
		<React.Fragment>
			<Flex flexDirection="row" alignItems="center">
				...
				/>
				{userError && (
					<Txt data-test="ghf__user-error" ml={2} color="red">
						{userError}
					</Txt>
				)}
			</Flex>
