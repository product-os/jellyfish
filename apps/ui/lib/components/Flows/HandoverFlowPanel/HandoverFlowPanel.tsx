...
import { NewOwnerStep } from './Steps';
...
export default function HandoverFlowPanel({
	...
	const { newOwner, userError, unassigned, reason, statusDescription } =
		flowState;
	...
	const transferOwnership = async () => {
		try {
			...
			if (cardOwner && newOwner && cardOwner.id === newOwner.id) {
				notifications.addNotification(
					...
					`${helpers.userDisplayName(
						cardOwner,
					)} already owns this ${cardTypeName}`,
				);
				return;
			}
			...
			if (cardOwner) {
				...
				console.warn(
					`No current owner found when unassigning the card ${card.slug}`,
				);
			}
			...
			notifications.addNotification(
				'danger',
				'Handover failed. Refresh the page and try again.',
			);
			return;
		}
	...
	const newOwnerDisplayName = newOwner
		? helpers.userDisplayName(newOwner)
		: null;
	...
	return (
		...
		>
			<StepsFlow.Step
				...
				status={stepStatus(completed.ownership)}
			>
				<NewOwnerStep
					user={user}
			...
			<StepsFlow.Step
				label="Reason"
				title="Give a reason"
				status={stepStatus(completed.reason)}
			>
				<TextareaStep
					placeholder={
						unassigned
			...
			{supportsStatus && (
				<StepsFlow.Step
					...
					status={stepStatus(completed.statusDescription)}
				>
					<TextareaStep
