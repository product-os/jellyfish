...
class MermaidEditor extends React.Component<any, any> {
	renderArea: any;
	...
	render() {
		const { preview, splitview, fullscreen } = this.state;
		...
		return (
			<Provider>
				<Flex
					...
					style={fullscreen || splitview ? fullScreenStyle : smallscreenstyle}
				>
					<Box
						style={{
					...
					>
						<div
							style={{
								display:
									(!splitview && !preview) || splitview ? 'block' : 'none',
								minHeight: '100%',
								width: splitview ? '50%' : '100%',
							}}
							ref={(element) => {
								return (this.renderArea = element);
							}}
						/>
