...
const NOTES = ['note over', 'note left of', 'note right of'];
...
monaco.languages.setMonarchTokensProvider('mermaid', {
	tokenizer: {
		root: [
			...
			[
				/(graph\s(TB|BT|RL|LR|TD)|sequenceDiagram|classDiagram|stateDiagrami|gantt|pie)/,
				'type',
			],
			[/:.+$/, 'statement'],
...
monaco.languages.registerCompletionItemProvider('mermaid', {
	provideCompletionItems: (model, position) => {
		...
		const actors = _.uniq(
			_.invokeMap(
				value.match(ACTOR_REGEXP),
				String.prototype.replace,
				ARROW_REMOVAL_REGEXP,
				'',
			),
		);

		const suggestions = [...actors, ...ARROWS, ...NOTES, ...NOUNS].map(
			(word) => {
				return {
					label: word,
