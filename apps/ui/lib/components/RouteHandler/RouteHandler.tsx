...
import { getLensForTarget } from '../../lens';
import ChannelRenderer from '../ChannelRenderer';

const PATH_SEPARATOR = '...';

export default class RouteHandler extends React.Component<any, any> {
	constructor(props) {
		super(props);
		this.state = {
			spaces: [],
	...
	calcWidth(channels) {
		...
		const width =
			window.innerWidth - sidebarWidth - Math.max(squished * squishedWidth, 0);
	...
	render() {
		const { types, actions, channels, user } = this.props;
		const { spaces } = this.state;
		const userHasOrg =
			Boolean(user) && _.get(user, ['linked_at', 'is member of']);
		...
		return (
			<React.Fragment>
				...
				{!channels.slice(1).length && !userHasOrg && (
					<Flex flex="1" justifyContent="center" pt="20%">
						<p
							style={{
								textAlign: 'center',
							}}
						>
							Looks like you are not part of an organisation yet.
							<br />
