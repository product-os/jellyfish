...
describe('Redux store reducers - core', () => {
	...
	test("SET_FLOW adds flow state if it doesn't already exist", () => {
		const initialState = _.cloneDeep(defaultState);
		...
		expect(newState.ui.flows).toEqual({
			[flowId]: {
				[cardId]: {
					someItem: 'someValue',
				},
	...
	test('SET_FLOW merges an existing flow state', () => {
		const initialState = _.cloneDeep(defaultState);
		const flowId = 'HANDOVER';
		const cardId = '3';
		const flowState = {
	...
	test('REMOVE_FLOW removes flow state', () => {
		const initialState = _.cloneDeep(defaultState);
		const flowId = 'HANDOVER';
		const cardId = '3';
		const flowState = {
